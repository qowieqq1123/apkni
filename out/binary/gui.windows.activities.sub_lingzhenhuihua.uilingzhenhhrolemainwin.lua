...
def_class("UILingZhenHHroleMainWin",UIWindowBase)
...
function UILingZhenHHroleMainWin:bindComponents()
...
function UILingZhenHHroleMainWin:unbindComponents()
...
function UILingZhenHHroleMainWin:onLoaded(...)
self:bindComponents()
_this=self
end


function UILingZhenHHroleMainWin:__delete()
_this=nil
...
function UILingZhenHHroleMainWin:onShow(argtable,afterOnloaded)
...
local npcSpeakParam=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"npcSpeakParam")
...
function UILingZhenHHroleMainWin:onShowArgRecv(argtable,afterOnloaded)
...
function UILingZhenHHroleMainWin:onHide()
self:clearTimer()
self:clearSpeakTimer()
self:clearEffect()
end

function UILingZhenHHroleMainWin:refresh(isInit)

self.campId=self.info:getLingZhenCamp()
...
self.nextUpdateDataTime=self.info:getNextUpdateDataTime()
...
function UILingZhenHHroleMainWin:refreshSelectCampPanel()
...
function UILingZhenHHroleMainWin:refreshStartGamePanel(isChange)
if isChange then
self.campId=self.info:getLingZhenCamp()
...
self.campShowPanel1:setActive(false)
self.campShowPanel2:setActive(false)
...
function UILingZhenHHroleMainWin:refreshProgressPanel(isNeedJump)
local allRewardCfg=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"stage")
local showRewardCount=#allRewardCfg


local todayTopScore=self.info:getTopScore()or 0
...
if cfg then
local targetScore=cfg[1]
local reward=cfg[2][1]
...
function UILingZhenHHroleMainWin:refreshCampShowPanel(campId)
...
local campData=self.info:getLingZhenCampScoreDataByCampId(campId)
if campData then
...
local allCampNameCfg=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"camp")
...
local otherCampData=self.info:getLingZhenCampScoreDataByCampId(otherCampId)
...
local allRewardList=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"reward")
...
function UILingZhenHHroleMainWin:clearEffect()
for i=1,2 do
...
function UILingZhenHHroleMainWin:setRemainingTimeTimer()
if self.info then
self:clearTimer()
local func=function()
...
function UILingZhenHHroleMainWin:clearTimer()
if self.timer then
...
function UILingZhenHHroleMainWin:doSpeaking()
self:clearSpeakTimer()

local speakLib=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"npcSpeakLib")
...
function UILingZhenHHroleMainWin:doTalkAnim()
...
function UILingZhenHHroleMainWin:talkEnd()
self:clearSpeakTimer()
...
function UILingZhenHHroleMainWin:clearSpeakTimer()
...
function UILingZhenHHroleMainWin:onCloseBtn()
jumpManager:jump({id=JUMP_TYPE.eActivity,args={subType=self.subType,subid=self.subid}},function()
UIFullLingZhenHuiHuaRoleControl:closeUI(true)
...
function UILingZhenHHroleMainWin:onHelpBtn()
end



function UILingZhenHHroleMainWin:onStartBtn()
UIFullLingZhenHuiHuaRoleControl:showGameWin(_this.activityArgs)

end



function UILingZhenHHroleMainWin:onRuleBtn()
...
function UILingZhenHHroleMainWin:onShopBtn()
local config=cfgHelper.get1(cfg_lingzhenhuihua2config_get,self.subid)
...
function UILingZhenHHroleMainWin:onLianzhiBtn()
...
function UILingZhenHHroleMainWin:onSelectCampBtn1()
...
function UILingZhenHHroleMainWin:onSelectCampBtn2()
...
function UILingZhenHHroleMainWin:onRandomCampBtn()
if self.campId and self.campId~=0 then
...
function UILingZhenHHroleMainWin:onClickRewardItem(itemId,index,guid,attach)
if itemId==-1 then
return
end
