...
def_class("UIXianJie_filteHUD2Win",UIWindowBase)
...
function UIXianJie_filteHUD2Win:bindComponents()
...
function UIXianJie_filteHUD2Win:unbindComponents()
...
function UIXianJie_filteHUD2Win:onLoaded(...)
_this=self
self:bindComponents()
...
function UIXianJie_filteHUD2Win:__delete()
_this=nil
self:unbindComponents()
end


function UIXianJie_filteHUD2Win:onHide()
...
function UIXianJie_filteHUD2Win:onShow(argtable,afterOnloaded)
self:refreshInfoPanel()
end

function UIXianJie_filteHUD2Win:refreshInfoPanel()
if self.filterList==nil or self.refreshFlag then
self.refreshFlag=false
self.filterList,self.lookUpkeyList=xianjieModel:getFilterHUD2Cfg()
self.filterRecord=xianjieModel:getFilterHUD2Record()
...
function UIXianJie_filteHUD2Win:onCtrlAllClick(idx)
...
self.filterRecord=xianjieController:getFilterHUD2Record()
end
...
xianjieModel:changeFilterHUD2Ex(type1)
end



function UIXianJie_filteHUD2Win:onFilterItemClick(idx)
local type1=self.lookUpkeyList[idx].type1
local list1=self.lookUpkeyList[idx].list1
local len=#list1
...
self:refeshFilterItemSelect(nil,idx)
self:openFilter2(false)
xianjieModel:changeFilterHUD2(type1)
else
...
function UIXianJie_filteHUD2Win:refeshFilterItemSelect(item,idx)
if item==nil then
...
function UIXianJie_filteHUD2Win:openFilter2(flag,idx,moveX,moveY)
if flag then
local type1=self.lookUpkeyList[idx].type1
local list1=self.lookUpkeyList[idx].list1
...
function UIXianJie_filteHUD2Win:onFilter2ItemClick(sidx)
...
self:refeshFilterItemSelect(nil,idx)
self:openFilter3(false)
xianjieModel:changeFilterHUD2(type1,type2)
else
...
function UIXianJie_filteHUD2Win:refeshFilter2ItemSelect(item,sidx)
if item==nil then
...
function UIXianJie_filteHUD2Win:onBlock()
...
UIManager:invokeUIMethod('UIXianJieMainWin','closeFilterHUD2Win2')
self:closeSelf()
end

function UIXianJie_filteHUD2Win:openFilter3(flag,sidx,moveX,moveY)
if flag then
local idx=self.filter2Index
...
self.filter3Panel:setChildScrollRectEnable(c>maxShowCnt)
...
function UIXianJie_filteHUD2Win:onFilter3ItemClick(ssidx)
local idx=self.filter2Index
...
self:refeshFilter2ItemSelect(nil,sidx)
self:refeshFilterItemSelect(nil,idx)
xianjieModel:changeFilterHUD2(type1,type2,type3)
end

function UIXianJie_filteHUD2Win:refeshFilter3ItemSelect(item,ssidx)
if item==nil then
...
function UIXianJie_filteHUD2Win:getFilterRecord(type1,type2,type3)
if not self.filterRecord then
return true
end
if not self.filterRecord[type1]then
...
function UIXianJie_filteHUD2Win:setFilterRecord(type1,type2,type3,flag)
if not self.filterRecord then
self.filterRecord=xianjieModel:getFilterHUD2Record()
end
if not self.filterRecord[type1]then
...
function UIXianJie_filteHUD2Win:refreshCurWin()
