...
local catEntrustSystem_EZMJ=catEntrustSystemBase.new({sysType=Entrust_Type.EZMJ})

function catEntrustSystem_EZMJ.getIcon(data)
return cfgHelper.get3(cfg_secretsceneziyuanfubenconfig_get,data.tagId,data.curId,'image')
end

function catEntrustSystem_EZMJ.getExInfoIcon(data)
...
function catEntrustSystem_EZMJ.getName(data)
local config=cfgHelper.get2(cfg_secretsceneziyuanfubenconfig_get,data.tagId,data.curId)
local curLayer=data.diff
return FMT.fmt("{0}阶{1}",curLayer,config.name)
end

function catEntrustSystem_EZMJ.getRewardInfo(val,data)
return FMT.fmt("装备宝箱+{0}",val)
end

function catEntrustSystem_EZMJ.checkShowWtCondition(data)
return true
end

function catEntrustSystem_EZMJ.getWtConditionInfo(data)
...
function catEntrustSystem_EZMJ.getDoingWtInfo()
return"秘境探索中"
end

function catEntrustSystem_EZMJ.checkShowSlider(data)
return true
end

function catEntrustSystem_EZMJ.getInitSliderData(wtInfo)
...
function catEntrustSystem_EZMJ.restockWtArgs(wtdata,args)
...
function catEntrustSystem_EZMJ.getToServerArgs(exclusiveData)
return{exclusiveData.mjid,exclusiveData.count}
end

function catEntrustSystem_EZMJ.getDoingPlayTxt(wtSlotData)
...
function catEntrustSystem_EZMJ.checkWtCondition(wtSlotData)
...
function catEntrustSystem_EZMJ.finishCallBack(wtSlotData)

end

function catEntrustSystem_EZMJ.doProgressNext(wtSlotData)
...
function catEntrustSystem_EZMJ.getSelectDesc(cnt)
return FMT.fmt("探索次数：{0}",cnt)
end

function catEntrustSystem_EZMJ.checkLocalizeOriginalData(exclusiveData)
...
function catEntrustSystem_EZMJ.transEntrustTypeServer(wtSlotData)
...
function catEntrustSystem_EZMJ.checkPrepareNoOpenPass(wtSlotData)
local noPass=true
...
function catEntrustSystem_EZMJ.getEntrustCost(wtSlotData)
local exclusiveData=wtSlotData.data.exclusiveData
...
function catEntrustSystem_EZMJ.checkSelectWtIsCanNext(wtSlotData)
return wtSlotData.canMaxDiff>wtSlotData.diff
end

function catEntrustSystem_EZMJ.checkShowGoUnlockNextDialouge(wtSlotData,callback)
...
function catEntrustSystem_EZMJ.checkComfireWt(wtSlotData)
