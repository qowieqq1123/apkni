...
def_class("tipsChildVocEquipBaseAttr",UICloneObject)





tipsChildVocEquipBaseAttr.abName="ui/windows/tips/child/tipschildvocequipbaseattr.ab"

tipsChildVocEquipBaseAttr.assetName="tipsChildVocEquipBaseAttr"


function tipsChildVocEquipBaseAttr:bindComponents()
...
function tipsChildVocEquipBaseAttr:unbindComponents()
...
function tipsChildVocEquipBaseAttr:onLoaded(...)
self:bindComponents()
end


function tipsChildVocEquipBaseAttr:__delete()
...
function tipsChildVocEquipBaseAttr:onShow(args,afterOnloaded)
...
_lvl=self.showEnhancelv
end
local baseAttr=equip and vocEquipHelper.getVocEquipBaseAttrsListByItemguid(itemguid,itemid)or
vocEquipHelper.getVocEquipBaseAttrsListByItemidAndLevel(itemid,_lvl)
...
local name,valstr
if attrId>0 then
name,valstr=equipsHelper.getAttr(attrId,attrValue)
else
name=v.attrName
if v.isPercent then
...
function tipsChildVocEquipBaseAttr:onHide()

end

function tipsChildVocEquipBaseAttr:onRecycle()
self.line:setActive(not self:isLastItem())
end
