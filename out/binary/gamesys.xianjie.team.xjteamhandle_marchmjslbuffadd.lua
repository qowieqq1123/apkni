...
local xjTeamHandle_marchMJSLBuffAdd={}


function xjTeamHandle_marchMJSLBuffAdd:onInit()
self.onlykey=FMT.fmt('{0}_{1}',self.teamType,self.marchguid_str)
self.teamData=xianjieModel:getMarchTeamData(self.marchguid)
end


function xjTeamHandle_marchMJSLBuffAdd:checkMyEnemyType()
...
local entityType=xianjieModel:getEntityTypeByGuid(teamData.taractorid,teamData.tarsceneidx)
if entityType==xjServerEnityType.eActor then
return xianjieModel:checkPVPEnemyType(teamData.srcactorid,teamData.taractorid)

elseif entityType==xjServerEnityType.eClientBuild and xianjieModel:isMoJunBuild_int64(teamData.taractorid)then
return xianjieModel:checkEnemyType(teamData.srcactorid)

elseif entityType==xjServerEnityType.eClientBuild and xianjieModel:isMoJiangBuild_int64(teamData.taractorid)then
return xianjieModel:checkEnemyType(teamData.srcactorid)
...
function xjTeamHandle_marchMJSLBuffAdd:getMarchtype()
return self.teamData and self.teamData.marchtype or-1
end

function xjTeamHandle_marchMJSLBuffAdd:getSpeedList()
return self.teamData.speedlist
end

function xjTeamHandle_marchMJSLBuffAdd:getSpeedCnt()
return#self:getSpeedList()
end



function xjTeamHandle_marchMJSLBuffAdd:getSpeedMulti()
...
function xjTeamHandle_marchMJSLBuffAdd:getBeginTime()
local speedlist=self.teamData.speedlist
return speedlist[1].param_1
end


function xjTeamHandle_marchMJSLBuffAdd:getZongMenPos()
local teamData=self.teamData
...
function xjTeamHandle_marchMJSLBuffAdd:getTargetPos()
local teamData=self.teamData
...
function xjTeamHandle_marchMJSLBuffAdd:getTargetData()
local teamData=self.teamData
if teamData then
if teamData.infoguid then
...
return xianjieModel:getXianMengData(teamData.taractorid)

else
return xianjieModel:getEntityDataByGuid(teamData.taractorid,teamData.tarsceneidx)
end
...
function xjTeamHandle_marchMJSLBuffAdd:getTeamState()
...
function xjTeamHandle_marchMJSLBuffAdd:getMoveWayTime(isBack)
...
function xjTeamHandle_marchMJSLBuffAdd:getMyMovePath(isBack)
local movePath
if isBack then
...
function xjTeamHandle_marchMJSLBuffAdd:getMoveTagList(movePath,bTime)
local speedlist=self:getSpeedList()
return xianjieController:getMoveTagList(movePath,bTime,speedlist)
end

function xjTeamHandle_marchMJSLBuffAdd:geLerpTime()
...
function xjTeamHandle_marchMJSLBuffAdd:getOwnerName()
local teamData=self.teamData
...
function xjTeamHandle_marchMJSLBuffAdd:getTeamEnityKey()
return self.teamData:getTeamEnityKey()
end

function xjTeamHandle_marchMJSLBuffAdd:getTargetName(state)
...
str=FMT.fmt("前往{0}({1},{2})",zmData.actorname,teamData.tarx,teamData.tary)
else
...
if xmData then
str=FMT.fmt("前往{0}({1},{2})",xmData.guildname,teamData.tarx,teamData.tary)
else
local monsterData=xianjieModel:getMonsterData(teamData.taractorid)
if monsterData then
local cfg=monsterData:getCfg()
if cfg and cfg.monster and cfg.monster[1]then
local groupid=cfg.monster[1]
local groupcfg=cfgHelper.get1(cfg_monstergroup_get,groupid)
str=FMT.fmt("前往{0}({1},{2})",groupcfg.name,teamData.tarx,teamData.tary)
else
str=FMT.fmt("前往目标({1},{2})",teamData.tarx,teamData.tary)
end
else
local entityData=xianjieModel:getEntityDataByGuid(teamData.taractorid,teamData.tarsceneidx)
if entityData and entityData.monster_id then
local monsterCfg=cfgHelper.get1(cfg_monstergroup_get,entityData.monster_id)
str=FMT.fmt("前往{0}({1},{2})",monsterCfg.name,teamData.tarx,teamData.tary)
else
str=FMT.fmt("前往目标({0},{1})",teamData.tarx,teamData.tary)
end
...
function xjTeamHandle_marchMJSLBuffAdd:getTargetIconName(state)
state=state or self:getTeamState()
...
function xjTeamHandle_marchMJSLBuffAdd:getTargetDesc(state)
state=state or self:getTeamState()
...
function xjTeamHandle_marchMJSLBuffAdd:getStateIconName(state)
state=state or self:getTeamState()
...
function xjTeamHandle_marchMJSLBuffAdd:checkIsShowProgress(state)
state=state or self:getTeamState()
...
function xjTeamHandle_marchMJSLBuffAdd:checkIsOnMove(state,isBack)
state=state or self:getTeamState()
if state==xjMarchTeamStateType.eGoto then
...
function xjTeamHandle_marchMJSLBuffAdd:checkMyWaiPai()
return self.teamData.isMyWaiPai
end


function xjTeamHandle_marchMJSLBuffAdd:maskSpeeUp()
return true
end

function xjTeamHandle_marchMJSLBuffAdd:checkSpeeUp(isWarning)
...
function xjTeamHandle_marchMJSLBuffAdd:doSpeedUp(itemid)
...
function xjTeamHandle_marchMJSLBuffAdd:onSpeedUp()
self.movePath2=nil
self.movePath1=nil
end
...
function xjTeamHandle_marchMJSLBuffAdd:checkIsCanRetract()
local state=self:getTeamState()
return state==xjMarchTeamStateType.eBack
end

function xjTeamHandle_marchMJSLBuffAdd:doRetract()
...
function xjTeamHandle_marchMJSLBuffAdd:onDelete()

end


function xjTeamHandle_marchMJSLBuffAdd:getMarchTeamModelSkinID()
...
return xjTeamHandle_marchMJSLBuffAdd