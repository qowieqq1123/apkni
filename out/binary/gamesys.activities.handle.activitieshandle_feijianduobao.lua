...
activitiesHandle_feijianduobao=new_activitiesHandle('activitiesHandle_feijianduobao',activitiesHandle)

function activitiesHandle_feijianduobao:onInit()

end

function activitiesHandle_feijianduobao:reqExchange(actId,subId,giftIdx,num)

local subType=SUB_ACTIVITY_TYPE.eFeiJianDuoBao
...
function activitiesHandle_feijianduobao:reqRepair(actId,subId)
...
function activitiesHandle_feijianduobao.recv_247_25(args)
local subType=SUB_ACTIVITY_TYPE.eFeiJianDuoBao
local actId=args[1]
local subId=args[2]
local great=args[3]
...
UIManager:invokeUIMethod("UISubAct_FeiJianDuoBaoWin","refreshView",actId,subType,subId)
UIManager:invokeUIMethod("UISubAct_FeiJianDuoBaoExchangeWin","refreshView",actId,subType,subId)
reddotControl.on_change_catch_type(CATCH_TYPE.eActivityChange,subType)
end

function activitiesHandle_feijianduobao.recv_247_26(actId,subId,great,free,lib_idx)
local subType=SUB_ACTIVITY_TYPE.eFeiJianDuoBao
...
function activitiesHandle_feijianduobao.recv_247_27(actId,subId,fix_time,great)
local subType=SUB_ACTIVITY_TYPE.eFeiJianDuoBao
local infoData=activitiesModel:getSubActInfo(actId,subType,subId)
if infoData then
...
UIManager:invokeUIMethod("UISubAct_FeiJianDuoBaoWin","play_Animation",actId,subType,subId)
end
UIManager:invokeUIMethod("UISubAct_FeiJianDuoBaoWin","refreshView_OnlyNext",actId,subType,subId)
reddotControl.on_change_catch_type(CATCH_TYPE.eActivityChange,subType)
end
end

function activitiesHandle_feijianduobao.recv_247_28(actId,subId,idx,num)
local subType=SUB_ACTIVITY_TYPE.eFeiJianDuoBao
local infoData=activitiesModel:getSubActInfo(actId,subType,subId)
if infoData then
