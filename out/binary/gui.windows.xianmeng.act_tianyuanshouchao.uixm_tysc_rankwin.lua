







def_class("UIXM_TYSC_RankWin",UIWindowBase)









function UIXM_TYSC_RankWin:bindComponents()

self.titleTxt=UIText.get(self,0)
self.menuGridPanel=UIObject.get(self,1)
self.rankScrollView=UIObject.get(self,2)
self.rankItem=UIObject.get(self,3)
self.rewardScrollView=UIObject.get(self,4)
self.closeBtn=UIButton.get(self,5)
self.tipsTxt=UIText.get(self,6)
self.noItemTips=UIText.get(self,7)
self.rewardGridPanel=UIObject.get(self,8)
self.rank1ScrollView=UIEnhancedScrollerLua.get(self,9)
self.rank2ScrollView=UIEnhancedScrollerLua.get(self,10)

self.closeBtn:setButtonClick(function()self:onCloseBtn()end)



end


function UIXM_TYSC_RankWin:unbindComponents()
local _UIObject_release=UIObject.release
_UIObject_release(self.titleTxt);self.titleTxt=nil;
_UIObject_release(self.menuGridPanel);self.menuGridPanel=nil;
_UIObject_release(self.rankScrollView);self.rankScrollView=nil;
_UIObject_release(self.rankItem);self.rankItem=nil;
_UIObject_release(self.rewardScrollView);self.rewardScrollView=nil;
_UIObject_release(self.closeBtn);self.closeBtn=nil;
_UIObject_release(self.tipsTxt);self.tipsTxt=nil;
_UIObject_release(self.noItemTips);self.noItemTips=nil;
_UIObject_release(self.rewardGridPanel);self.rewardGridPanel=nil;
_UIObject_release(self.rank1ScrollView);self.rank1ScrollView=nil;
_UIObject_release(self.rank2ScrollView);self.rank2ScrollView=nil;
end
















local UIXM_TYSC_Rank1Scroller=simple_class(UIEnhancedScroller)
local UIXM_TYSC_Rank2Scroller=simple_class(UIEnhancedScroller)
local pageConfig=
{
[1]={
name='仙盟',
checkReddot=function()
return false
end,
open=function(self_)
self_:openXianMengPage()
end,
close=function(self_)
self_:closeXianMengPage()
end,
},
[2]={
name='盟员',
checkReddot=function()
return false
end,
open=function(self_)
self_:openMemberPage()
end,
close=function(self_)
self_:closeMemberPage()
end,
},
[3]={
name='奖励',
checkReddot=function()
return xianmengModel:checkRankReddot_TYSC()
end,
open=function(self_)
self_:openRewardPage()
end,
close=function(self_)
self_:closeRewardPage()
end,
},
}
local _this=nil
local menu_slot_name='button_dytab'


function UIXM_TYSC_RankWin:onLoaded(...)
_this=self
self:bindComponents()
self.scrollscript1=UIXM_TYSC_Rank1Scroller(self.rank1ScrollView:getGameObject(),self.rank1ScrollView:getCSharpObject(),nil,nil)
self.scrollscript2=UIXM_TYSC_Rank2Scroller(self.rank2ScrollView:getGameObject(),self.rank2ScrollView:getCSharpObject(),nil,nil)
end

function UIXM_TYSC_RankWin:refreshMenuItemSelect(item,idx,flag)
if item==nil then
item=self.menuGridPanel:getChildLayoutGroupGridItem(idx-1)
end

if flag then
item:SetChildModelAnimationState(2,eAnimationID.common_window_dianji)
end

item:SetChildUIModelShowSlotAttachment(2,menu_slot_name,flag and FMT.fmt("{0}_{1}",menu_slot_name,2)or FMT.fmt("{0}_{1}",menu_slot_name,1))
end

function UIXM_TYSC_RankWin:refreshMenuItemReddot(item,idx)
if item==nil then
item=self.menuGridPanel:getChildLayoutGroupGridItem(idx-1)
end
local cfg=pageConfig[idx]
local isReddot=cfg.checkReddot()
item:SetChildActive(3,isReddot)
end

function UIXM_TYSC_RankWin:onMenuItemClick(page)
if page==self.curPage then
return
end

local old=self.curPage
self.curPage=page
if old~=nil then
self:refreshMenuItemSelect(nil,old,false)
self:refreshMenuPage(old,false)
end
self:refreshMenuItemSelect(nil,page,true)
self:refreshMenuPage(page,true)
end

function UIXM_TYSC_RankWin:refreshMenuPage(page,flag)
local cfg=pageConfig[page]
if flag then
cfg.open(self)
self:refreshTips()
else
cfg.close(self)
end
end


function UIXM_TYSC_RankWin:__delete()
_this=nil
self:unbindComponents()
end


function UIXM_TYSC_RankWin:onHide()

end




function UIXM_TYSC_RankWin:onShow(argtable,afterOnloaded)
local page=1
self.refreshMark={}
self.curLevel=xianmengModel:getLevelEx_TYSC()
self.curLevelCfg=cfgHelper.get1(cfg_skyshouchaojibieconfig_get,self.curLevel)
local titleName
if self.curLevel>0 then
titleName=xianmengModel:getLevelName_TYSC(self.curLevel)
else
titleName=''
end
local title_str=FMT.fmt('{0}排行榜',titleName)
self.titleTxt:setText(title_str)
if afterOnloaded then
local cnt=#pageConfig
self.menuGridPanel:setChildLayoutGroupCreateItems(cnt)
local grids=self.menuGridPanel:getChildLayoutGroupGridList()
for i=1,cnt do
local item=grids[i-1]
local cfg=pageConfig[i]
item:SetChildText(1,cfg.name)
local isSelected=i==page
local func=function()
if _this==nil then return end
if isSelected then

item:SetChildUIModelShowSlotAttachment(2,menu_slot_name,isSelected and FMT.fmt("{0}_{1}",menu_slot_name,2)or FMT.fmt("{0}_{1}",menu_slot_name,1))
end
end
item:SetChildUIModelShowTarget(2,2017,1,{},eAnimationID.common_window_enter,false,false,0,func)
self:refreshMenuItemSelect(item,i,i==page)
self:refreshMenuItemReddot(item,i)
item:SetChildButtonClick(0,function()
if _this==nil then return end
_this:onMenuItemClick(i)
end)
end
end

self:onMenuItemClick(page)
end

function UIXM_TYSC_RankWin:getRankReward(typo,rank,look)
if self.rankRewardLookup==nil then
self.rankRewardLookup={}
end
if self.rankRewardLookup[typo]==nil then
self.rankRewardLookup[typo]={}
end
if self.rankRewardLookup[typo][rank]==nil then
for i,v in ipairs(look)do
if rank>=v[1]and rank<=v[2]then
self.rankRewardLookup[typo][rank]=v[3]
return v[3]
end
end
end
return self.rankRewardLookup[typo][rank]
end



function UIXM_TYSC_RankWin:openXianMengPage()
self.rankList=xianmengModel:getXMRankList_TYSC()
local num=#self.rankList
local isShow=num>0
self.rankScrollView:setActive(isShow)
self.noItemTips:setActive(not isShow)
self.rankItem:setActive(isShow)
self.curRank=nil
self.curScore=nil
if isShow then
for i,data in ipairs(self.rankList)do
if xianmengModel:isMyXM(data.xmGuid)then
self.curRank=data.rank
self.curScore=data.jifen
break
end
end
self.rank1ScrollView:setActive(true)
self.rank2ScrollView:setActive(false)
self.scrollscript1:initData(self.rankList,102,num)
self:refreshRankItem()
else
self.noItemTips:setText('暂无排名信息')
end
end

function UIXM_TYSC_RankWin:closeXianMengPage()
self.rankScrollView:setActive(false)
self.rankItem:setActive(false)
end

function UIXM_TYSC_Rank1Scroller:OnSetCellView(dataIndex,cellIndex,cell)
self:RefreshCell(dataIndex,cellIndex,cell)
end

function UIXM_TYSC_Rank1Scroller:OnRefreshCellView(dataIndex,cellIndex,cell)
self:RefreshCell(dataIndex,cellIndex,cell)
end

function UIXM_TYSC_Rank1Scroller:RefreshCell(dataIndex,cellIndex,item)
if _this==nil then return end
local data=_this.rankList[dataIndex]

local rank=data.rank

local rankIcon
local rank_str=tostring(rank)
if rank<=3 then
rankIcon=FMT.fmt('icon_phbmingci_{0}',rank)
end
local showRankIcon=rankIcon~=nil
item:SetChildActive(6,showRankIcon)
item:SetChildActive(0,not showRankIcon)
if showRankIcon then
item:SetChildCSImageSprite(6,globalABLookup.rankList,rankIcon)
item:SetChildText(12,rank_str)
else
item:SetChildText(0,rank_str)
end

local hasXM=data.xmName~=''
item:SetChildActive(13,hasXM)
local tips2_str=''
if hasXM then

item:SetChildActive(7,true)
local image=xianmengModel.splitGuildIcon(data.xmIcon)
local abname=globalABLookup.xianmengicons

item:SetChildCSImageSprite(8,abname,cfgHelper.get3(cfg_guildiconconfig_get,xianmengIconType.eIcon,image.icon,'icon'))

item:SetChildCSImageSprite(7,abname,cfgHelper.get3(cfg_guildiconconfig_get,xianmengIconType.eBG,image.bg,'icon'))

item:SetChildCSImageSprite(9,abname,cfgHelper.get3(cfg_guildiconconfig_get,xianmengIconType.eKuang,image.kuang,'icon'))

local name_str=data.xmName
local serverName=loginModel:getServerName(data.xmServerId)
if serverName~=''then
name_str=FMT.fmt('<color=#b67b36>[{0}]</color>\n{1}',serverName,name_str)
end
item:SetChildText(1,name_str)

local icon=moneyModel.getIconNameEx(eMoneyType.mtTianYuanJiFen)
item:SetChildCSImageIcon(2,icon,true)
item:SetChildText(3,data.jifen)
else
tips2_str='该仙盟已解散'
end
item:SetChildText(14,tips2_str)

playerController:setHeadIcon(item,11,nil)

local rewardList=_this:getRankReward(1,data.rank,_this.curLevelCfg.xmRank)
local c=0
if rewardList~=nil then
c=#rewardList
end
local showReward=c>0
item:SetChildActive(4,showReward)
item:SetChildActive(5,not showReward)
if showReward then
item:SetChildLayoutGroupCreateItems(4,c)
local grids2=item:GetChildLayoutGroupGridList(4)
for i=1,c do
local rewardItem=grids2[i-1]
local itemid=rewardList[i][1]
local itemnum=rewardList[i][2]
local itemcount,showCountBG
if itemnum>1 then
itemcount=tostring(itemnum)
showCountBG=true
else
itemcount=''
showCountBG=false
end
local conf={itemid=itemid,itemcount=itemcount,showCountBG=showCountBG,showStage=true,showname=false}
local prop=itemsComponentHelper.getCommonFillDataSmall(conf)
rewardItem:SetChildPropData(0,prop)
rewardItem:SetBaseItemClickEvent(0,function(...)
if _this==nil then return end
_this:onClickItem(...)
end)
end
end
end





function UIXM_TYSC_RankWin:openMemberPage()
self.rankList=xianmengModel:getmemberRankList_TYSC()
local num=#self.rankList
local isShow=num>0
self.rankScrollView:setActive(isShow)
self.noItemTips:setActive(not isShow)
self.rankItem:setActive(isShow)
self.curRank=nil
self.curScore=nil
if isShow then
local myActorid=playerModel:getActorID()
for i,data in ipairs(self.rankList)do
if mathHelper.compareInt64(myActorid,data.playerId)then
self.curRank=data.rank
self.curScore=data.jifen
break
end
end
self.rank1ScrollView:setActive(false)
self.rank2ScrollView:setActive(true)
self.scrollscript2:initData(self.rankList,102,num)
self:refreshRankItem()
else
self.noItemTips:setText('暂无排名信息')
end
end

function UIXM_TYSC_RankWin:closeMemberPage()
self.rankScrollView:setActive(false)
self.rankItem:setActive(false)
end

function UIXM_TYSC_Rank2Scroller:OnSetCellView(dataIndex,cellIndex,cell)
self:RefreshCell(dataIndex,cellIndex,cell)
end

function UIXM_TYSC_Rank2Scroller:OnRefreshCellView(dataIndex,cellIndex,cell)
self:RefreshCell(dataIndex,cellIndex,cell)
end

function UIXM_TYSC_Rank2Scroller:RefreshCell(dataIndex,cellIndex,item)
if _this==nil then return end
local data=_this.rankList[dataIndex]


local rank=data.rank
local rankIcon
local rank_str
local isNotRank=rank==nil or rank==0
if isNotRank then
rank_str='暂无名次'
else
rank_str=tostring(rank)
end
if not isNotRank and rank<=3 then
rankIcon=FMT.fmt('icon_phbmingci_{0}',rank)
end
local showRankIcon=rankIcon~=nil
item:SetChildActive(6,showRankIcon)
item:SetChildActive(0,not showRankIcon)
if showRankIcon then
item:SetChildCSImageSprite(6,globalABLookup.rankList,rankIcon)
item:SetChildText(12,rank_str)
else
item:SetChildText(0,rank_str)
end

local hasMember=data.playerName~=''
item:SetChildActive(13,hasMember)
local tips2_str=''
local headParams
if hasMember then

item:SetChildActive(7,false)

headParams={iconInfo=data.iconInfo,scale=0.8}

item:SetChildText(1,data.playerName)

local icon=moneyModel.getIconNameEx(eMoneyType.mtTianYuanJiFen)
item:SetChildCSImageIcon(2,icon,true)
item:SetChildText(3,data.jifen)
else
tips2_str='该成员已离开仙盟'
end
item:SetChildText(14,tips2_str)

playerController:setHeadIcon(item,11,headParams)

local rewardList=_this:getRankReward(2,data.rank,_this.curLevelCfg.memberRank)
local c=0
if rewardList~=nil then
c=#rewardList
end
local showReward=c>0
item:SetChildActive(4,showReward)
item:SetChildActive(5,not showReward)
if showReward then
item:SetChildLayoutGroupCreateItems(4,c)
local grids2=item:GetChildLayoutGroupGridList(4)
for i=1,c do
local rewardItem=grids2[i-1]
local itemid=rewardList[i][1]
local itemnum=rewardList[i][2]
local itemcount,showCountBG
if itemnum>1 then
itemcount=tostring(itemnum)
showCountBG=true
else
itemcount=''
showCountBG=false
end
local conf={itemid=itemid,itemcount=itemcount,showCountBG=showCountBG,showStage=true,showname=false}
local prop=itemsComponentHelper.getCommonFillDataSmall(conf)
rewardItem:SetChildPropData(0,prop)
rewardItem:SetBaseItemClickEvent(0,function(...)
if _this==nil then return end
_this:onClickItem(...)
end)
end
end
end



function UIXM_TYSC_RankWin:refreshTips()
local tips_str
if self.curPage==3 then
tips_str=FMT.fmt('个人天渊积分：{0}',xianmengModel:getMyScore_TYSC())
else
tips_str='活动结束后奖励通过邮件发送'
end
self.tipsTxt:setText(tips_str)
end

function UIXM_TYSC_RankWin:refreshRankItem()
local item=self.rankItem:getChildWidgetBase()
local rewardList
local name_str
local score=self.curScore
local image
local headid=false
if self.curPage==1 then
if self.curRank then
rewardList=self:getRankReward(1,self.curRank,self.curLevelCfg.xmRank)
end
name_str=xianmengModel:getXMName()
if score==nil then
if limitActivitiesModel:checkActDoing(LIMIT_ACT_TYPE.eTianYuanShouChao)then
score=xianmengModel:getNowScore_TYSC()
else
score=xianmengModel:getPreScore_TYSC()
end
end
image=xianmengModel:getGuildImage()
else
if self.curRank then
rewardList=self:getRankReward(2,self.curRank,self.curLevelCfg.memberRank)
end
name_str=playerModel:getActorName()
if score==nil then
score=xianmengModel:getMyScore_TYSC()
end
headid=true
end


local rank=self.curRank
local rankIcon
local rank_str
local isNotRank=rank==nil or rank==0
if isNotRank then
rank_str='暂无名次'
else
rank_str=tostring(rank)
end
if not isNotRank and rank<=3 then
rankIcon=FMT.fmt('icon_phbmingci_{0}',rank)
end
local showRankIcon=rankIcon~=nil
item:SetChildActive(6,showRankIcon)
item:SetChildActive(0,not showRankIcon)
if showRankIcon then
item:SetChildCSImageSprite(6,globalABLookup.rankList,rankIcon)
item:SetChildText(12,rank_str)
else
item:SetChildText(0,rank_str)
end


item:SetChildActive(7,image~=nil)
if image~=nil then
local abname=globalABLookup.xianmengicons

item:SetChildCSImageSprite(8,abname,cfgHelper.get3(cfg_guildiconconfig_get,xianmengIconType.eIcon,image.icon,'icon'))

item:SetChildCSImageSprite(7,abname,cfgHelper.get3(cfg_guildiconconfig_get,xianmengIconType.eBG,image.bg,'icon'))

item:SetChildCSImageSprite(9,abname,cfgHelper.get3(cfg_guildiconconfig_get,xianmengIconType.eKuang,image.kuang,'icon'))
end

local args
if headid then
args={iconInfo=nil,scale=0.8}
end
playerController:setHeadIcon(item,11,args)

item:SetChildText(1,name_str)

local icon=moneyModel.getIconNameEx(eMoneyType.mtTianYuanJiFen)
item:SetChildCSImageIcon(2,icon,true)
item:SetChildText(3,score)

local c=0
if rewardList~=nil then
c=#rewardList
end
local showReward=c>0
item:SetChildActive(4,showReward)
item:SetChildActive(5,not showReward)
if showReward then
item:SetChildLayoutGroupCreateItems(4,c)
local grids2=item:GetChildLayoutGroupGridList(4)
for i=1,c do
local rewardItem=grids2[i-1]
local itemid=rewardList[i][1]
local itemnum=rewardList[i][2]
local itemcount,showCountBG
if itemnum>1 then
itemcount=tostring(itemnum)
showCountBG=true
else
itemcount=''
showCountBG=false
end
local conf={itemid=itemid,itemcount=itemcount,showCountBG=showCountBG,showStage=true,showname=false}
local prop=itemsComponentHelper.getCommonFillDataSmall(conf)
rewardItem:SetChildPropData(0,prop)
rewardItem:SetBaseItemClickEvent(0,function(...)
if _this==nil then return end
_this:onClickItem(...)
end)
end
end
end

function UIXM_TYSC_RankWin:getRewardPageSortList()
local list={}
if self.curLevel>0 then
local jifenReward=self.curLevelCfg.jifenReward
local score=xianmengModel:getMyScore_TYSC()
local r_score=xianmengModel:getMyRewardScore_TYSC()
for i,v in ipairs(jifenReward)do
local fix=score>=v[1]
local flag=v[1]<=r_score
local state=flag==true and 0 or 1
local weight=state*100000+(100000-v[1])
table.insert(list,{v,fix,flag,weight})
end
table.sort(list,function(a,b)
return a[4]>b[4]
end)
end
return list
end

function UIXM_TYSC_RankWin:openRewardPage()
local rankList=self:getRewardPageSortList()
local num=#rankList
local isShow=num>0
self.rewardScrollView:setActive(isShow)
self.noItemTips:setActive(not isShow)
if isShow then
local func=function(i)
if _this==nil then return end
local item=_this.rewardGridPanel:getChildLayoutGroupGridItem(i-1)
local data=rankList[i]
local cfg=data[1]
local fix=data[2]
local flag=data[3]

local icon=moneyModel.getIconNameEx(eMoneyType.mtTianYuanJiFen)
item:SetChildCSImageIcon(5,icon,true)

item:SetChildText(0,FMT.fmt('{0}天渊积分',cfg[1]))

local rewardList=cfg[2]
local c=0
if rewardList~=nil then
c=#rewardList
end
local showReward=c>0
item:SetChildActive(1,showReward)
if showReward then
item:SetChildLayoutGroupCreateItems(1,c)
local grids2=item:GetChildLayoutGroupGridList(1)
for i=1,c do
local rewardItem=grids2[i-1]
local itemid=rewardList[i][1]
local itemnum=rewardList[i][2]
local itemcount,showCountBG
if itemnum>1 then
itemcount=tostring(itemnum)
showCountBG=true
else
itemcount=''
showCountBG=false
end
local conf={itemid=itemid,itemcount=itemcount,showCountBG=showCountBG,showStage=true,showname=false}
local prop=itemsComponentHelper.getCommonFillDataSmall(conf)
rewardItem:SetChildPropData(0,prop)
rewardItem:SetBaseItemClickEvent(0,function(...)
if _this==nil then return end
_this:onClickItem(...)
end)
end
end

local showBtn=fix and not flag
item:SetChildActive(2,showBtn)
item:SetChildButtonClick(2,function()
if _this==nil then return end
_this:onRewardBtn()
end)

item:SetChildActive(3,flag)

item:SetChildActive(4,not fix and not flag)
end
self.rewardGridPanel:setChildLayoutGroupCreateItems(num,func)
else
self.noItemTips:setText('暂无仙盟积分奖励')
end
end

function UIXM_TYSC_RankWin:closeRewardPage()
self.rewardScrollView:setActive(false)
end

function UIXM_TYSC_RankWin:onRewardBtn()
xianmengController:send_248_16()
end

function UIXM_TYSC_RankWin:onClickItem(itemId,index,guid,attach)
tipsManager.showTips({itemid=itemId,itemguid=nil,move=TIPS_MOVE_POS.eLeft})
end

function UIXM_TYSC_RankWin:onCloseBtn()
self:closeSelf()
end



function UIXM_TYSC_RankWin:recv_reward()
if self.curPage==3 then
self:openRewardPage()
self:refreshTips()
self:refreshMenuItemReddot(nil,3)
end
end

