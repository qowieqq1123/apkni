...
def_class("UILimitTimeGiftWin",UIWindowBase)
...
function UILimitTimeGiftWin:bindComponents()

self.Content=UIObject.get(self,0)
self.jump=UIButton.get(self,1)
self.jumpBgModel=UIObject.get(self,2)
self.jumpMaoModel=UIObject.get(self,3)
self.jumpPanel=UIObject.get(self,4)
self.leftImage=UIImage.get(self,5)
self.mao=UIObject.get(self,6)
self.moneyBtn=UIButton.get(self,7)
self.moneyBtn_xianyu=UIButton.get(self,8)
self.moneyRoot=UIObject.get(self,9)
self.moneyRoot_xianyu=UIObject.get(self,10)
self.name=UIText.get(self,11)
self.packScrollerView=UIObject.get(self,12)
self.time=UIText.get(self,13)

self.jump:setButtonClick(function()self:onJump()end)
...
function UILimitTimeGiftWin:unbindComponents()
local _UIObject_release=UIObject.release
_UIObject_release(self.Content);self.Content=nil;
_UIObject_release(self.jump);self.jump=nil;
_UIObject_release(self.jumpBgModel);self.jumpBgModel=nil;
_UIObject_release(self.jumpMaoModel);self.jumpMaoModel=nil;
_UIObject_release(self.jumpPanel);self.jumpPanel=nil;
...
function UILimitTimeGiftWin:onLoaded(...)
self:bindComponents()
...
function UILimitTimeGiftWin:__delete()
...
function UILimitTimeGiftWin:onShow(argtable,afterOnloaded)
self.activityArgs=argtable
self.actid=argtable.act_id
self.subType=argtable.sub_act_type
self.subid=argtable.sub_act_id
local config_=activitiesModel:getSubActivityConfig(self.subType,self.subid)
local config=table.deepCopy(config_)
self.config=config


local rewardsSort=config.rewardsSort
if rewardsSort then
local rewards=config.rewards
if#rewardsSort~=#rewards then
loggerUtil.logErrFMT('限时礼包{0}初始排序配置长度与rewards不匹配',self.subid)
else
local temp={}
local lookup={}
for i,v in ipairs(rewardsSort)do
if lookup[v]then
loggerUtil.logErrFMT('限时礼包{0}初始排序内不能有相同的值',self.subid)
end
lookup[v]=true
temp[i]=rewards[v]
temp[i].reallyIdx=v
...
self.jumpPanel:setActive(config.jump~=nil)
if config.jump then
self.winlua:SetChildCSImageSprite(self.jump:getID(),"ui/windows/activities/sub_xianshilibao/xianshilibao_atlas_pak.ab",config.jump[1])
self.jumpBgModel:setChildUIModelShowTarget(5714,1,{},eAnimationID.stand)
self.jumpMaoModel:setChildUIModelShowTarget(5715,1,{},eAnimationID.stand)
else
...
function UILimitTimeGiftWin:onRefresh()
...
function UILimitTimeGiftWin:refreshGiftItemByIndex(listIndex,sortIndex)
...
function UILimitTimeGiftWin:getItemList(itemList)

if type(itemList[1][1])=="number"then


local zmLevel=activitiesHandle_xianshilibao:getLiBaoZmLevel(self.actid,self.subid)
...
function UILimitTimeGiftWin:refreshGiftItem(cmp,config,listIndex,sortIndex)
...
local itemList=self:getItemList(config[1])or{}

local isSelectFin=true

local giftData=activitiesHandle_xianshilibao:getLiBaoData(self.actid,self.subid,listIndex)
...
idx=activitiesHandle_xianshilibao:getLiBaoSelectData(self.actid,self.subid,listIndex,i)
end
...
if not self.isOver then
selectIndex=activitiesHandle_xianshilibao:getLiBaoSelectData(self.actid,self.subid,listIndex,i)
end
...
function UILimitTimeGiftWin:onBuyClick(listIndex,buyCostType,buyCostVal,itemList,buyCount,buyLimit)
...
local selectIndex=activitiesHandle_xianshilibao:getLiBaoSelectData(self.actid,self.subid,listIndex,i)
if selectIndex then
...
local info={listIndex,1,is_assistant}
...
local is_assistant=0
local info={listIndex,num,is_assistant}
for i,v in ipairs(indexList)do
table.insert(info,v)
end
local jsonStr=jsonHelper.encode(info)
...
function UILimitTimeGiftWin:showReward()
...
function UILimitTimeGiftWin:onAddClick(listIndex,sortIndex,itemList)
UIManager:showWindow("UILimitTimeGiftSelectWin",{actid=self.actid,subid=self.subid,perentWin=self,rewardIndex=listIndex,sortIndex=sortIndex,itemList=itemList})
end

function UILimitTimeGiftWin:sortReward(rewards,names)
local showList={}

local pfOpen_yiyuanlibao=pfwindowslController:checkPFWinState_ByWinType_DfShow(pfwindowslController.winType.yiyuanlibao)
local lv_cond=self.config.lv_condition or defaultT
local zongMenLv=activitiesHandle_xianshilibao:getLiBaoZmLevel(self.actid,self.subid)
for i,v in ipairs(rewards)do
local sortId=i
local item=v
local listIndex=v.reallyIdx or i
item.listIndex=listIndex
item.sortId=sortId
if names then
item.name=names[listIndex]or''
end
local giftData=activitiesHandle_xianshilibao:getLiBaoData(self.actid,self.subid,listIndex)
if giftData then
local buyCount=giftData.buyCount or 0
...
buyCount=0
activitiesHandle_xianshilibao:setLiBaoData(self.actid,self.subid,i,{buyCount=0,buyTime=nil})
end
end
local isSold=buyCount>=buyLimit
if isSold then
...
if lv_cond[i]and(zongMenLv>=lv_cond[i][1]and zongMenLv<=lv_cond[i][2])then
isCanShow=false
end
...
function UILimitTimeGiftWin:onHide()
self:clearAllFMTweener()
end

function UILimitTimeGiftWin:onMoneyChange(moneyType,lastVal,val)
if self.isShowMoney and self.moneyType==moneyType and UIManager:isActive("UILimitTimeGiftWin")then
self:freshMoneyValue(self.moneyType,lastVal)
end

if self.isShowMoney and moneyType==eMoneyType.mtXianYu and UIManager:isActive("UILimitTimeGiftWin")then
self:freshMoneyValue_xianyu(lastVal)
end
end

function UILimitTimeGiftWin:onItemChange(changeType,itemguid,itemid,lastcount,itemcount)
if self.isShowMoney and self.moneyType==itemid and UIManager:isActive("UILimitTimeGiftWin")then
self:freshMoneyValue(self.moneyType,lastcount)
end
if self.isShowMoney and itemid==eMoneyType.mtXianYu and UIManager:isActive("UILimitTimeGiftWin")then
self:freshMoneyValue_xianyu(lastcount)
end
end

function UILimitTimeGiftWin:freshMoneyValue(moneyType,lastVal)
...
function UILimitTimeGiftWin:freshMoneyValue_xianyu(lastVal)
...
function UILimitTimeGiftWin:clearFMTweenerByIndex(index)
...
function UILimitTimeGiftWin:clearAllFMTweener()
...
function UILimitTimeGiftWin:freshMoney()
...
function UILimitTimeGiftWin:onMoneyBtn()
...
function UILimitTimeGiftWin:onMoneyBtn_xianyu()
UIFullRechargeController:showRechargeWindow()
end

function UILimitTimeGiftWin:onJump()
jumpManager:jump(self.config.jump[2])
end