...
def_class("UIExchargeShopTipsWin",UIWindowBase)
...
function UIExchargeShopTipsWin:bindComponents()
...
function UIExchargeShopTipsWin:unbindComponents()
...
function UIExchargeShopTipsWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UIExchargeShopTipsWin:__delete()
_this=nil
...
function UIExchargeShopTipsWin:onHide()

end

function UIExchargeShopTipsWin.onSubActivityStateChange(actID,subType,subid,state)
...
function UIExchargeShopTipsWin:onShow(argtable,afterOnloaded)
...
self.auto_excharge=self.actcfg.auto[1]
self.actID=actID
...
function UIExchargeShopTipsWin:refreshActTime()
local sub_actInfo=activitiesModel:getSubActInfo(self.actID,self.subType,self.subid)
local time=sub_actInfo:getEndLeftTime()
...
function UIExchargeShopTipsWin:updateView()
...
function UIExchargeShopTipsWin:onClickItem(itemId,index,guid,attach)
tipsManager.showTips({itemid=itemId,itemguid=nil,move=TIPS_MOVE_POS.eLeft})
end

function UIExchargeShopTipsWin:onGotoBtn()
activitiesController:jump(self.actID,self.subType,self.subid)
end

function UIExchargeShopTipsWin:onCancelBtn()
local actID=self.actID
local subType=self.subType
local subid=self.subid
local extraParams=self.extraParams
self:closeSelf()
activitiesModel:extraCheck(actID,nil,nil,extraParams,activityExtracheckType.eExchangeShop)
end


function UIExchargeShopTipsWin:playSpeak(delay)
self:clearTalk()
...
function UIExchargeShopTipsWin:doSpeaking()
...
function UIExchargeShopTipsWin:doTalkAnim()
...
function UIExchargeShopTipsWin:finishSpeak()
self.speakObj:setChildCanvasGroupAlpha(0)

self:playSpeak(5)
end

function UIExchargeShopTipsWin:clearTalk()
