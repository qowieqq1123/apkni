...
def_class("UIXJBuildingInfoWin",UIWindowBase)
...
function UIXJBuildingInfoWin:bindComponents()
...
self.buildingName=UIText.get(self,4)
self.condBuilding=UIText.get(self,5)
self.condBulidingImg=UIImage.get(self,6)
self.condition_1=UIObject.get(self,7)
self.condition_2=UIObject.get(self,8)
self.condition_3=UIObject.get(self,9)
self.conditionPanel=UIObject.get(self,10)
self.conditionRoot=UIObject.get(self,11)
self.costIcon=UIObject.get(self,12)
self.costValue=UIText.get(self,13)
self.desc=UIText.get(self,14)
self.detailButton=UIButton.get(self,15)
self.detailOpen=UIObject.get(self,16)
self.helpBtn=UIButton.get(self,17)
self.iconAds=UIImage.get(self,18)
self.level=UIText.get(self,19)
self.lvUpBtn=UIButton.get(self,20)
self.materials=UIObject.get(self,21)
self.materialsItem_1=UIBaseItem.get(self,22)
self.materialsItem_2=UIBaseItem.get(self,23)
self.materialsItem_3=UIBaseItem.get(self,24)
self.materialsItem_4=UIBaseItem.get(self,25)
self.materialsItem_5=UIBaseItem.get(self,26)
self.maxLevelTip=UIObject.get(self,27)
self.model=UIObject.get(self,28)
self.nextLevel=UIText.get(self,29)
self.payAds=UIText.get(self,30)
self.ruleList=UIObject.get(self,31)
self.ruleMask=UIButton.get(self,32)
self.rulePart=UIObject.get(self,33)
self.sliderUpgrade=UIObject.get(self,34)
self.small_model=UIObject.get(self,35)
self.speedUpBtnText=UIText.get(self,36)
self.suCost=UIObject.get(self,37)
self.txtUpgradeBar=UIText.get(self,38)
self.upgradePanel=UIObject.get(self,39)
self.upTime=UIText.get(self,40)
...
self.detailButton:setButtonClick(function()self:onDetailButton()end)
...
function UIXJBuildingInfoWin:unbindComponents()
...
_UIObject_release(self.condBuilding);self.condBuilding=nil;
_UIObject_release(self.condBulidingImg);self.condBulidingImg=nil;
...
_UIObject_release(self.detailButton);self.detailButton=nil;
_UIObject_release(self.detailOpen);self.detailOpen=nil;
...
_UIObject_release(self.maxLevelTip);self.maxLevelTip=nil;
...
_UIObject_release(self.upTime);self.upTime=nil;
self.condition=nil;
...
function UIXJBuildingInfoWin:onLoaded(...)
self:bindComponents()
_this=self
self.checkList={}
...
function UIXJBuildingInfoWin:__delete()
...
function UIXJBuildingInfoWin:onShow(argtable,afterOnloaded)
...
self.curLvCfg=cfgHelper.get2(cfg_monijybuilduplvlconfig_get,self.build_id,self.bdData.level)
local mdata_cur=isometricMapSystem:getModelByStatus(self.build_id,self.curLvCfg.level,0,nil,nil,nil,self.bdData.un_build_id,nil,true,true)
...
if self.curLvCfg.build_desc then
self.desc:setText(FMT.fmt("<color=#ffffff00></color>{0}",self.curLvCfg.build_desc))
end

self:refreshLevel()
self:refreshDetailButton()
end

function UIXJBuildingInfoWin:refreshDetailButton()
if self.build_id==SLG_SYSTEM_TYPE.eYanDaoTai then
self.detailButton:setActive(false)
else
self.detailButton:setActive(true)
end
end

function UIXJBuildingInfoWin:refreshLevel()
local level=self.bdData.level
self.buildingName:setText(FMT.fmt("{0}\n{1}\n级",self.config.name,level))
self.nextLvCfg=cfgHelper.get2(cfg_monijybuilduplvlconfig_get,self.build_id,level+1)

local isMax=not self.nextLvCfg

self.level:setText(level)

self.nextLevel:setActive(not isMax)

self.nextLevel:setText(level+1)
...
local curAttr=attrList[level]or{}
local nextAttr=attrList[level+1]
for i,v in ipairs(curAttr)do
local next=nextAttr and nextAttr[i]or nil





local isOnlyDetailShow=v.isOnlyDetailShow or false
if not isOnlyDetailShow then
table.insert(list,{v,next})
end

end

self.attrPanel:setChildLayoutGroupCreateItems(#list,nil)
local childGrids=self.attrPanel:getChildLayoutGroupGridList()
for i=1,childGrids.Count do
local childItem=childGrids[i-1]
local attr=list[i][1]
childItem:SetChildText(0,FMT.fmt("{0}：",attr.name))
...
local nextAttr=list[i][2]

if nextAttr then
local nextAttrVal=nextAttr.val
if nextAttr.addVal and nextAttr.addVal>0 then
nextAttrVal=nextAttrVal+nextAttr.addVal
end
if nextAttrVal~=attrVal then
childItem:SetChildActive(-1,true)
childItem:SetChildActive(2,true)
childItem:SetChildText(2,xianjieModel:getBuildingAttrVal(nextAttrVal,nextAttr.flag))
else
childItem:SetChildActive(-1,false)
end
else
childItem:SetChildActive(-1,true)
childItem:SetChildActive(2,false)
end
end
...
function UIXJBuildingInfoWin:refreshMoneyPanel()
...
function UIXJBuildingInfoWin:refreshCostPanel()
if self.nextLvCfg.uplevel_times>0 then
self.upTime:setText(FMT.fmt('耗时：<color={0}>{1}</color>','#171311',timeHelper.format_time_stamp4(self.nextLvCfg.uplevel_times)))
else
self.upTime:setText('立即完成')
end
...
function UIXJBuildingInfoWin:onClickMaterialItem(itemId,index,guid,attach)
...
function UIXJBuildingInfoWin:getLevelUpCND(cfgs)
local list={}
...
function UIXJBuildingInfoWin:refreshConditionPanel(config)
...
function UIXJBuildingInfoWin:onGoToButton(ftype,arg1,arg2)
if xianjieController:checkInMoGongZhengDuo()then
toastMessage('当前处于魔尊宝库中，无法前往')
return
end
...
function UIXJBuildingInfoWin.on_building_event(etype,sfId,bdId,arg1,arg2)
...
function UIXJBuildingInfoWin:onHide()

end

function UIXJBuildingInfoWin:isCanLevelUp()
...
function UIXJBuildingInfoWin:refreshUpgradePanel()
...
function UIXJBuildingInfoWin:startLevelUpTimer()
local tick=function()
local cdd=buildingCDControl:getCDData(buildingCDType.build,self.bdData.un_build_id)
local dtime=cdd.cd
self.lastTime=dtime
...
function UIXJBuildingInfoWin:stopLevelUpTimer()
self:removeCDUpdateFunc('UPCD')
end

function UIXJBuildingInfoWin:onDetailWinClose()
self.detailOpen:setActive(false)
end


function UIXJBuildingInfoWin:setSPNeedText(need,mtype)
local needText
...
function UIXJBuildingInfoWin:updateSPNeedText()
...
function UIXJBuildingInfoWin:refreshSpeedPanel(typo)
...
function UIXJBuildingInfoWin:getSortSpeedupItemList()
local sortItemList={}
...
function UIXJBuildingInfoWin:setSpeedupItemExpireTimer(expireTime)
...
function UIXJBuildingInfoWin:clearSpeedupItemExpireTimer()
...
function UIXJBuildingInfoWin:onBtnAdsSpeedup()
...
function UIXJBuildingInfoWin:showDialog()
local data=self.baseCfg[1]
...
function UIXJBuildingInfoWin:moneySpeedUp()
local data=self.baseCfg[1]
local need,count=self:countSpeedUpNeed(data)
local moneyId=data[1]
local have=moneyModel.getMoney(moneyId)
...
if itemsConfig.isMoney(moneyId)then
moneySystem:useMoney(moneyId,need,cb,WARNING_TYPE.eWarning)
else
if have>=need then
cb()
else
gainControl:showGainWin(moneyId,need)
end
end
end

function UIXJBuildingInfoWin:onAdItemClick()
if self.speedup_item_id then
tipsManager.showTips({formType=TIPS_FORM_TYPE.eWatchItem,itemid=self.speedup_item_id})
end
end

function UIXJBuildingInfoWin:countSpeedUpNeed(needData)
...
function UIXJBuildingInfoWin:onBtnFinishUpgrade()
...
function UIXJBuildingInfoWin:onDetailButton()
self.detailOpen:setActive(true)
self:showWindow("UIXJBuildingDetailWin",{bdData=self.bdData})
end



function UIXJBuildingInfoWin:onLvUpBtn()
...
function UIXJBuildingInfoWin:handleLevelUp()
if self.bdData.build_id==SLG_SYSTEM_TYPE.eYanDaoTai then
local studyList=yandaotaiModel:getStudyList()
if studyList and studyList.id and studyList.starTime then
UIManager.error('正在研究中，暂不可升级')
return
end
end
...
function UIXJBuildingInfoWin:onSpeedBtn()

end

function UIXJBuildingInfoWin:onClickClose()
self:closeSelf()
end

function UIXJBuildingInfoWin:onHelpBtn()
if self.attrConfig and self.attrConfig.ruleName then
self:refreshRulePart(self.attrConfig.ruleName)
self.rulePart:setActive(true)
self.ruleMask:setActive(true)
end
end

function UIXJBuildingInfoWin:onRuleMask()
self.rulePart:setActive(false)
self.ruleMask:setActive(false)
end


function UIXJBuildingInfoWin:refreshRulePart(ruleName)
local desclist={}
local name=ruleName
for i=1,10 do
