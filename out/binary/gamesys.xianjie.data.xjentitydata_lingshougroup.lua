...
local xjEntityData_lingshouGroup={}

function xjEntityData_lingshouGroup:onInit()
self.gridX=self.x
self.gridZ=self.y
self.x=nil
self.y=nil
...
self.defaultSpeed=cfgHelper.get4(cfg_fairylandbaseconfig_get,1,"march",xjServerMarchType.eLingShouGroupAttak,1)

self.isMySelf=mathHelper.compareInt64(playerModel:getActorID(),self.ownerActorId)
end

function xjEntityData_lingshouGroup:refreshData(data)
...
function xjEntityData_lingshouGroup:createEntity(needRefreshAOI)
...
self.ent_key=xianjieController:addEntity(XJ_ENTITY_TYPE.eLingShouGroup,{self.infoguid},needRefreshAOI)
end
end

function xjEntityData_lingshouGroup:onDelete()

end

function xjEntityData_lingshouGroup:getCfg()
...
function xjEntityData_lingshouGroup:getName()
...
function xjEntityData_lingshouGroup:getModelData()
local cfg=self:getCfg()
...
return xjEntityData_lingshouGroup
