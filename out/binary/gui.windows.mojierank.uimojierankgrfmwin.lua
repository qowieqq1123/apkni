...
def_class("UIMoJieRankGRFMWin",UIWindowBase)
...
function UIMoJieRankGRFMWin:bindComponents()
...
function UIMoJieRankGRFMWin:unbindComponents()
...
function UIMoJieRankGRFMWin:onLoaded(...)
self:bindComponents()
_this=self
end


function UIMoJieRankGRFMWin:__delete()
self:unbindComponents()
_this=nil
end


function UIMoJieRankGRFMWin:onClickRewardItem(itemId,index,guid,attach)
if itemId==-1 then return end
tipsManager.showTips({itemid=itemId,itemguid=guid,move=TIPS_MOVE_POS.eRight})
end


function UIMoJieRankGRFMWin:severmkrankfresh(rankType)
if _this then

if rankType==eRankListType.eZhuMoRank then
...
function UIMoJieRankGRFMWin:onShow(argtable,afterOnloaded)
...
function UIMoJieRankGRFMWin:onHide()
self.center:setActive(false)
self.norank:setActive(false)
end


function UIMoJieRankGRFMWin:getTypeReward(flag)
local cfgrank=self.mojiecfg.rank
return cfgrank[flag]
end

function UIMoJieRankGRFMWin:getRewardByRank(cfg,rank)
if cfg then
...
function UIMoJieRankGRFMWin:refreshRankList()
...
function UIMoJieRankGRFMWin:onFreshAction(i,grid)
...
function UIMoJieRankGRFMWin:onStartAction()

end

function UIMoJieRankGRFMWin:frsehSelfRank()
...
local val=mathHelper.formatNumber4(tonumber(tostring(grgxnum)),2)
...
local num=tonumber(tostring(grgxnum))
