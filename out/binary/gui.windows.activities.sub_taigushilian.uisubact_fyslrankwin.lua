...
def_class("UISubAct_fyslRankWin",UIWindowBase)
...
function UISubAct_fyslRankWin:bindComponents()
...
function UISubAct_fyslRankWin:unbindComponents()
...
function UISubAct_fyslRankWin:onLoaded(...)
_this=self
self:bindComponents()
self.topimg={0,1,2,3}
...
function UISubAct_fyslRankWin:__delete()
self:unbindComponents()
_this=nil
end


function UISubAct_fyslRankWin:onHide()
...
function UISubAct_fyslRankWin:onShow(argtable,afterOnloaded)
local page=2
if argtable then
self.actID=argtable[1]
self.subType=argtable[2]
self.subid=argtable[3]
page=argtable[4]or 2
...
self.showList={}
self.bossState=activitiesHandle_fuyaoBoss:checkbossOpen(_this.actID,_this.subType,_this.subid)
local boss_config=cfg_fuyaoshilianconfig_get(_this.subid).boss
for k,v in ipairs(boss_config)do
if self.bossState[k]==rankstate.finish or self.bossState[k]==rankstate.open then
local mosterGroupid=v[1][1][1]



local slnamecfg=cfg_fuyaoshilianconfig_get(_this.subid).slname
...
local reddot=activitiesHandle_fuyaoBoss:checkreddotBossAll(_this.actID,_this.subType,_this.subid)
_this.jlreddot:setActive(reddot)
UISubAct_fyslRankWin:onClickshowPanel(page)
UISubAct_fyslRankWin:refreshResidueTime()
end


function UISubAct_fyslRankWin:initShouLingList()
...
local flag,reddotlist=activitiesHandle_fuyaoBoss:checkreddotSingleBoss(_this.actID,_this.subType,_this.subid,bossid)
...
function UISubAct_fyslRankWin:refreshShouLingList()
local grids=_this.taskScroller:getChildScrollViewItemWidgets()
local count=grids.Count
for i=1,count do
local item=grids[i-1]
...
function UISubAct_fyslRankWin:refreshMenuItemSelect(item,idx,flag)
if item==nil then
...
function UISubAct_fyslRankWin:refreshMenuItemReddot(item,idx)
if item==nil then
item=_this.menuGridPanel:getChildLayoutGroupGridItem(idx-1)
end
...
function UISubAct_fyslRankWin:onMenuItemClick(page)
...
function UISubAct_fyslRankWin:refreshMenuPage(page,flag)
...
function UISubAct_fyslRankWin:getRankReward(typo,rank,look)
...
function UISubAct_fyslRankWin:getJiangReward(typo,rank,look)
...
function UISubAct_fyslRankWin:openMemberPage()
...
local cfg_playerank=cfg_fuyaoshilianconfig_get(_this.subid).playerank
...
item:SetChildText(10,data.sectname)
...
function UISubAct_fyslRankWin:closeMemberPage()
self.rankScrollView:setActive(false)
self.rankItem:setActive(false)
end


function UISubAct_fyslRankWin:refreshRankItem()
...
item:SetChildText(1,str)
item:SetChildText(10,UISettingModel:getZMName()or"")
...
function UISubAct_fyslRankWin:getRewardPageSortList()
local list={}
local boss_config_single=_this.showList[_this.shoulingIdx][1]
local bossid=boss_config_single[7]
local jifenReward={}
...
local score=activitiesHandle_fuyaoBoss:getDamageRole(_this.actID,_this.subType,_this.subid,bossid)
...
local weight=state*1000000000+(100000000-v[1])
...
function UISubAct_fyslRankWin:openRewardPage()
...
local boss_config_single=_this.showList[_this.shoulingIdx][1]
local bossid=boss_config_single[7]
alldamage=activitiesHandle_fuyaoBoss:getDamageRole(_this.actID,_this.subType,_this.subid,bossid)
...
local allbossState=activitiesHandle_fuyaoBoss:checkbossOpen(_this.actID,_this.subType,_this.subid)
...
function UISubAct_fyslRankWin:closeRewardPage()
self.rewardScrollView:setActive(false)
end


function UISubAct_fyslRankWin:onRewardBtn(cfg,bossid)
...
function UISubAct_fyslRankWin:onClickItem(itemId,index,guid,attach)
if itemId>0 then
tipsManager.showTips({itemid=itemId,itemguid=nil,move=TIPS_MOVE_POS.eLeft})
end
end
function UISubAct_fyslRankWin:onCloseBtn()
self:closeSelf()
end

function UISubAct_fyslRankWin:onJianglibtn()
UISubAct_fyslRankWin:onClickshowPanel(2)
end
function UISubAct_fyslRankWin:onPaihangbangbtn()
UISubAct_fyslRankWin:onClickshowPanel(1)
end


function UISubAct_fyslRankWin:onClickshowPanel(index)
...
function UISubAct_fyslRankWin:onShouLingClickItem(shoulingidx,page,bossState,bossdata)
...
local str=FMT.fmt("{0}后开启",UISubAct_fyslRankWin.format_time_stampbyboss((bossstart-nowstamp)))
UIManager.info(str)
end
return
end
...
function UISubAct_fyslRankWin:onPjbtn()
local boss_config_single=_this.showList[_this.shoulingIdx][1]
local bossid=boss_config_single[7]
UIManager:showWindow('UISubAct_fyslPinJitwoWin',{_this.actID,_this.subType,_this.subid,bossid})
end


function UISubAct_fyslRankWin:onClickhead(actorId)
...
function UISubAct_fyslRankWin.format_time_stampbyboss(inteval)
local SS=inteval%60
...
function UISubAct_fyslRankWin:refreshResidueTime()
...
local str=FMT.fmt("{0}后结算",UISubAct_fyslRankWin.format_time_stampbyboss((bossend-nowstamp)))
...
function UISubAct_fyslRankWin:refreshTips()
...
function UISubAct_fyslRankWin:recv_reward(actID,subType,subID)
...
function UISubAct_fyslRankWin:recv_paihangbang(actID,subType,subID)
