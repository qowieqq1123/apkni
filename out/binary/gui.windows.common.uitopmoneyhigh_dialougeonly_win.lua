...
def_class("UITopMoneyHigh_dialougeOnly_Win",UIWindowBase)
...
function UITopMoneyHigh_dialougeOnly_Win:bindComponents()
...
function UITopMoneyHigh_dialougeOnly_Win:unbindComponents()
...
function UITopMoneyHigh_dialougeOnly_Win:onLoaded(...)
self:bindComponents()
self.money={}
...
function UITopMoneyHigh_dialougeOnly_Win:__delete()
...
function UITopMoneyHigh_dialougeOnly_Win:onHide()
self:clearFlyIcon()
end
...
function UITopMoneyHigh_dialougeOnly_Win:onShow(argtable,afterOnloaded)
...
self.money=argtable.moneytypes
...
function UITopMoneyHigh_dialougeOnly_Win:clearFlyIcon()
...
function UITopMoneyHigh_dialougeOnly_Win:getMoneyList()
return self.money
end

function UITopMoneyHigh_dialougeOnly_Win:freshMoneyList()
...
function UITopMoneyHigh_dialougeOnly_Win:fillData(index,moneyInfo)
...
function UITopMoneyHigh_dialougeOnly_Win:freshMoneyValue(index,moneyInfo,lastVal)
local rootStr=FMT.fmt('moneyRoot{0}',index)
if moneyInfo==nil then
self[rootStr]:setActive(false)
return
...
function UITopMoneyHigh_dialougeOnly_Win:clearFMTweener(mtype)
...
function UITopMoneyHigh_dialougeOnly_Win:getMoneyRoot(moneyType)
...
function UITopMoneyHigh_dialougeOnly_Win:flyMoneyIcon(spos,moneyType)
...
function UITopMoneyHigh_dialougeOnly_Win:handleFly(moneyType,spos,tpos)
...
function UITopMoneyHigh_dialougeOnly_Win:onMoneyChange(moneyType,lastVal,val)
local moneyList=self.money
...
function UITopMoneyHigh_dialougeOnly_Win:onItemChange(changeType,itemguid,itemid,lastcount,itemcount)
local moneyList=self.money
for i,v in ipairs(moneyList)do
local moneyInfo=moneyList[i]
