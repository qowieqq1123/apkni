UIScaleNode=simple_class(baseNode)


function UIScaleNode:reset()
self._base.reset(self)
self.isplaying=false
self.bComplete=false
end

function UIScaleNode:update(interval)
...
local tweener=widget:SetChildDOScale(wIndex,val,duration,function()
self.isplaying=false
self.bComplete=true
self:quicklyTick()
end)
...
widget:SetChildScale(wIndex,Vector3(val,val,val))
return nodeState.success
end
