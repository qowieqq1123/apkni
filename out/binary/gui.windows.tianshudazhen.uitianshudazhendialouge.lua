...
def_class("UITianShuDaZhenDialouge",UIWindowBase)
...
function UITianShuDaZhenDialouge:bindComponents()
...
function UITianShuDaZhenDialouge:unbindComponents()
...
function UITianShuDaZhenDialouge:onLoaded(...)
self:bindComponents()
...
function UITianShuDaZhenDialouge:__delete()
...
function UITianShuDaZhenDialouge:onShow(showdata)
...
if showdata.alignment then
self.winlua:SetChildTextAlignment(self.dialougeText:getID(),showdata.alignment)
end

if showdata.canvasindex then
self.winlua:SetCanvasIndex(11,showdata.canvasindex)
end
...
if moneytypes then
showMoney=true
UIManager:showWindow('UITopMoneyHigh_dialougeOnly_Win',{moneytypes=moneytypes,parentName='UITianShuDaZhenDialouge'})
else
showMoney=false
UIManager:closeWindow('UITopMoneyHigh_dialougeOnly_Win')
end
...
function UITianShuDaZhenDialouge:setContent(content)
self.dialougeText:setText(content)
end


function UITianShuDaZhenDialouge:handleTimeCount(time)
if time>0 then
...
function UITianShuDaZhenDialouge:autoCancelClick(time)
if time>0 then
local function tick(i,t,c)
...
function UITianShuDaZhenDialouge:autoOkClick(time)
if time>0 then
local function tick(i,t,c)
...
function UITianShuDaZhenDialouge:onOkButton()
...
function UITianShuDaZhenDialouge:onCancelButton()
...
function UITianShuDaZhenDialouge:onBGClick()
if self.showdata.allowclickBG==nil or self.showdata.allowclickBG==true then
self:onCloseBtn()
end
end

function UITianShuDaZhenDialouge:onCloseBtn()

self:doClose()
end

function UITianShuDaZhenDialouge:doClose()
...
function UITianShuDaZhenDialouge:doOk()
local okcallback=self.showdata.okcallback
if okcallback then
okcallback()
end
...
function UITianShuDaZhenDialouge:doCancel()
self:closeSelf()
end

function UITianShuDaZhenDialouge:onChangeChoose()
