...
worldTaskSky_Manual=simple_class(worldTaskSky)

function worldTaskSky_Manual:__init(data)
worldTaskSky.__init(self,data)
end

function worldTaskSky_Manual:start(serverTime)
worldTaskSky.start(self,serverTime)

if not self.corners then return end

self.offsetList=self.sky_offset[self:getShowDiscipleCnt()]
...
function worldTaskSky_Manual:createMoveData(index,forward,fast)
...
function worldTaskSky_Manual:startMove(isForward,fast)
...
function worldTaskSky_Manual:onMoveComplete(index,fast)
...
function worldTaskSky_Manual:goBack()
...
function worldTaskSky_Manual:isWaitingBack()
