...
local xjEntityData_RPEvent={}


function xjEntityData_RPEvent:onInit()
local cfg=self:getCfg()
self.name="xjEntityData_RPEvent"
...
function xjEntityData_RPEvent:refreshData(d)
for i,v in pairs(d)do
self[i]=v
end
end

function xjEntityData_RPEvent:getCfg()
return xianjieModel:getXJResPointClassifyCfg(self.rpType,self.rpId)
end

function xjEntityData_RPEvent:createEntity(needRefreshAOI)
if xianjieModel:checkSceneIndex(self.sceneidx)and self.ent_key==nil then
self.ent_key=xianjieController:addEntity(XJ_ENTITY_TYPE.eRPEvent,{guid=self.rpGuid},needRefreshAOI)
end
end


function xjEntityData_RPEvent:onDelete()
if xianjieModel:haveResPointData(self.dataGuid)then
xianjieModel:refreshResPointData(self.dataGuid,nil,false)
end
end

function xjEntityData_RPEvent:getModelData()
local cfg=self:getCfg()
