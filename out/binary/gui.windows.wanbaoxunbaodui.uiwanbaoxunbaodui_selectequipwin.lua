...
def_class("UIWanBaoXunBaoDui_SelectEquipWin",UIWindowBase)
...
function UIWanBaoXunBaoDui_SelectEquipWin:bindComponents()
...
function UIWanBaoXunBaoDui_SelectEquipWin:unbindComponents()
...
function UIWanBaoXunBaoDui_SelectEquipWin:onLoaded(...)
self:bindComponents()
...
function UIWanBaoXunBaoDui_SelectEquipWin:__delete()
...
function UIWanBaoXunBaoDui_SelectEquipWin:onShow(argtable,afterOnloaded)
...
function UIWanBaoXunBaoDui_SelectEquipWin:onHide()
...
function UIWanBaoXunBaoDui_SelectEquipWin:onCloseBtn()

UIFullWanBaoXunBaoDuiController:closeWindow('UIWanBaoXunBaoDui_SelectEquipWin')
end

function UIWanBaoXunBaoDui_SelectEquipWin:initUI()
local equipdata=wanBaoXunBaoDuiController:getMaomaoEquipBagDataInBag(1,self.dropDownIndex)

local equipedData={}
if self.type==WBXBD_SelectEquip_TYPE.REFINE then
equipedData=wanBaoXunBaoDuiModel:getCatEquips()
else
if self.catguid then
local catinfo=wanBaoXunBaoDuiModel:getCatData(self.catguid)
equipedData=catinfo.equipList or{}
end
end
...
function UIWanBaoXunBaoDui_SelectEquipWin:onStartView()

end

function UIWanBaoXunBaoDui_SelectEquipWin:onFreshListView(index,item)
index=index+1
...
function UIWanBaoXunBaoDui_SelectEquipWin:freshEventListRect()
self.loopListViewCmp:ResetListView()
end

function UIWanBaoXunBaoDui_SelectEquipWin:freshQualityDropDown()
local optiontxts={}
for index=1,5 do
table.insert(optiontxts,FMT.fmt('{0}品质及以上',eQualityColorName[index]))
end
self.sortTypeDropdown:setOption(optiontxts)
self.sortTypeDropdown:setValue(self.dropDownIndex-1)
end

function UIWanBaoXunBaoDui_SelectEquipWin:onChangeQualityDropDown(dropIdx)
self.selectIndex=1
local idx=dropIdx+1
