...
def_class("UISubAct_QianDaoWin",UIWindowBase)
...
function UISubAct_QianDaoWin:bindComponents()
...
function UISubAct_QianDaoWin:unbindComponents()
...
function UISubAct_QianDaoWin:onLoaded(...)
self:bindComponents()
end


function UISubAct_QianDaoWin:__delete()
...
function UISubAct_QianDaoWin:onShow(argtable,afterOnloaded)
...
function UISubAct_QianDaoWin:createActor(bodyId,otherData,animationID,pos,animList,effectAnim,animTime,animAudio)
...
function UISubAct_QianDaoWin:setAnimAndEffect(stWidget,idx,animationID,animList,effectAnim,animTime)
if effectAnim then
...
function UISubAct_QianDaoWin:setAnimAudio(idx,animationID,animList,animTime,animAudio)
...
function UISubAct_QianDaoWin:refreshActor()
...
function UISubAct_QianDaoWin:onHide()
...
function UISubAct_QianDaoWin:refreshList(dataRecv)
...
elseif i<today then
item:SetChildActive(1,isGot)
item:SetChildCSImageSprite(6,abName,dajiang and assetmaskName[2]or assetmaskName[1])
...
item:SetChildCSImageSprite(6,abName,dajiang and assetName[2]or assetName[1])
...
function UISubAct_QianDaoWin:onClickRewardItem(itemId,index,guid,attach)
if itemId==-1 then
return
end
...
function UISubAct_QianDaoWin:setRemainingTimeTimer()
self:clearTimer()
local func
func=function()
...
function UISubAct_QianDaoWin:refreshRemainingTimeTimer()
local time=activitiesModel:getSubActEndLeftTime(self.actid,self.subType,self.subid)
if time>0 then

local time_str=FMT.fmt('活动剩余时间：<color=#171311>{0}</color>',timeHelper.format_time_stamp11(time,true))
...
function UISubAct_QianDaoWin:clearTimer()
if self.timer then
...
function UISubAct_QianDaoWin:onCloseBtn()
if self.activityArgs.parentWin then
