...
def_class("UIGuildOrderSetupWin_autoBaiShan",UIWindowBase)
...
function UIGuildOrderSetupWin_autoBaiShan:bindComponents()
...
function UIGuildOrderSetupWin_autoBaiShan:unbindComponents()
...
function UIGuildOrderSetupWin_autoBaiShan:onLoaded(...)
self:bindComponents()
_this=self
...
function UIGuildOrderSetupWin_autoBaiShan:__delete()
self:unbindComponents()
end


function UIGuildOrderSetupWin_autoBaiShan:onHide()
...
function UIGuildOrderSetupWin_autoBaiShan:onShow(argtable,afterOnloaded)
self.orderID=GUILD_ORDER_TYPE.eAutoBaiShan
self.isChange=false
...
function UIGuildOrderSetupWin_autoBaiShan:onControllBtn(idx)
...
function UIGuildOrderSetupWin_autoBaiShan:refreshItemControllBtn(orderID)
...
function UIGuildOrderSetupWin_autoBaiShan:setgrayall(flag,isInit)
for i=1,#_this.jumpAnimationlist do
local widget=_this.jumpAnimationlist[i]:getWidgetBase()
widget:SetChildGray(1,flag)
widget:SetChildGray(2,flag)
end
for i=1,#_this.jumpAnimationtzlist do
local widgettwo=_this.jumpAnimationtzlist[i]:getWidgetBase()
...
function UIGuildOrderSetupWin_autoBaiShan:checkGray()
if self.isgray then
...
function UIGuildOrderSetupWin_autoBaiShan:refreshPinZhiAllToggleState()
local isall=true
...
function UIGuildOrderSetupWin_autoBaiShan:onPinZhiAllToggleChanged(name,isToggle,data)
...
function UIGuildOrderSetupWin_autoBaiShan:setcolorButton()
for i=1,#_this.jumpAnimationlist do
local widget=_this.jumpAnimationlist[i]:getWidgetBase()
...
self:colorchoose(widget,_this.setcolorflag[i])
end
end
function UIGuildOrderSetupWin_autoBaiShan:colorchoose(widget,flag)
...
function UIGuildOrderSetupWin_autoBaiShan:onChangeColorClick(widget,index)
if self.lockRefresh then return end
if self.isgray then
UIManager.info('拜山弟子法令已关闭，开启法令后可变更选项')
return
end
...
function UIGuildOrderSetupWin_autoBaiShan:refreshColorClick(widget,index)
local setup,cfg=guildOrderModel:getSetupData(self.orderID)
...
function UIGuildOrderSetupWin_autoBaiShan:refreshTeZhiAllToggleState()
local isall=true
...
function UIGuildOrderSetupWin_autoBaiShan:onTeZhiAllToggleChanged(name,isToggle,data)
if self.lockRefresh then return end
if not self:checkGray()then
self.lockRefresh=true
self.tezhiAll:setToggle(self.isTeZhiAllToggle)
...
function UIGuildOrderSetupWin_autoBaiShan:settezhiButton()
for i=1,#_this.jumpAnimationtzlist do
local widget=_this.jumpAnimationtzlist[i]:getWidgetBase()
...
self:tezhichoose(widget,_this.settezhiflag[i])
end
end
function UIGuildOrderSetupWin_autoBaiShan:tezhichoose(widget,flag)
local ischoose=flag==1 and true or false
widget:SetChildActive(0,not ischoose)
widget:SetChildActive(1,ischoose)
end
...
function UIGuildOrderSetupWin_autoBaiShan:onChangetezhiClick(widget,index)
if self.lockRefresh then return end
if _this.isgray then
UIManager.info('拜山弟子法令已关闭，开启法令后可变更选项')
return
end
_this.settezhiflag[index]=_this.settezhiflag[index]==1 and 0 or 1
local setup,cfg=guildOrderModel:getSetupData(_this.orderID)
setup.specialityPreviewFlaglist[index]=_this.settezhiflag[index]

self:tezhichoose(widget,_this.settezhiflag[index])
...
function UIGuildOrderSetupWin_autoBaiShan:refreshGuanZhuAllToggleState()
local isall=true
...
function UIGuildOrderSetupWin_autoBaiShan:onGuanZhuAllToggleChanged(name,isToggle,data)
if self.lockRefresh then return end
if not self:checkGray()then
self.lockRefresh=true
self.guanzhuAll:setToggle(self.isGuanZhuAllToggle)
...
function UIGuildOrderSetupWin_autoBaiShan:setguanzhuButton()
...
function UIGuildOrderSetupWin_autoBaiShan:onDescSlotClick(idx,spetype,speid)
...
function UIGuildOrderSetupWin_autoBaiShan:guanzhuchoose(widget,flag)
local ischoose=flag==1 and true or false
...
function UIGuildOrderSetupWin_autoBaiShan:onChangeguanzhuClick(widget,index)
if self.lockRefresh then return end
