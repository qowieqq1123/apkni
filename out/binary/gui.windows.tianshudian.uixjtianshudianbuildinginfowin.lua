...
def_class("UIXJTianShuDianBuildingInfoWin",UIWindowBase)
...
function UIXJTianShuDianBuildingInfoWin:bindComponents()
...
self.conditionRoot=UIObject.get(self,8)
self.conditionScrollview=UIObject.get(self,9)
self.costIcon=UIObject.get(self,10)
self.costValue=UIText.get(self,11)
self.detailButton=UIButton.get(self,12)
self.detailOpen=UIObject.get(self,13)
self.iconAds=UIImage.get(self,14)
self.lvUpBtn=UIButton.get(self,15)
self.lvUpReddot=UIObject.get(self,16)
self.materials=UIObject.get(self,17)
self.maxLevelTip=UIObject.get(self,18)
self.model=UIObject.get(self,19)
self.payAds=UIText.get(self,20)
self.sliderUpgrade=UIObject.get(self,21)
self.small_model=UIObject.get(self,22)
self.speedUpBtnText=UIText.get(self,23)
self.suCost=UIObject.get(self,24)
self.txtUpgradeBar=UIText.get(self,25)
self.upgradePanel=UIObject.get(self,26)
self.upTime=UIText.get(self,27)
...
function UIXJTianShuDianBuildingInfoWin:unbindComponents()
...
function UIXJTianShuDianBuildingInfoWin:onLoaded(...)
self:bindComponents()
_this=self
self.checkList={}
...
function UIXJTianShuDianBuildingInfoWin:__delete()
...
function UIXJTianShuDianBuildingInfoWin:onShow(argtable,afterOnloaded)
...
function UIXJTianShuDianBuildingInfoWin:refreshLevel()
...
local isOnlyShowAdd=false
if self.attrConfig.getIsOnlyShowAdd then
isOnlyShowAdd=self.attrConfig:getIsOnlyShowAdd()or nil
end
...
if isOnlyShowAdd then
if next and next.val>v.val then
...
local numStr=xianjieModel:getBuildingAttrVal(attr.val,attr.flag)
if attr.icon then
local iconName=iconHelper.getIconName(attr.icon)
local iconStr=chatEmotHelper.getIconEmotMesg(iconName,30)
numStr=FMT.fmt("{0} {1}",iconStr,numStr)

end

childItem:SetChildText(1,numStr)
local nextAttr=list[i][2]
if nextAttr then
childItem:SetChildActive(2,true)
childItem:SetChildText(2,xianjieModel:getBuildingAttrVal(nextAttr.val,nextAttr.flag))
else
...
self.maxLevelTip:setActive(isMax)
...
function UIXJTianShuDianBuildingInfoWin:refreshMoneyPanel()
...
function UIXJTianShuDianBuildingInfoWin:refreshCostPanel()
...
local costLen=Mathf.Max(#cost,5)

self.materials:setChildLayoutGroupCreateItems(costLen,function(i)
local item=self.materials:getChildLayoutGroupGridItem(i-1)

local isShow=cost[i]~=nil
...
local conf={itemid=matItemId,showStage=showStage,itemcount=countStr,showCountBG=true,showname=false}
...
item:SetBaseItemChildID(-1,matItemId)
item:SetBaseItemClickEvent(-1,_onClickMaterialItem)
end
end)
...
function UIXJTianShuDianBuildingInfoWin:onClickMaterialItem(itemId,index,guid,attach)
...
function UIXJTianShuDianBuildingInfoWin:getLevelUpCND(cfgs)
local list={}
...
data.pass=systemModel.isOpen(v.param)
end
...
function UIXJTianShuDianBuildingInfoWin:refreshConditionPanel(config)
local datas=self:getLevelUpCND(config.uplevel_condition)

local len=#datas
self.conditionRoot:setChildLayoutGroupCreateItems(len)

local passCount=0

for i=1,#datas do
local widget=self.conditionRoot:getChildLayoutGroupGridItem(i-1)
local data=datas[i]
if data then
local str
local bicon
...
self.cant_upgrade=passCount<len

self.lvUpReddot:setActive(self:isCanLevelUp())
end

function UIXJTianShuDianBuildingInfoWin:onGoToButton(ftype,arg1,arg2)
if ftype==1 then
self:onClickClose()
...
function UIXJTianShuDianBuildingInfoWin.on_building_event(etype,sfId,bdId,arg1,arg2)
...
function UIXJTianShuDianBuildingInfoWin:onHide()

end

function UIXJTianShuDianBuildingInfoWin:isCanLevelUp()
...
function UIXJTianShuDianBuildingInfoWin:refreshUpgradePanel()
...
function UIXJTianShuDianBuildingInfoWin:startLevelUpTimer()
local tick=function()
local cdd=buildingCDControl:getCDData(buildingCDType.build,self.bdData.un_build_id)
local dtime=cdd.cd
self.lastTime=dtime
...
function UIXJTianShuDianBuildingInfoWin:stopLevelUpTimer()
self:removeCDUpdateFunc('UPCD')
end

function UIXJTianShuDianBuildingInfoWin:onDetailWinClose()
self.detailOpen:setActive(false)
end


function UIXJTianShuDianBuildingInfoWin:setSPNeedText(need,mtype)
local needText
...
function UIXJTianShuDianBuildingInfoWin:updateSPNeedText()
...
function UIXJTianShuDianBuildingInfoWin:refreshSpeedPanel(typo)
...
function UIXJTianShuDianBuildingInfoWin:getSortSpeedupItemList()
local sortItemList={}
...
function UIXJTianShuDianBuildingInfoWin:setSpeedupItemExpireTimer(expireTime)
...
function UIXJTianShuDianBuildingInfoWin:clearSpeedupItemExpireTimer()
...
function UIXJTianShuDianBuildingInfoWin:onBtnAdsSpeedup()
...
function UIXJTianShuDianBuildingInfoWin:showDialog()
local data=self.baseCfg[1]
...
function UIXJTianShuDianBuildingInfoWin:moneySpeedUp()
local data=self.baseCfg[1]
local need,count=self:countSpeedUpNeed(data)
local moneyId=data[1]
local have=moneyModel.getMoney(moneyId)
...
function UIXJTianShuDianBuildingInfoWin:onAdItemClick()
if self.speedup_item_id then
tipsManager.showTips({formType=TIPS_FORM_TYPE.eWatchItem,itemid=self.speedup_item_id})
end
end

function UIXJTianShuDianBuildingInfoWin:countSpeedUpNeed(needData)
...
function UIXJTianShuDianBuildingInfoWin:onBtnFinishUpgrade()
...
function UIXJTianShuDianBuildingInfoWin:onDetailButton()
self.detailOpen:setActive(true)
self:showWindow("UIXJTianShuDianBuildingDetailWin",{bdData=self.bdData})
end



function UIXJTianShuDianBuildingInfoWin:onLvUpBtn()
...
function UIXJTianShuDianBuildingInfoWin:handleLevelUp()
...
function UIXJTianShuDianBuildingInfoWin:onSpeedBtn()

end

function UIXJTianShuDianBuildingInfoWin:onClickClose()
self:closeSelf()
end
