...
def_class("UILingZhenHHroleGameExWin",UIWindowBase)
...
function UILingZhenHHroleGameExWin:bindComponents()
...
function UILingZhenHHroleGameExWin:unbindComponents()
...
function UILingZhenHHroleGameExWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UILingZhenHHroleGameExWin:__delete()
...
function UILingZhenHHroleGameExWin:onShow(argtable,afterOnloaded)
...
local cfg=cfgHelper.get1(cfg_lingzhenhuihua2config_get,self.subid)
self.cbNum=cfg.num
...
local npcSpeakParam=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"catSpeakParam")
...
self.mapCfg=cfgHelper.get(cfg_lingzhenhuihua2mapconfig_get,self.selectMapId)
self.mapIconNameList=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"mapIconName")
...
function UILingZhenHHroleGameExWin:onHide()
self:clearComboModeTimer()
self:clearTimer()
self:clearSpeakTimer()
end

function UILingZhenHHroleGameExWin:initData()
local list={}
...
function UILingZhenHHroleGameExWin:countPos(index,layer,needCountMinPos)
...
function UILingZhenHHroleGameExWin:setCreateData()
...
function UILingZhenHHroleGameExWin:countTopLeft()
local x,y=self:countTopLeftByData(self.layer)
self.rectMinX=x
self.rectMinY=y
end

function UILingZhenHHroleGameExWin:countTopLeftByData(layer)
local layerData=self.mapData[layer]
local col=layerData.col
...
function UILingZhenHHroleGameExWin:getNextIndex()
...
function UILingZhenHHroleGameExWin:createItemList()
self.isPlaying=true
...
function UILingZhenHHroleGameExWin:clearTimer()
...
function UILingZhenHHroleGameExWin:addToItemList(data,checkStack)
...
function UILingZhenHHroleGameExWin:removeFormItemList(data,checkStack)
...
function UILingZhenHHroleGameExWin:setAllItemStackUp()
...
function UILingZhenHHroleGameExWin:setChangeItemStackUp(data,checkAdd)
...
function UILingZhenHHroleGameExWin:checkStackUp(dataA,dataB)
...
function UILingZhenHHroleGameExWin:checkStackUp_allUpLayer(data)
...
function UILingZhenHHroleGameExWin:checkHasUnderItem(data)
local itemLayer=data.layer
local itemX=data.x
local itemY=data.y
...
function UILingZhenHHroleGameExWin:handleItemClick(id)
...
function UILingZhenHHroleGameExWin:addToCombineList(data)
self.isPlaying=true
...
function UILingZhenHHroleGameExWin:playMoveAnim(index,callback)
...
function UILingZhenHHroleGameExWin:checkAndCombine()
local rval=-1
local index=-1
local count=0
for i,v in ipairs(self.combineList)do
...
function UILingZhenHHroleGameExWin:addScore(score)
self:setScore(self.score:getValue()+score)
end

function UILingZhenHHroleGameExWin:enterComboMode()
...
function UILingZhenHHroleGameExWin:clearComboModeTimer()
...
function UILingZhenHHroleGameExWin:clearComboMode()
self:clearComboModeTimer()
self.isComboMode=false
end

function UILingZhenHHroleGameExWin:setScore(score)
...
function UILingZhenHHroleGameExWin:addToQueue(data)
table.insert(self.selectQueue,1,data)
end

function UILingZhenHHroleGameExWin:removeFormQueue(id)
...
function UILingZhenHHroleGameExWin:pushToPool(id,widget)
widget:SetChildActive(-1,false)
self.itemPool[id]=widget
end

function UILingZhenHHroleGameExWin:getItemFromPool()
...
function UILingZhenHHroleGameExWin:handleSuccess()
...
activitiesHandle_rolelingzhendiaoke:reqSetScore(_this.actid,_this.subType,_this.subid,score)
local callback=function()
UIFullLingZhenHuiHuaRoleControl:showMainWinNoCloud(_this.activityArgs)
if not _this then return end
self:closeSelf()
end
...
function UILingZhenHHroleGameExWin:handleFailure()
self:clearComboMode()
local count=self.skillCount[4]
local skillId=4
local cfg=cfgHelper.get1(cfg_lzhh2skillinfoconfig_get,skillId)
...
function UILingZhenHHroleGameExWin:showFailureWin()
...
function UILingZhenHHroleGameExWin:showSkillWin(id)

self:showWindow('UILZHHroleSkillWin',{id=id,count=self.skillCount[id],act_id=self.actid,sub_act_type=self.subType,sub_act_id=self.subid})

end

function UILingZhenHHroleGameExWin:handleSkill(id)
...
function UILingZhenHHroleGameExWin:checkNeedHandleSkillList()
...
function UILingZhenHHroleGameExWin:refreshSkillBtn()
...
function UILingZhenHHroleGameExWin:removeItem(removeCount,isLast)
local list={}
local len=#self.combineList
...
function UILingZhenHHroleGameExWin:removeSingleItemById(targetId)
local list={}
local len=#self.combineList
for i=1,len do
...
function UILingZhenHHroleGameExWin:recaptionItem(data)
...
function UILingZhenHHroleGameExWin:revocationItem()
...
function UILingZhenHHroleGameExWin:refreshRecordScrollView()
...
function UILingZhenHHroleGameExWin:randomItemList()
local list={}
...
function UILingZhenHHroleGameExWin:redealItemList()
self:clearTimer()
...
function UILingZhenHHroleGameExWin:clearCombineList()
...
function UILingZhenHHroleGameExWin:initAddScoreShow()
local maxScoreItemCount=10
self.scoreArea:setChildLayoutGroupCreateItems(maxScoreItemCount)
end


function UILingZhenHHroleGameExWin:addScoreShow(score)
...
function UILingZhenHHroleGameExWin:getScoreItemIndex()
...
function UILingZhenHHroleGameExWin:setExpireTimer()
self:clearExpireTimer()
local func=function()
...
endContent="灵阵绘画已结算"
clickCallback=function()
return UIFullLingZhenHuiHuaRoleControl:closeUI(true)
end
end
...
function UILingZhenHHroleGameExWin:clearExpireTimer()
...
function UILingZhenHHroleGameExWin:doSpeaking()
self:clearSpeakTimer()
...
local speakLib=cfgHelper.get(cfg_lingzhenhuihua2config_get,_this.subid,"catSpeakLib")
local specialSpeakLib=cfgHelper.get(cfg_lingzhenhuihua2config_get,_this.subid,"catSpecialSpeakLib")
...
function UILingZhenHHroleGameExWin:doTalkAnim()
...
function UILingZhenHHroleGameExWin:talkEnd()
self:clearSpeakTimer()
...
function UILingZhenHHroleGameExWin:clearSpeakTimer()
...
function UILingZhenHHroleGameExWin:onHelpBtn()
local langId=cfgHelper.get(cfg_lingzhenhuihua2config_get,self.subid,"ruleLangId")or''
local d={}
d.title='规则'
d.mode=3
d.name=langId
...
function UILingZhenHHroleGameExWin:onSkillBtnC()
if self.isCreatingMap or self.isInAnim then
...
function UILingZhenHHroleGameExWin:onSkillBtnB()
if self.isCreatingMap or self.isInAnim then
...
function UILingZhenHHroleGameExWin:onSkillBtnA()
if self.isCreatingMap or self.isInAnim then
...
function UILingZhenHHroleGameExWin:onCloseClick()
...
function UILingZhenHHroleGameExWin:onCloseBtn()
self:onCloseClick()
end

function UILingZhenHHroleGameExWin:onBottomClick()
self.bottomModel:setChildModelAnimationState(eAnimationID.touch)
self:doSpeaking()
end


function UILingZhenHHroleGameExWin:test_printSelectMapId()
UIManager.info(FMT.fmt("当前地图id：{0}",self.selectMapId))
end


function UILingZhenHHroleGameExWin:test_finishGame(score)
self.score:setValue(score)
self:handleSuccess()
end
