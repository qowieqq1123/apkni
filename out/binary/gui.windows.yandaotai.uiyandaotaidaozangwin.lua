...
def_class("UIYanDaoTaiDaoZangWin",UIWindowBase)
...
function UIYanDaoTaiDaoZangWin:bindComponents()
...
self.lineRoot=UIObject.get(self,7)
self.openBtn=UIButton.get(self,8)
self.overviewBtn=UIButton.get(self,9)
self.reddot=UIObject.get(self,10)
self.root=UIObject.get(self,11)
self.scrollView=UIObject.get(self,12)
self.tabList=UIObject.get(self,13)
self.tabScrollView=UIObject.get(self,14)
...
function UIYanDaoTaiDaoZangWin:unbindComponents()
...
function UIYanDaoTaiDaoZangWin:onLoaded(...)
this=self
self:bindComponents()
...
function UIYanDaoTaiDaoZangWin:__delete()
this=nil
...
function UIYanDaoTaiDaoZangWin:onShow(argtable,afterOnloaded)
UIManager:callWindowFunc('UIXianJieBottomMaskWin','showModel',6364,{0,-33})

self.treeType=YDT_TREE_TYPE.eDaoZang
self.tabSelect=1
if argtable then
self.treeType=argtable.treeType or YDT_TREE_TYPE.eDaoZang
...
self.curMaxLevel=cfgHelper.get2(cfg_technologybaseconfig_get,self.buildLevel,'uplimit')
...
function UIYanDaoTaiDaoZangWin:onShowArgRecv(argtable,afterOnloaded)
UIManager:callWindowFunc('UIXianJieBottomMaskWin','showModel',6364,{0,-33})
self:refreshWin(true)
end


function UIYanDaoTaiDaoZangWin:onHide()

end

function UIYanDaoTaiDaoZangWin:initTabList()
local showLen=0
...
function UIYanDaoTaiDaoZangWin:refreshTabList()
...
function UIYanDaoTaiDaoZangWin:refreshTabListReddot()
local showLen=0
local list=yandaotaiModel:getTechnologyTabList(this.treeType)
for i,cfg in ipairs(list)do
if yandaotaiModel:getIsShowTree(cfg.id)then
...
function UIYanDaoTaiDaoZangWin:refreshLevelUpPanel()
...
function UIYanDaoTaiDaoZangWin.on_building_event(etype,sfId,bdId,arg1,arg2)
if not this then
return
end
...
this.curMaxLevel=cfgHelper.get2(cfg_technologybaseconfig_get,this.buildLevel,'uplimit')
...
function UIYanDaoTaiDaoZangWin:refreshWin(isInit)
...
function UIYanDaoTaiDaoZangWin.refreshMapItem(index)
local areaCmp=
{
owner=0,
button=1,
...
if maxLevel>this.curMaxLevel then
maxlv=this.curMaxLevel
else
maxlv=maxLevel
end
...
function UIYanDaoTaiDaoZangWin:playEffect(index)
...
function UIYanDaoTaiDaoZangWin.onClickArea(index,maxLevel)
...
{
id=id,
level=yandaotaiModel:getTechnologyListLevel(id)or 0,
bdData=this.bdData,
...
function UIYanDaoTaiDaoZangWin:initLine()
local lineList={}
...
function UIYanDaoTaiDaoZangWin:refreshLine()
...
function UIYanDaoTaiDaoZangWin:onLevelUpBtn()
self:showWindow("UIXJBuildingInfoWin",self.bdData)
end


function UIYanDaoTaiDaoZangWin:onOpenBtn()
local call=function()
...
function UIYanDaoTaiDaoZangWin:onReturnBtn()
