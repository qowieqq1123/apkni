...
def_class("UIMoJieStageAimChapterContentWin_ZhenJi",UIWindowBase)
...
function UIMoJieStageAimChapterContentWin_ZhenJi:bindComponents()

self.background=UIImage.get(self,0)
self.btnPTZJTiShi=UIButton.get(self,1)
...
self.progressBox=UIObject.get(self,6)
...
self.btnPTZJTiShi:setButtonClick(function()self:onBtnPTZJTiShi()end)
...
function UIMoJieStageAimChapterContentWin_ZhenJi:unbindComponents()
local _UIObject_release=UIObject.release
_UIObject_release(self.background);self.background=nil;
_UIObject_release(self.btnPTZJTiShi);self.btnPTZJTiShi=nil;
...
function UIMoJieStageAimChapterContentWin_ZhenJi:onLoaded(...)
self:bindComponents()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:__delete()
_this=nil

self:stopOpenTimer()
self:stopAutoEndTimer()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:onShow(argtable,afterOnloaded)
...
function UIMoJieStageAimChapterContentWin_ZhenJi:onHide()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:onJumpBtn()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:refreshAll()
self:initActiveData()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:initActiveData()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:activeObjects()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:freshBackGround()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:freshProgressBox()
local wb=self.progressBox:getChildWidgetBase()

local tips=FMT.fmt("{0}：{1}",self.winArgs.stageScoreRewardTips,self.stageHandle.chapter_stage_scroe)
...
local pval=seasonModel:getStageSegementProgressVal(self.showParams.handleType,self.showParams.stageIdx,self.stageHandle.chapter_stage_scroe)
wb:SetChildProgressValue(1,pval*100,100)
wb:SetChildProgressText(1,"")

local serverWideScore=self.stageHandle.chapter_stage_scroe
...
function UIMoJieStageAimChapterContentWin_ZhenJi:onClickBox(index)
...
local serverWideTargetScore=stageScoreRewardInfo[_stageRewardInfoIndex.target]
local serverWideCurrentScore=self.stageHandle.chapter_stage_scroe
...
function UIMoJieStageAimChapterContentWin_ZhenJi:freshJumpBtn()
...
local type=args[1]
local tips=args[2]
if type==0 or type==2 then return tips end

if type==1 then
return FMT.fmt(tips,_this.stageHandle.chapter_scroe)
end
end
function UIMoJieStageAimChapterContentWin_ZhenJi:freshTips()
...
if self.winArgs.recordStr[1]==2 then
...
self.isShowMidTips=tipStr~=nil or showTipStr
if tipStr then
...
function UIMoJieStageAimChapterContentWin_ZhenJi:startAutoEndTimer()
self:stopAutoEndTimer()

local curTime=timeHelper.getServerShortTime()
local beginTime=self.stageHandle.beginTime
local cfg=seasonModel:getStageConfigEx(_this.handleType,_this.stageIdx)
local autoEndTime=beginTime+cfg.autoEnd

local strFmt=_attachmentArgsTips(self.winArgs.recordStr)

local func=function()
curTime=timeHelper.getServerShortTime()
local left=autoEndTime-curTime
local timeStr=timeHelper.format_time_stamp4(left)
local infoStr=FMT.fmt(strFmt,timeStr)
_this.tipsTx:setText(infoStr)

if left<0 then
_this:stopAutoEndTimer()
_this:refreshAll()
end
end

self.autoEndTimer=self:setTimer(1,0,func)
func()
end

function UIMoJieStageAimChapterContentWin_ZhenJi:stopAutoEndTimer()
if self.autoEndTimer then
self:stopTimerByID(self.autoEndTimer)
self.autoEndTimer=nil
end
end

function UIMoJieStageAimChapterContentWin_ZhenJi:startOpenTimer()
self:stopOpenTimer()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:stopOpenTimer()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:freshMainTask()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:freshFinishImg()
end

function UIMoJieStageAimChapterContentWin_ZhenJi:refreshStagePassLevelRewardPart()
...
function UIMoJieStageAimChapterContentWin_ZhenJi:onBtnPTZJTiShi()
local screenPos=self.btnPTZJTiShi:getChildUIScreenPos(false)
screenPos.y=screenPos.y+12
local winParams={
parentWin=self,
width=270,
descStrList={"阶段结束后，进度不再累计"},
screenPos=screenPos,
}
self:showWindow("UIXianJie_puTongZhenJi_mojingRuleWin",winParams)
end

function UIMoJieStageAimChapterContentWin_ZhenJi.on_39_2(season_id,chapter_idx,param1,param2)
if season_id==_this.handleType and chapter_idx==_this.stageIdx and _this.stage then

end
end

function UIMoJieStageAimChapterContentWin_ZhenJi.on_39_3(season_id,chapter_idx)
if season_id==_this.handleType and chapter_idx==_this.stageIdx and _this.stage then

end
end

function UIMoJieStageAimChapterContentWin_ZhenJi.onSeasonChange()
_this.stageHandle=seasonModel:getStage(_this.handleType,_this.stageIdx)
_this:refreshAll()

end

function UIMoJieStageAimChapterContentWin_ZhenJi.onSeasonStageChange(season_id,chapter_idx)
...
function UIMoJieStageAimChapterContentWin_ZhenJi.onSeasonStageDataChange(season_id,chapter_idx)
if _this.handleType==season_id and chapter_idx==_this.stageIdx then
_this.stageHandle=seasonModel:getStage(_this.handleType,_this.stageIdx)
_this:refreshAll()
end
