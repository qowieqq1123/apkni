worldTripProgress_RunTo=simple_class(worldTripProgress_Base)
worldTripProgress_RunTo.name="worldTripProgress_RunTo"

function worldTripProgress_RunTo:__init(trip)
worldTripProgress_Base.__init(self,trip)
self.duration=math.ceil(self.trip.trip_duration)
end

function worldTripProgress_RunTo:start(time)
...
function worldTripProgress_RunTo:addTrigger()
...
function worldTripProgress_RunTo:addMove(index)
local startPos=self.trip.corners[1]
local endPos=self.trip.corners[#self.trip.corners]
...
moveList:addMove(worldTripMove_GradientShow.New(obj,startPos,true,true))

moveList:addMove(worldTripMove_NavMove.New(obj,self.trip.corners,self.trip.speed,self.trip.move_duration))
...
function worldTripProgress_RunTo:onDiscipleChange()
