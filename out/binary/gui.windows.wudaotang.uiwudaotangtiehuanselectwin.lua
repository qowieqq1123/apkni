...
def_class("UIWuDaoTangTieHuanSelectWin",UIWindowBase)
...
function UIWuDaoTangTieHuanSelectWin:bindComponents()
...
function UIWuDaoTangTieHuanSelectWin:unbindComponents()
...
function UIWuDaoTangTieHuanSelectWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UIWuDaoTangTieHuanSelectWin:__delete()
self:unbindComponents()
end


function UIWuDaoTangTieHuanSelectWin:onHide()

end

function UIWuDaoTangTieHuanSelectWin:refreshAfterItemUse(...)
...
function UIWuDaoTangTieHuanSelectWin:onShow(argtable,afterOnloaded)
local entityID=argtable.entityID
self.parentWin=argtable.parentWin
self.dizii_index=argtable.dizii_index
self.is_qihuan=argtable.is_qihuan or false

self.dizii_data=argtable.dizii_data
if self.dizii_data then
self.old_dizi_guid=self.dizii_data.guid
end
...
local btnGray=false
if _this.select_index<0 or _this.old_dizi_guid then
btnGray=true
end
...
function UIWuDaoTangTieHuanSelectWin:getPlanList()
self.planlist={}
...
function UIWuDaoTangTieHuanSelectWin:refreshSelectPlanGrid()
local buildLv=self.buildLv
...
function UIWuDaoTangTieHuanSelectWin:onSelectPlan(index)
...
function UIWuDaoTangTieHuanSelectWin:refreshSelectPlanItem(index,isselect)
local widget=self.selectWidgets[index]
widget:SetChildActive(0,isselect)
end

function UIWuDaoTangTieHuanSelectWin:getCurPlanID()
...
function UIWuDaoTangTieHuanSelectWin:getDZFixNum(planid)
...
function UIWuDaoTangTieHuanSelectWin:getDiscipleList()
...
if self.wudaoing_guids[1]==guid and _this.old_dizi_guid~=guid then
weight=90
checkFlag=false
end
end
if self.wudaoing_guids[2]then
if self.wudaoing_guids[2]==guid and _this.old_dizi_guid~=guid then
weight=90
checkFlag=false
end
end
if self.wudaoing_guids[3]then
if self.wudaoing_guids[3]==guid and _this.old_dizi_guid~=guid then
weight=90
checkFlag=false
end
end
...
function UIWuDaoTangTieHuanSelectWin:refreshDiscipleGrid()
...
if _this.select_index==-1 and _this.old_dizi_guid==guid then

UIWuDaoTangTieHuanSelectWin:onDisciplClick(1,i-1)
end
end
...
function UIWuDaoTangTieHuanSelectWin:refreshDisclpleSelect(idx,isselect)
if idx<0 then
return
end
...
function UIWuDaoTangTieHuanSelectWin:refreshDisclpleSelectEx(item,isselect)
item:SetChildActive(10,isselect)
end

function UIWuDaoTangTieHuanSelectWin:onDisciplClick(clicknum,index)
index=index+1
if index==0 then
return
end
local locData=_this.disciplelist[index]
local netdata=locData.disciple
local guid=netdata.discipleguid
...
if _this.wudaoing_guids[1]then
if _this.wudaoing_guids[1]==guid and _this.old_dizi_guid~=guid then
UIManager.error(FMT.fmt('不能选择悟道中的弟子'))
return
end
end
if _this.wudaoing_guids[2]then
if _this.wudaoing_guids[2]==guid and _this.old_dizi_guid~=guid then
UIManager.error(FMT.fmt('不能选择悟道中的弟子'))
return
end
end
if _this.wudaoing_guids[3]then
if _this.wudaoing_guids[3]==guid and _this.old_dizi_guid~=guid then
UIManager.error(FMT.fmt('不能选择悟道中的弟子'))
return
end
end
...
local isselect=_this.curSelectLookup[discipleguidStr]==true
if isselect then
_this.curSelectLookup[discipleguidStr]=nil
else
local num=_this:getSelectDiscipleNum()
if num>=maxMan then



end
_this.curSelectLookup[discipleguidStr]=true
end

local old=_this.select_index
_this.select_index=index
_this:refreshDisclpleSelect(old,false)
_this:refreshDisclpleSelect(index,true)


_this.select_dz=netdata.discipleguid

local btnGray=false
if _this.select_index<0 or _this.old_dizi_guid==guid then
btnGray=true
end
_this.commitBtn:setGray(btnGray)
_this.commitBtn:setButtonInteractable(not btnGray)

_this:refreshDiscipleNum()
end

function UIWuDaoTangTieHuanSelectWin:getSelectDiscipleNum()
local num=0
...
function UIWuDaoTangTieHuanSelectWin:refreshCostView()
...
function UIWuDaoTangTieHuanSelectWin:refreshDiscipleNum()
local planid=self:getCurPlanID()
local buildLv=self.buildLv
...
function UIWuDaoTangTieHuanSelectWin:onCommitBtn()
local func=function()
local select_dz=_this.disciplelist[_this.select_index].disciple.discipleguid


if _this.old_dizi_guid~=select_dz then
...
function UIWuDaoTangTieHuanSelectWin:onRewardBtn()
