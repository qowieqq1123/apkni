...
def_class("UIXianJie_noteMXSLSingleWin",UIWindowBase)
...
function UIXianJie_noteMXSLSingleWin:bindComponents()
...
function UIXianJie_noteMXSLSingleWin:unbindComponents()
...
function UIXianJie_noteMXSLSingleWin:onLoaded(...)
self:bindComponents()
...
function UIXianJie_noteMXSLSingleWin:__delete()
...
function UIXianJie_noteMXSLSingleWin:onShow(argtable,afterOnloaded)
...
function UIXianJie_noteMXSLSingleWin:onHide()
if xianjieModel:jude_xianjielog_reddot()~=xianjieModel:Get_reddotchangeflag()then
notifySystem:postNotify(notifyConfig.onXianjieLogReddotChange)
xianjieModel:Set_reddotchangeflag(not xianjieModel:Get_reddotchangeflag())
end
...
function UIXianJie_noteMXSLSingleWin:onStartAction(index,widget)

end

function UIXianJie_noteMXSLSingleWin:onFreshAction(index,widget)
self:refreshItem(widget,index)
end

function UIXianJie_noteMXSLSingleWin:splitStr(str)
return cjson.decode(str)
end


function UIXianJie_noteMXSLSingleWin:RefreshWin()
...
self.onekeyReddot:setActive(xianjieModel:get_MXSLSingleReddot())
self.onekeyBtn:setActive(xianjieModel:get_MXSLSingleReddot())
local log_numtb=xianjieModel:Get_lognum()or{}
local logNum5=log_numtb[5]or 0

local logNum6=0
local curNum=math.max(logNum5,logNum6)
self.tipsText:setText(string.format("奖励/记录日志保存上限：<color=%s>%d/100</color>",curNum<100 and"#549327"or"#c82c2c",curNum))
end


function UIXianJie_noteMXSLSingleWin:refreshItem(item,idx)
if item==nil then
return
end
local datatb=xianjieModel:Get_singleMXSLSingletb(idx)
...
function UIXianJie_noteMXSLSingleWin:Set_BigType(item,cfgid)
...
function UIXianJie_noteMXSLSingleWin:Set_Reward(item,len,list,recv,islose)
...
function UIXianJie_noteMXSLSingleWin:SetFightBtn(item,datatb,idx)
...
timetxt=_timetxt,
fightarry={reportId,{nil,reportId,eRePlayerType.xianjielog,huifang_txt,is_win,showBattle=true,eReplayType=eRePlayerType.xianjielog,isMXSL=true}},
rijbdata=_rijbdata,
}
...
if jianbaotxt then
local args={nil,reportId,eRePlayerType.xianjielog,huifang_txt,is_win,showBattle=true,extraCall=_fun,isMXSL=true}
...
local args={nil,reportId,eRePlayerType.xianjielog,huifang_txt,is_win,showBattle=true,eReplayType=eRePlayerType.xianjielog,isMXSL=true}
if fightLogType==1 then
fightController:send_log_list({reportId},args)
elseif fightLogType==2 then
fightController:send_log_list({reportId},args,true)
...
function UIXianJie_noteMXSLSingleWin:Set_Righttop(item,datatb)
...
function UIXianJie_noteMXSLSingleWin:showjijie(guid)
...
function UIXianJie_noteMXSLSingleWin:setRiZhiInfo(item,cfgid,datatb,str_cfg,handletype)
if cfgid>=3 then
...
function UIXianJie_noteMXSLSingleWin:SetStr(str_cfg,item,json_str,cfgid,handletype)
local str_2="暂无信息"
...
function UIXianJie_noteMXSLSingleWin:onClearBtn()
local tb=xianjieModel:Find_MXSLSingleGuid()
if next(tb)then
...
function UIXianJie_noteMXSLSingleWin:onOnekeyBtn()
local tb=xianjieModel:Find_MXSLSingleReward()
if next(tb)then
...
function UIXianJie_noteMXSLSingleWin:onTips()
