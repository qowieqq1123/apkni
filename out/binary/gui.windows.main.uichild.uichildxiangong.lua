...
UIChildXianGong=simple_class(UIChildObject)

local defaultIcon='button_zjmxiangong'
local _this

function UIChildXianGong:onLoaded()
reddotClassManager.register_event(REDDIT_TYPE.eXianGong,self.refreshReddot)
end

function UIChildXianGong:onShow(isInit)
if isInit then
_this=self
end
...
function UIChildXianGong:OnButtonClick()
...
function UIChildXianGong:OnYuanZhuButtonClick()
xianjieController:reqHelpAll()
end

function UIChildXianGong:release()
...
function UIChildXianGong.refreshReddot(class,sub_typo,last_flag,flag)
...
function UIChildXianGong:refreshReddotView(isreddot)
...
function UIChildXianGong:doPunchRotation(isreddot)
