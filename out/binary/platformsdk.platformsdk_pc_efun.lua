...
platformSDK_PC_Efun=simple_class(platformSDK_None)
...
function platformSDK_PC_Efun:__init(...)
...
function platformSDK_PC_Efun:reqLogin()
...
function platformSDK_PC_Efun:reqVerificationLogin(sdkParams)
...
function platformSDK_PC_Efun:reqLogout()
...
function platformSDK_PC_Efun:reqPay(id,count,params,subscribe)
...
function platformSDK_PC_Efun:eEfunTrackEvent(eventName)
...
function platformSDK_PC_Efun:getEfunBindState()
local data=platformHelper:getPlayerInfo()
local jsonStr=jsonHelper.encode(data)
...
function platformSDK_PC_Efun:getPhoneCaptcha(phoneNumber)
local data=platformHelper:getPlayerInfo()
...
function platformSDK_PC_Efun:reqPhoneBind(phoneNumber,captchaCode)
local data=platformHelper:getPlayerInfo()
data.json_phoneNumber=phoneNumber
...
function platformSDK_PC_Efun:onEfunBindInfo(json,jsonStr)
...
function platformSDK_PC_Efun:onPhoneCaptchaCallBack(json,jsonStr)
local info=json.jsonStr
...
function platformSDK_PC_Efun:onReqPhoneBindCallBack(json,jsonStr)
local info=json.jsonStr
...
function platformSDK_PC_Efun:reqReport(typo,info)
...
function platformSDK_PC_Efun:reqFangChenMi()

end


function platformSDK_PC_Efun:onPay(json,jsonStr)
...
if code=="3000"then
UIManager:showWindow('UIGooglePayWin',{winState=1,Code=code,Msg=info.msg})
else

UIManager:showWindow('UIGooglePayWin',{winState=2,Code=code,Msg=info.msg})
end
else
...
function platformSDK_PC_Efun:LoginCallBack(json,jsonStr)
LoginData=json
self:LoginHandle(json,jsonStr)
end


function platformSDK_PC_Efun:LoginHandle(json,jsonStr)
local info=json.jsonStr
...
platformSDK.printSDK('登入資訊__刷新 platformSDK_PC_Efun')
else
loginModel:onLogin(info,phpParams,"")
platformSDK.printSDK('登入__成功 platformSDK_PC_Efun',deviceHelper.getRuntimePlatformStr())
...
function platformSDK_PC_Efun:LogoutCallBack(json,jsonStr)
local info=json.jsonStr
local result=info.result
...
function platformSDK_PC_Efun:PurchasePaymentResult()

end


function platformSDK_PC_Efun:OnApplicationQuit()
local showdata=
{
type='UIDialouge',
title='溫馨提示',
...
function platformSDK_PC_Efun:reqApplicationQuit_PC()
_WindowsHelper.ExecCmd("setQuitState","quit")
platformHelper:exitGame()
end
...
function platformSDK_PC_Efun:reqMsgSecCheck(scene,content,callback,to_PrivatePlayerData)
if not this then
this=self
end
...
function platformSDK_PC_Efun.onreMsgSecCheck(args)
...
function platformSDK_PC_Efun:reqEfunCheckGoogleOAuth()
return _WindowsHelper._EfunCheckGoogleOAuth()
end



function platformSDK_PC_Efun:reqEfunGoogleOAuthBegin()
_WindowsHelper._EfunGoogleOAuthBegin()
end


function platformSDK_PC_Efun:reqEfunGoogleReOAuthBegin()
_WindowsHelper._EfunGoogleReOAuthBegin()
end



function platformSDK_PC_Efun:reqGoogleOAuthCompleteState()
_WindowsHelper._EfunGetGoogleOAuthCompleteState()
end


function platformSDK_PC_Efun:onGoogleOAuthCompleteStateCB(json,jsonStr)
local info=json.jsonStr
local code=info.code
...
function platformSDK_PC_Efun:EfunGoogleVerifyOrder()
_WindowsHelper._EfunGoogleVerifyOrder()
end


function platformSDK_PC_Efun:onEfunGoogleVerifyOrderCB(json,jsonStr)
local info=json.jsonStr
local code=info.code
