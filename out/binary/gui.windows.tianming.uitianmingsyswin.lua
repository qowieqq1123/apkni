...
def_class("UITianmingSysWin",UIWindowBase)
...
function UITianmingSysWin:bindComponents()

self.layout=UIObject.get(self,0)
self.TipsWin=UIObject.get(self,1)
self.dzListScrollView=UIObject.get(self,2)
self.BtnClose=UIButton.get(self,3)
self.BtnGoTo=UIButton.get(self,4)
self.tipsText=UIImage.get(self,5)
self.tipsRewardScroller=UIObject.get(self,6)
self.timeText=UIText.get(self,7)
self.libaoScroller=UIObject.get(self,8)
self.dzmodel=UIObject.get(self,9)
self.upTianMing=UIButton.get(self,10)
self.dzListPanel=UIObject.get(self,11)
self.btnLeft=UIButton.get(self,12)
self.btnRight=UIButton.get(self,13)
...
function UITianmingSysWin:unbindComponents()
...
function UITianmingSysWin:onLoaded(...)
self:bindComponents()
end


function UITianmingSysWin:__delete()
...
function UITianmingSysWin:onShow(argtable,afterOnloaded)
...
function UITianmingSysWin:onHide()
self.dzListScrollView:setActive(false)
self.libaoScroller:setActive(false)
end

function UITianmingSysWin:checkShowTipsWin()
...
function UITianmingSysWin:refresh(isInit,dzId,needJump)

self:refreshDiZiListPanel(isInit,needJump)
...
function UITianmingSysWin:findPageIndexByDzId(dzId)
if dzId then
...
function UITianmingSysWin:refreshPage()
...
function UITianmingSysWin:refreshJT()
local len=#self.sortShowItemList
self.btnLeft:setActive(self.selectPageIndex>1)
self.btnRight:setActive(self.selectPageIndex<len)
end

function UITianmingSysWin:refreshDiZiListPanel(isInit,needJump)
...
local isNeedJump=isInit or needJump or false
...
local scrollViewSize=count*158+15
local isEnable=true
...
function UITianmingSysWin:testJump(index)
self.dzListScrollView:setChildScrollViewSelectItem(index,false,false,true)
end

function UITianmingSysWin:selectPage(index)
...
function UITianmingSysWin:refreshDzModel()
...
local modelParams=UIDiscipleModel:getDiscipleInsideModelInfoByData_lihui(info,{notClothing=true})
...
function UITianmingSysWin:sortShowList(showList)
local sortList={}
...
function UITianmingSysWin:sortLibaoList()
local dzItemData=self.sortShowItemList[self.selectPageIndex]
...
function UITianmingSysWin:refreshLibaoList()
...
self.libaoScroller:setActive(true)
self.libaoScroller:setChildScrollViewCreateGrids(count,count)
...
local bgAbName="ui/windows/tianming/newtianming_atlas_pak.ab"
...
widget:SetChildCSImageSprite(libaoItemIndex.bg,bgAbName,"image_newtianming_5")
else

widget:SetChildActive(libaoItemIndex.skillPanel,false)
widget:SetChildCSImageSprite(libaoItemIndex.bg,bgAbName,"image_newtianming_4")
end
...
self:onLibaoFreeBtn_oneKey(dzId,libaoId)
end)
end

widget:SetChildActive(libaoItemIndex.freeBtnReddot,openLibao and not isGot)
widget:SetChildImageExGray(libaoItemIndex.freeBtn,not openLibao)
widget:SetChildImageExGray(libaoItemIndex.buyBtn,not openLibao)
...
widget:SetChildActive(libaoItemIndex.rebate,not isSellOut)
end
end
end


function UITianmingSysWin:setLiBaoDiziTianMingSkillItem(widget,dzData,openLibao,tmLevel)
local dzInfo=dzData.imageInfo
...
function UITianmingSysWin:setTipsReward(rewards)
...
function UITianmingSysWin:setRewardShow(rewards,widget,index)
if not rewards then
return
end
...
function UITianmingSysWin:onUpTianMing()
...
function UITianmingSysWin:onBtnGoTo()
self:goToNewGear()
end



function UITianmingSysWin:onBtnClose()
self:closeTips()
end



function UITianmingSysWin:onBtnLeft()
if self.selectPageIndex==1 then
return
end
self.selectPageIndex=self.selectPageIndex-1
...
function UITianmingSysWin:onBtnRight()
local len=#self.sortShowItemList
if self.selectPageIndex==len then
return
end
...
function UITianmingSysWin:getDiscipleData(diziId)
if not diziId then
return
end
...
function UITianmingSysWin:onClickRewardItem(itemId,index,guid,attach)
...
function UITianmingSysWin:onLibaoFreeBtn(dzId,libaoId)
local isGot=tianmingzengliModel:checkTMZLFreeLibaoIsGot(dzId,libaoId)
if isGot then
...
function UITianmingSysWin:onLibaoFreeBtn_oneKey(dzId,libaoId)
local isGot=tianmingzengliModel:checkTMZLFreeLibaoIsGot(dzId,libaoId)
if isGot then
...
local hasDz=dzData.discipleguid~=nil
local dzNowTmLv=UIDiscipleModel:getTianMingLevelEx(dzData)
local clickLibaoCfg=libaoCfgList[libaoId]
local clickLibaoTargetTmLv=clickLibaoCfg[1]
if not hasDz or dzNowTmLv<clickLibaoTargetTmLv then
local dzName=dzData.disciplename
local tmLv_str=UIDiscipleModel.getTianMingLevelDesc(clickLibaoTargetTmLv,1)
local tipsStr=FMT.fmt("弟子“{0}”{1}可领取",dzName,tmLv_str)
UIManager.error(tipsStr)
return
end
...
local canGetList={}
for id,libaoCfg in ipairs(libaoCfgList)do
local isGotFree=tianmingzengliModel:checkTMZLFreeLibaoIsGot(dzId,id)
local targetTianmingLv=libaoCfg[1]
local openLibao=hasDz and dzNowTmLv>=targetTianmingLv or false
if not isGotFree and openLibao then
canGetList[#canGetList+1]={dzId,id,1}
end
end
if next(canGetList)~=nil then
tianmingzengliController:reqTianMingZengLiGetFreeLibaoBatch(canGetList)
end
end


function UITianmingSysWin:onLibaoBuyBtn(dzId,libaoId)
