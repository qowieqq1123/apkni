...
def_class("UIXM_ZZSH_resourceSelectWin",UIWindowBase)
...
function UIXM_ZZSH_resourceSelectWin:bindComponents()
...
function UIXM_ZZSH_resourceSelectWin:unbindComponents()
...
function UIXM_ZZSH_resourceSelectWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UIXM_ZZSH_resourceSelectWin:getMapView()
return self.mapView
end


function UIXM_ZZSH_resourceSelectWin:__delete()
_this=nil
...
function UIXM_ZZSH_resourceSelectWin:onHide()
...
function UIXM_ZZSH_resourceSelectWin:onShow(argtable,afterOnloaded)
self.defaultMainIndex=1
if self.qbguid~=nil then
if not UIManager:invokeUIMethod('UIXM_ZZSH_resourceInfoWin','containQBGuid',self.qbguid)then
self.qbguid=nil
end
end

self.needtime={}
local names={'宝地等阶','出击路程'}
...
function UIXM_ZZSH_resourceSelectWin:playEnterAnim()
...
function UIXM_ZZSH_resourceSelectWin:playLeaveAnim()
...
function UIXM_ZZSH_resourceSelectWin:refreshView()
self.qbguid=nil
...
function UIXM_ZZSH_resourceSelectWin:onDropdownChange(idx)
...
function UIXM_ZZSH_resourceSelectWin:onSortConditionClick()
if self.closeLock then return end
local args={}
args.infotype=zhengzhanshanhaiModel.qbType.eResource
...
function UIXM_ZZSH_resourceSelectWin.selecConditionBack(sortCondition)
if _this==nil then
return
end
...
function UIXM_ZZSH_resourceSelectWin:onSortOrderClick()
if self.closeLock then return end
self.sortOrder=not self.sortOrder
self:refreshInfo(true)
end

function UIXM_ZZSH_resourceSelectWin:getQBData(qbData)
local d={}
...
function UIXM_ZZSH_resourceSelectWin:sortList()
local list={}
...
function UIXM_ZZSH_resourceSelectWin:initInfo(isInit)
local temp=UIManager:invokeUIMethod('UIXM_ZZSH_entitySelectWin','getQingBaoList',zhengzhanshanhaiModel.qbType.eResource,isInit)
...
if lp[1]==nil then lp[1]={typo=1,qblist={},name='专属宝地'}end
...
if lp[2]==nil then lp[2]={typo=2,qblist={},name='无主宝地'}end
...
function UIXM_ZZSH_resourceSelectWin:refreshInfo(click)
...
function UIXM_ZZSH_resourceSelectWin:getQBIndex(guid)
...
function UIXM_ZZSH_resourceSelectWin:refreshMainItemSelect(mainItem,mainIndex,flag)
...
function UIXM_ZZSH_resourceSelectWin:refreshMainItemReddot(mainItem,mainIndex)
if mainItem==nil then
mainItem=self.itemScrollView:getMainItem(mainIndex-1)
end
if mainItem then
...
function UIXM_ZZSH_resourceSelectWin:refreshSubItemSelect(subItem,mainIndex,subIndex,flag)
if subItem==nil then
subItem=self.itemScrollView:getSubItem(mainIndex-1,subIndex-1)
end
if subItem then
subItem:SetChildActive(4,flag)
end
end

function UIXM_ZZSH_resourceSelectWin:refreshSubItemReddot(subItem,mainIndex,subIndex)
if subItem==nil then
subItem=self.itemScrollView:getSubItem(mainIndex-1,subIndex-1)
end
if subItem then
...
subItem:SetChildActive(8,isReddot)
end
end

function UIXM_ZZSH_resourceSelectWin:mainClickAction(mainItem)
...
function UIXM_ZZSH_resourceSelectWin:subClickAction(subItem)
if self.closeLock then return end
...
function UIXM_ZZSH_resourceSelectWin:mainCreateAction(mainItem)
local index=mainItem.Index+1
...
function UIXM_ZZSH_resourceSelectWin:subCreateAction(subItem)
...
subItem:SetChildActive(0,showModel)
if showModel then
local modelParams=comHelper.getMonsterModelParamsEx(cfg.model)
local size=cfg.modelSet[5]or cfg.modelSet[1]
subItem:SetChildUIModelShowTarget(0,modelParams.body,size,modelParams.componets,eAnimationID.stand)
end
local showModelIcon=cfg.modelIcon~=nil
subItem:SetChildActive(7,showModelIcon)
if showModelIcon then
subItem:SetChildCSImageSprite(7,globalABLookup.zzshentityicons,cfg.modelIcon)
end


subItem:SetChildCSImageIcon(1,moneyModel.getIconNameEx(cfg.moneytype),true)

local nameStr=qbData:getName()



subItem:SetChildText(2,nameStr)

local time_str=timeHelper.format_time_stamp3(d.costTime)
subItem:SetChildText(3,time_str)
...
subItem:SetChildActive(9,not(stage<=gather[2]))
subItem:SetChildActive(10,stage<=gather[2])
subItem:SetChildButtonClick(9,function()
self:showTips1(subItem)
end)

subItem:SetChildButtonClick(10,function()
self:showTips2(subItem)
end)
...
function UIXM_ZZSH_resourceSelectWin:onExpandAction(index)

end



function UIXM_ZZSH_resourceSelectWin:onJumpBtn()
UIManager:invokeUIMethod('UIXM_ZZSH_entitySelectWin','onCloseBtn')
UIManager:invokeUIMethod('UIXM_ZZSH_MapWin','openWorldMap')
end


function UIXM_ZZSH_resourceSelectWin:showTips1(item)
local args={}
args.desclist={}
local num=10
...
args.title1="争斗宝地"
local screenPoint=item:GetChildScreenPointToLocalPointRectangle(10)
...
function UIXM_ZZSH_resourceSelectWin:showTips2(item)
local args={}
args.desclist={}
local num=10
...
function UIXM_ZZSH_resourceSelectWin:SetNeedTime(subItem)
self.needtime[#self.needtime+1]=subItem

end

function UIXM_ZZSH_resourceSelectWin:setSubTimer()
local func
func=function()
local lerp=self:judetime()
if lerp>0 then
...
function UIXM_ZZSH_resourceSelectWin:refreshMyQBTime(subItem,lerp)
...
function UIXM_ZZSH_resourceSelectWin:judetime()
