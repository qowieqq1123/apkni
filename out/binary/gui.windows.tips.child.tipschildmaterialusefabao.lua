







def_class("tipsChildMaterialUseFabao",UICloneObject)





tipsChildMaterialUseFabao.abName="ui/windows/tips/child/tipschildmaterialusefabao.ab"

tipsChildMaterialUseFabao.assetName="tipsChildMaterialUseFabao"


function tipsChildMaterialUseFabao:bindComponents()

self.attrRoot=UIObject.get(self,0)
self.elementRoot=UIObject.get(self,1)
self.weiliRoot=UIObject.get(self,2)
self.attrTitle=UIText.get(self,3)
self.attrDesc=UIText.get(self,4)
self.elementTitle=UIText.get(self,5)
self.elementDesc=UIText.get(self,6)
self.weiliTitle=UIText.get(self,7)
self.weiliDesc=UIText.get(self,8)
self.line=UIObject.get(self,9)
self.topline=UIObject.get(self,10)
self.showLianZhiBtn=UIButton.get(self,11)
self.showLianHuaBtn=UIButton.get(self,12)
self.btnPanel=UIObject.get(self,13)

self.showLianZhiBtn:setButtonClick(function()self:onShowLianZhiBtn()end)

self.showLianHuaBtn:setButtonClick(function()self:onShowLianHuaBtn()end)

end


function tipsChildMaterialUseFabao:unbindComponents()
local _UIObject_release=UIObject.release
_UIObject_release(self.attrRoot);self.attrRoot=nil;
_UIObject_release(self.elementRoot);self.elementRoot=nil;
_UIObject_release(self.weiliRoot);self.weiliRoot=nil;
_UIObject_release(self.attrTitle);self.attrTitle=nil;
_UIObject_release(self.attrDesc);self.attrDesc=nil;
_UIObject_release(self.elementTitle);self.elementTitle=nil;
_UIObject_release(self.elementDesc);self.elementDesc=nil;
_UIObject_release(self.weiliTitle);self.weiliTitle=nil;
_UIObject_release(self.weiliDesc);self.weiliDesc=nil;
_UIObject_release(self.line);self.line=nil;
_UIObject_release(self.topline);self.topline=nil;
_UIObject_release(self.showLianZhiBtn);self.showLianZhiBtn=nil;
_UIObject_release(self.showLianHuaBtn);self.showLianHuaBtn=nil;
_UIObject_release(self.btnPanel);self.btnPanel=nil;
end








function tipsChildMaterialUseFabao:onLoaded(...)
self:bindComponents()
end

function tipsChildMaterialUseFabao:__delete()
self:unbindComponents()
end

function tipsChildMaterialUseFabao:onShow(args)
local data=args.argtable
self.formType=data.formType
local childType=args.childType
local nodeidx=args.nodeidx
self.itemid=data.itemid
local itemguid=data.itemguid
local attach=data.attach
if attach.MaterialUseFabaotopLine==false then
self.topline:setActive(false)
end

if not fabaoHelper.isCanUseToMakeFabao(self.itemid)then
self:recycleSelf()
return
end
local isFuluSelect=self.formType==TIPS_FORM_TYPE.eFuluSelect or self.formType==TIPS_FORM_TYPE.eFuluMake
if isFuluSelect then
self:recycleSelf()
return
end

local showBtn=false

if self.formType~=TIPS_FORM_TYPE.eLianqiGeBagItem and
self.formType~=TIPS_FORM_TYPE.eLianqiGeItem and
self.formType~=TIPS_FORM_TYPE.eFabaoJilian and
self.formType~=TIPS_FORM_TYPE.eFabaoLianhua then


showBtn=true
end

self.diziguid=attach.diziguid

local itemConfig=itemsConfig.getConfig(self.itemid)
local fabaoAttrs=itemConfig.static
local power=itemConfig.power
local lianhua=itemConfig.lianhua
local isLianhua=self.formType==TIPS_FORM_TYPE.eFabaoLianhua
local hasContent=false
if fabaoAttrs and not isLianhua then
hasContent=true
end
if power and not isLianhua then
hasContent=true
end
if lianhua then
hasContent=true
end

if not hasContent then
self:recycleSelf()
return
end

if not showBtn then

self:showBaseAttr()


self:showLianZhiAttr()


self:showLianHuaAttr()
else
self.attrRoot:setActive(false)
self.elementRoot:setActive(false)
self.weiliRoot:setActive(false)
end
self.btnPanel:setActive(showBtn)


local parentsMove=data.move
if not parentsMove then

parentsMove=TIPS_MOVE_POS.eRight
end


if parentsMove==TIPS_MOVE_POS.eLeft then

self.move=TIPS_MOVE_POS.eRight
elseif parentsMove==TIPS_MOVE_POS.eDefault or parentsMove==TIPS_MOVE_POS.eRight or parentsMove==TIPS_MOVE_POS.eRightTwo then

self.move=TIPS_MOVE_POS.eLeft
elseif parentsMove==TIPS_MOVE_POS.eCenter then

self.move=TIPS_MOVE_POS.eRightTwo
end
end

function tipsChildMaterialUseFabao:onHide()

end

function tipsChildMaterialUseFabao:showBaseAttr()
local itemid=self.itemid
local itemConfig=itemsConfig.getConfig(itemid)

local isLianhua=self.formType==TIPS_FORM_TYPE.eFabaoLianhua
local isEquip=itemsConfig.isEquip(itemid)
local fabaoRangeAttrs=fabaoHelper.getBaseAttrsRange(itemid)
if fabaoRangeAttrs and not isLianhua then
local title=FMT.cfmt(FONT_COLOR.eTitle2Color,'作为法宝主材料时，法宝基础属性')
local attrStr=''

for i,v in ipairs(fabaoRangeAttrs)do
local attrId=v[1]
local min=v[2][1]
local max=v[2][2]
local attrname,minstr=equipsHelper.getAttr(attrId,min)
local attrname,maxstr=equipsHelper.getAttr(attrId,max)
local attrTxt=FMT.cfmt(FONT_COLOR.eTipWhiteColor,'{0}：{1}~{2}',attrname,minstr,maxstr)
attrStr=i~=1 and FMT.fmt('{0}\n{1}',attrStr,attrTxt)or attrTxt
end
self.attrTitle:setText(title)
self.attrDesc:setText(attrStr)
self.attrRoot:setActive(true)
else
self.attrRoot:setActive(false)
end
end

function tipsChildMaterialUseFabao:showLianZhiAttr()
local itemConfig=itemsConfig.getConfig(self.itemid)

local power=itemConfig.power
local element=itemConfig.element

local isLianhua=self.formType==TIPS_FORM_TYPE.eFabaoLianhua
local isEquip=itemsConfig.isEquip(self.itemid)

if power and not isLianhua then
local title=FMT.cfmt(FONT_COLOR.eTitle2Color,'法宝炼制时，可提升五行威力')
local attrTxt
if not isEquip then
local min=mathHelper.decimal(power[1][1]/100)
local max=mathHelper.decimal(power[#power][2]/100)
attrTxt=element and FMT.cfmt(FONT_COLOR.eTipWhiteColor,'{0}系技能威力增加{1}%~{2}%',ELEMENT_TYPE.getName(element),min,max)
or FMT.cfmt(FONT_COLOR.eTipWhiteColor,'技能威力增加{0}%~{1}%',min,max)
else
local stagePower=power[#power]
local min=mathHelper.decimal(power[1][1][1]/100)
local max=mathHelper.decimal(stagePower[#stagePower][2]/100)
attrTxt=element and FMT.cfmt(FONT_COLOR.eTipWhiteColor,'{0}系技能威力增加{1}%~{2}%',ELEMENT_TYPE.getName(element),min,max)
or FMT.cfmt(FONT_COLOR.eTipWhiteColor,'技能威力增加{0}%~{1}%',min,max)
end
self.elementTitle:setText(title)
self.elementDesc:setText(attrTxt)
self.elementRoot:setActive(true)
else
self.elementRoot:setActive(false)
end
end

function tipsChildMaterialUseFabao:showLianHuaAttr()
local itemConfig=itemsConfig.getConfig(self.itemid)
local lianhua=itemConfig.lianhua
if lianhua then
local title=FMT.cfmt(FONT_COLOR.eTitle2Color,'法宝炼化时，可提升炼化属性')
local attrStr=''
for i,v in ipairs(lianhua)do
local attrId=v[1]
local range=v[2]
local step=v[3]
local eroFormat=type(range[1])=='number'
local attrname,minstr=equipsHelper.getAttr(attrId,eroFormat and range[1]*step or range[1][1]*step)
local attrname,maxstr=equipsHelper.getAttr(attrId,eroFormat and range[2]*step or range[#range][2]*step)
local attrTxt=FMT.cfmt(FONT_COLOR.eTipWhiteColor,'{0}：{1}~{2}',attrname,minstr,maxstr)
attrStr=i~=1 and FMT.fmt('{0}\n{1}',attrStr,attrTxt)or attrTxt
end
self.weiliTitle:setText(title)
self.weiliDesc:setText(attrStr)
self.weiliRoot:setActive(true)
else
self.weiliRoot:setActive(false)
end
end






















function tipsChildMaterialUseFabao:onShowLianZhiBtn()
UIManager:showWindow("UIFabaoMaterialTipsDescWin",{itemid=self.itemid,showType=1,move=self.move})
end

function tipsChildMaterialUseFabao:onShowLianHuaBtn()
UIManager:showWindow("UIFabaoMaterialTipsDescWin",{itemid=self.itemid,showType=2,move=self.move})
end