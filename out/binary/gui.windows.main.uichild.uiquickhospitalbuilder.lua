...
UIQuickHospitalBuilder=simple_class(UIChildObject)

local iconname='button_zjmyulinzai'
local _this
function UIQuickHospitalBuilder:onLoaded()

_this=self
end

function UIQuickHospitalBuilder:onShow()
...
if bdData then
UIFullYuLingZhaiControl:showMainWindow({bdData=bdData})
end
end,true)
...
function UIQuickHospitalBuilder:release()
...
function UIQuickHospitalBuilder.refreshReddot(class,sub_typo,last_flag,flag)
_this:setChildActive(2,flag)
_this:doPunchRotation(flag)
end

function UIQuickHospitalBuilder:doPunchRotation(reddot)
