...
def_class("UIPushGiftBuyWin",UIWindowBase)
...
function UIPushGiftBuyWin:bindComponents()
...
function UIPushGiftBuyWin:unbindComponents()
...
function UIPushGiftBuyWin:onLoaded(...)
self:bindComponents()
...
function UIPushGiftBuyWin:__delete()
self:unbindComponents()
end

function UIPushGiftBuyWin:onShow(argtable,afterOnloaded)
argtable=argtable or{}
local idlist=pushGiftTwoModel:getGiftIds()
...
function UIPushGiftBuyWin:onHide()

end



function UIPushGiftBuyWin:sortList(list)
if#list<=1 then return list end
local nlist={}
local tlist={}
for i,v in ipairs(list)do
if not pushGiftTwoConfig.isForver(v)then
tlist[#tlist+1]=v
else
nlist[#nlist+1]=v
end
...
function UIPushGiftBuyWin:onSelect(giftId)
...
pushGiftTwoModel:removeNewGift(giftId)
self.giftId=giftId
self.isSetZero=nil
self:stopAllTimer()
self:freshGiftPanel()
...
function UIPushGiftBuyWin:onTimeCount(giftId)
local doingData=pushGiftTwoModel:getDoingData(self.giftId)
...
function UIPushGiftBuyWin:onShowArgRecv()

end

function UIPushGiftBuyWin:freshGiftPanel()
local cfg=pushGiftTwoConfig.getConfig(self.giftId)
local len=#cfg.times
...
function UIPushGiftBuyWin:initIdxData(len)
self.giftIdxList={}
self.giftIdxLook={}
self.giftItemLook={}
for i=1,len do
...
local sortTag={}
for i=1,len do
local leftTimes=pushGiftTwoModel:getIdxLeftBuyTimes(self.giftId,i)
...
function UIPushGiftBuyWin:freshItemList()
...
function UIPushGiftBuyWin:bindGrid(index,widget)
self:freshGiftInfo(widget,self.giftId,index)
end

function UIPushGiftBuyWin:freshGiftInfo(widget,id,index)
...
local maxTimes=pushGiftTwoConfig.getBuyTimesByCfg(id,idx)
local leftTimes=pushGiftTwoModel:getIdxLeftBuyTimes(id,idx)
...
if isOption then
itemInfo=nil
local len=#pushGiftTwoConfig:getOptionCfg(id,idx,i)
if len>1 then
local selectIdx=pushGiftTwoModel:getSelectGiftItemIdx(id,idx,i)or nil

if selectIdx and pushGiftTwoModel:isOptionItemUnlock(id,idx,i,selectIdx)then
itemInfo=pushGiftTwoConfig:getOptionItemCfg(id,idx,i,selectIdx)[1]
...
itemInfo=pushGiftTwoConfig:getOptionItemCfg(id,idx,i,1)[1]
isOption=false
...
local fixTime=pushGiftTwoConfig.getOpenTime(id)
...
function UIPushGiftBuyWin:freshGiftItems(widget,id,index)
local idx=self.giftIdxLook[index]
local cfg=pushGiftTwoConfig.getConfig(id)
local rewardsCfg=cfg.reward
if cfg.clientreward then
...
local optionRewardsCfg=cfg.rewards
local rewards=rewardsCfg and rewardsCfg[idx]or{}
local optionRewards=optionRewardsCfg and optionRewardsCfg[idx]or nil
...
function UIPushGiftBuyWin:freshGiftBuyInfo(id,idx)
...
function UIPushGiftBuyWin:bindBtnGrid(index,widget)
local id=self.idList[index]
local cfg=pushGiftTwoConfig.getConfig(id)
local isSelect=id==self.giftId
local isNew=pushGiftTwoModel:isNewGift(id)
local isForver=pushGiftTwoConfig.isForver(id)
...
function UIPushGiftBuyWin:freshBtnsPanel()
...
function UIPushGiftBuyWin:onfreshBtnsPanel()
local len=#self.idList
for i=1,len do
...
function UIPushGiftBuyWin:onfreshBtnInfo(idx)
local widget=self.btnScrollView:getGridObjectByindex(idx-1)
if widget==nil then return end
self:bindBtnGrid(idx,widget)
end

function UIPushGiftBuyWin:onBuySuccess(id,idx)
...
function UIPushGiftBuyWin:showNextGift()
...
function UIPushGiftBuyWin:onBtnBuy(id,idx)
local leftTimes=pushGiftTwoModel:getIdxLeftBuyTimes(id,idx)
if leftTimes<=0 then
...
if not pushGiftTwoModel:isFillAllOption(id,idx,lookup,defaultlookup)then
...
pushGiftTwoController.buyGift(id,idx,lookup,defaultlookup)
end
...
function UIPushGiftBuyWin.onItemClick(itemid,index,itemguid,attach)
...
function UIPushGiftBuyWin:findGiftIdx()
...
function UIPushGiftBuyWin:onScrollChange()
...
function UIPushGiftBuyWin:onSelectOption(selectLookup,args)
...
pushGiftTwoModel:setSelectGiftItemIdx(giftId,lvIdx,hoidIdx,index,false)
end
pushGiftTwoModel:flushSelectGiftItem()
self:freshItemList()
end
end

function UIPushGiftBuyWin:showOptionSelect(giftId,lvIdx,shoidIdx)
...
local selectIdx=pushGiftTwoModel:getSelectGiftItemIdx(giftId,lvIdx,hoidIdx)
local len=#pushGiftTwoConfig:getOptionCfg(giftId,lvIdx,hoidIdx)
if len>1 then
holeList[#holeList+1]=hoidIdx
if itemList[hoidIdx]==nil then itemList[hoidIdx]={}end
local itemholeList=itemList[hoidIdx]
for i,vv in ipairs(v)do
if pushGiftTwoModel:isOptionItemUnlock(giftId,lvIdx,hoidIdx,i)then
