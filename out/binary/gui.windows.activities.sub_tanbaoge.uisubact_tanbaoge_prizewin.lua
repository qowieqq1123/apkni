...
def_class("UISubAct_tanbaoge_prizeWin",UIWindowBase)
...
function UISubAct_tanbaoge_prizeWin:bindComponents()
...
self.drawBtnPanel=UIObject.get(self,16)
...
function UISubAct_tanbaoge_prizeWin:unbindComponents()
...
function UISubAct_tanbaoge_prizeWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UISubAct_tanbaoge_prizeWin:__delete()
self:clearTimer()
...
function UISubAct_tanbaoge_prizeWin:onShow(argtable,afterOnloaded)
if argtable then
...
function UISubAct_tanbaoge_prizeWin:onHide()
self:clearTimer()
end

function UISubAct_tanbaoge_prizeWin:refreshBtn()
self.drawBtnPanel:setActive(self.showDrawBtn)
if self.showDrawBtn then
local costCfg=self.config.useItem
...
local useFreeNum=self.activityData.data.freeNum
...
self.drawBtnManyText:setText(FMT.fmt("探宝{0}次",manyDrawCount))
...
function UISubAct_tanbaoge_prizeWin:showSpecialEffect()
...
function UISubAct_tanbaoge_prizeWin:onDrawBtnOne()
_this:clearTimer()
self.btnClickMask:setActive(true)
UIManager:invokeUIMethod("UISubAct_tanbaogeWin","onTanbaoBtnOne")
self:onClickBg()
end



function UISubAct_tanbaoge_prizeWin:onDrawBtnMany()
_this:clearTimer()
self.btnClickMask:setActive(true)
UIManager:invokeUIMethod("UISubAct_tanbaogeWin","onTanbaoBtnTen")
self:onClickBg()
end

function UISubAct_tanbaoge_prizeWin:onClickBg()
_this:clearTimer()
self.btnClickMask:setActive(true)
if self.callback then
self.callback()
end
UIManager:invokeUIMethod("UISubAct_tanbaogeWin","checkTowerMove")
self:closeSelf()
end

function UISubAct_tanbaoge_prizeWin:clearTimer()
