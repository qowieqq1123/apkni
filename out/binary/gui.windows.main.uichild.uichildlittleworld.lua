...
UIChildLittleWorld=simple_class(UIChildObject)

local defaultIcon='button_xiaoshijie'
local _this

function UIChildLittleWorld:onLoaded()

end

function UIChildLittleWorld:onShow(isInit)
if isInit then
_this=self
end
...
function UIChildLittleWorld:OnButtonClick()
zongmenControl:jumpBuildingWin({type=SLG_SYSTEM_TYPE.eLittleWorld,isOpenRepairWin=true})

UIManager:invokeUIMethod("UIMainSubEnterPanelWin","onMask")
end

function UIChildLittleWorld:OnYuanZhuButtonClick()
xianjieController:reqHelpAll()
end

function UIChildLittleWorld:release()
...
function UIChildLittleWorld.refreshReddot(class,sub_typo,last_flag,flag)
...
function UIChildLittleWorld:refreshReddotView(isreddot)
...
function UIChildLittleWorld:doPunchRotation(isreddot)
