...
def_class("UIHuanJingGuaJiLoseWin",UIWindowBase)
...
function UIHuanJingGuaJiLoseWin:bindComponents()
...
function UIHuanJingGuaJiLoseWin:unbindComponents()
...
function UIHuanJingGuaJiLoseWin:onLoaded(...)
self:bindComponents()
end


function UIHuanJingGuaJiLoseWin:__delete()
...
function UIHuanJingGuaJiLoseWin:onShow(argtable,afterOnloaded)
...
function UIHuanJingGuaJiLoseWin:showGuajiReward()

local curLayer=UIHuanJingControl:getCurrentLevel()

local guajiLayer=UIHuanJingControl:getGuaJILayer()
...
local cha=UIHuanJingControl:getChapterLevel(curLayer)
local cha2=UIHuanJingControl:getChapterLevel(guajiLayer)

local cfg=cfgHelper.get1(cfg_guanqianewconfig_get,curLayer)
local next_id=cfg.next_id

self.layer:setText(FMT.fmt("本次挑战后山试炼：{0}-{1}",cha2[1],cha2[2]))
if next_id or not isClear then
self.nextLayer:setText(FMT.fmt("{0}-{1}",cha[1],cha[2]))
else
self.nextLayer:setText(FMT.fmt("{0}-{1}",cha[1],cha[2]))
end

local rewardList=UIHuanJingControl:getGuaJIReward()
UIHuanJingControl:clearGuaJIReward()
UIHuanJingControl:setGuaJILayer(nil)


local showReward=(guajiLayer~=curLayer or(not next_id))and next(rewardList)
local list={}
local lookUp={}
if showReward then
...
function UIHuanJingGuaJiLoseWin:onHide()

end

function UIHuanJingGuaJiLoseWin:onCloseTips()
...
function UIHuanJingGuaJiLoseWin:onQuitButton()
local cb=self.quitCallBack
self:closeSelf()
if cb then cb()end
end

function UIHuanJingGuaJiLoseWin:onContinueButton()
local cb=self.continuCallBack
self:closeSelf()
if cb then cb()end
end

function UIHuanJingGuaJiLoseWin:onFightCountBtn()
if self.isWait then
return
end
...
function UIHuanJingGuaJiLoseWin:stopCloseTimer()
