worldTripMoveList_RunTour=simple_class(worldTripMoveList)
worldTripMoveList_RunTour.name="worldTripMoveList_RunTour"

worldTripMoveList_RunTour.MoveType={
Move=6,
Animation=1,
Effect=2,
Bubble=3,
...
function worldTripMoveList_RunTour:__init(tour,job,objects)
...
function worldTripMoveList_RunTour:start(fast)
self.cPoint=0
self:startAction()
end

function worldTripMoveList_RunTour:onStep(pass)
self:quit()
...
function worldTripMoveList_RunTour:setMove(moveType,move)
...
function worldTripMoveList_RunTour:getMove(moveType)
return self.list[moveType]
end

function worldTripMoveList_RunTour:extractAIIndex()
...
function worldTripMoveList_RunTour:extractPointIndex()
...
function worldTripMoveList_RunTour:refreshTarget(iPos)
...
function worldTripMoveList_RunTour:startAction()
...
self.path=worldDispatchFactory:getPathCorners(eWorldTripType.Run,nVec2,cVec2,self.tourCfg.id)
...
function worldTripMoveList_RunTour:startMove()

self.action=0
self:setProgress(self.MoveType.Move)
local runMove=self.list[self.MoveType.Move]
runMove.corners=self.path
runMove.duration=worldDispatchFactory:getDuration(eWorldTripType.Run,self.path)
runMove:start(0)
end

function worldTripMoveList_RunTour:startPrev(index)
self.action=index
local aiInfo=self.aiCfg[self.aiIdx]
...
function worldTripMoveList_RunTour:startPost(index)
self.action=index
local aiInfo=self.aiCfg[self.aiIdx]
...
function worldTripMoveList_RunTour:startData(actionData,direction)
