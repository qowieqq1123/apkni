...
def_class("UITopMoneyWin8",UIWindowBase)
...
function UITopMoneyWin8:bindComponents()
...
function UITopMoneyWin8:unbindComponents()
...
function UITopMoneyWin8:onLoaded(...)
self:bindComponents()
self.money={}
...
function UITopMoneyWin8:__delete()
...
function UITopMoneyWin8:onShow(argtable,afterOnloaded)

if argtable then
self.money=argtable.moneys or argtable
self.isDaoYanWin=argtable.isDaoYanWin
...
function UITopMoneyWin8:onHide()
self:clearFlyIcon()
end

function UITopMoneyWin8:clearFlyIcon()
...
function UITopMoneyWin8:getMoneyList()
return self.money
end

function UITopMoneyWin8:freshMoneyList()
...
function UITopMoneyWin8:fillData(index,moneyInfo)
...
if moneyInfo.max then
moneyStr=FMT.fmt("{0}/{1}",moneyStr,moneyInfo.max)
end
...
function UITopMoneyWin8:freshMoneyValue(index,moneyInfo,lastVal)
local rootStr=FMT.fmt('moneyRoot{0}',index)
if moneyInfo==nil then
self[rootStr]:setActive(false)
return
...
function UITopMoneyWin8:clearFMTweener(mtype)
...
function UITopMoneyWin8:onAddClick(moneyType)
...
function UITopMoneyWin8:getMoneyRoot(moneyType)
...
function UITopMoneyWin8:flyMoneyIcon(spos,moneyType)
...
function UITopMoneyWin8:handleFly(moneyType,spos,tpos)
...
function UITopMoneyWin8:onMoneyChange(moneyType,lastVal,val)
local moneyList=self.money
...
function UITopMoneyWin8:clickMoney(idx)
local moneyInfo=self.money[idx]
if moneyInfo==nil then return end
local moneyType=moneyInfo[1]

if moneyInfo.addCallBack then
moneyInfo.addCallBack(moneyType)
else
if moneyType==eMoneyType.mtXianYu then
UIFullRechargeController:showRechargeWindow()
else
if self.isDaoYanWin then
gainControl:showGainWin_daoyan(moneyType)
else
...
function UITopMoneyWin8:onMoney1Btn()
self:clickMoney(1)
end



function UITopMoneyWin8:onMoney2Btn()
self:clickMoney(2)
end



function UITopMoneyWin8:onMoney3Btn()
self:clickMoney(3)
end



function UITopMoneyWin8:onMoney4Btn()
self:clickMoney(4)
end



function UITopMoneyWin8:onItemChange(changeType,itemguid,itemid,lastcount,itemcount)
local moneyList=self.money
for i,v in ipairs(moneyList)do
local moneyInfo=moneyList[i]
