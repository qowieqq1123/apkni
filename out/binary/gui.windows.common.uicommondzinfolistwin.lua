...
def_class("UICommonDZInfoListWin",UIWindowBase)
...
function UICommonDZInfoListWin:bindComponents()
...
self.modelEx=UIObject.get(self,37)
...
function UICommonDZInfoListWin:unbindComponents()
...
_UIObject_release(self.modelEx);self.modelEx=nil;
self.model=nil;
self.gfSlot=nil;
...
function UICommonDZInfoListWin:onLoaded(...)
_this=self
self:bindComponents()
self.speakTweens={}
...
function UICommonDZInfoListWin:__delete()
...
function UICommonDZInfoListWin:onShow(argtable,afterOnloaded)
if argtable then
...
self.teamMaxDiscipleCount=#teamList
...
table.insert(disciplesList[teamIndex],{index=index,data=data})
end
end
...
if self.subWinName then
self:showWindow(self.subWinName,{parentWin='UICommonDZInfoListWin',data=subWinArgs,lookType=self.lookType})
end
end
end


function UICommonDZInfoListWin:onHide()
...
function UICommonDZInfoListWin:refresh()
...
function UICommonDZInfoListWin:freshDisciplesList()
...
function UICommonDZInfoListWin:freshOther()
...
if actorInfo.nameEx then
self.name:setActive(false)
self.name2:setActive(true)
self.name2:setText(actorInfo.nameEx)
end
end
else
...
function UICommonDZInfoListWin.onClickRoleItem(index)
...
function UICommonDZInfoListWin:refreshModelSelect(flag)
self:freshModels()
...
local widget=self.modelEx:getChildWidgetBase(-1)
...
function UICommonDZInfoListWin:killSpeakTween(index)
...
function UICommonDZInfoListWin:killAllSpeakTween()
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList or{}
...
function UICommonDZInfoListWin:freshModels()
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList or{}
for _,model in ipairs(self.model)do
model:setActive(false)
end
self.modelEx:setActive(true)
...
local widget=self.modelEx:getChildWidgetBase(-1)
local v=disciplesList[self.selectRoleIdx]
local dzData=v.data
local baseData=dzData.base
...
function UICommonDZInfoListWin:freshInfo()
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList or{}
...
function UICommonDZInfoListWin:freshEquip()
...
function UICommonDZInfoListWin:fillItem(equip,equipSlotIdx)
local prop={}
...
function UICommonDZInfoListWin:fillLingShouItem(lsData,equipSlotIdx)
local prop={}
local widget=self.equipListWidget:GetChildWidgetBase(equipSlotIdx)
if lsData then
...
function UICommonDZInfoListWin:freshGFSkillSlot()
for i=1,2 do
self:freshGFSkillSlotEx(i)
end
end

function UICommonDZInfoListWin:freshGFSkillSlotEx(idx)
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList
local dzData=disciplesList[self.selectRoleIdx].data
local gflist={}
local baseData=dzData.base
...
function UICommonDZInfoListWin:freshFaBaoSkill()
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList
local dzData=disciplesList[self.selectRoleIdx].data
...
function UICommonDZInfoListWin:freshPolygonAtrrPanel()
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList
local dzData=disciplesList[self.selectRoleIdx].data
local netData=dzData.base
...
function UICommonDZInfoListWin:freshVocSkillSlot()
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList
local dzData=disciplesList[self.selectRoleIdx].data
local netData=dzData.base
local image=UIDiscipleModel.calculationDiscipleImageBase(netData)
...
function UICommonDZInfoListWin:freshVocSkillSlotEx(item,data)
...
function UICommonDZInfoListWin:onVocSkillItemClick(skillID,skillLv)
...
function UICommonDZInfoListWin:freshLayout()
...
function UICommonDZInfoListWin:onCloseBtn()
self:closeSelf()
end



function UICommonDZInfoListWin:onInfoBtn()
local disciplesList=self.teamList[self.selectTeamIndex].disciplesList
local dzData=disciplesList[self.selectRoleIdx].data
...
function UICommonDZInfoListWin:onGfSlot_1()
self:onGFSlotClick(1)
end



function UICommonDZInfoListWin:onGfSlot_2()
self:onGFSlotClick(2)
end

function UICommonDZInfoListWin:onStSlotFunc(skillID,skillLv)
local args={skillID=skillID,skillLv=skillLv,attend=eSkillTipsType.eDZSTSkill,dis_guid=nil,changLv=false}
UIManager:showWindow('UIDiscipleJobSkillTipsWin',args)
end

function UICommonDZInfoListWin:onStSlot()
if self.stSlotFunc then
self.stSlotFunc()
end
end

function UICommonDZInfoListWin:onMask()
self:closeSelf()
end

function UICommonDZInfoListWin:onBaseItemClick(id,equipType,guid,attach)
if id~=-1 then
...
function UICommonDZInfoListWin:onLingShouItemClick(id,equipType,guid,attach)
if id>0 then
...
function UICommonDZInfoListWin:onGFSlotClick(idx)
local gfID=self.usingGFList[idx]
local hasGF=gfID>0
if hasGF then
...
function UICommonDZInfoListWin:changeTitle(titleText)
self.title:setText(titleText)
end

function UICommonDZInfoListWin:selectTeam(teamIndex)
