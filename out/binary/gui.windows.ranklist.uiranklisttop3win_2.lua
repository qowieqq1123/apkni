...
def_class("UIRankListTop3Win_2",UIWindowBase)
...
function UIRankListTop3Win_2:bindComponents()
...
function UIRankListTop3Win_2:unbindComponents()
...
function UIRankListTop3Win_2:onLoaded(...)
self:bindComponents()
_this=self
...
function UIRankListTop3Win_2:__delete()
...
function UIRankListTop3Win_2:onShow(argtable,afterOnloaded)
...
function UIRankListTop3Win_2:onHide()

end



function UIRankListTop3Win_2:onClickHelpBtn()
...
function UIRankListTop3Win_2:onClickDetailBtn()
local args={
...
function UIRankListTop3Win_2:onClickTab(index)
...
function UIRankListTop3Win_2:onClickPlayer(index)
...
function UIRankListTop3Win_2:refreshTabList()
local rankNum=#self.rank
if rankNum>1 then
self.tabList:setChildLayoutGroupCreateItems(rankNum,function(index)
...
function UIRankListTop3Win_2:refreshPanel()
...
function UIRankListTop3Win_2:refreshPlayer(index)
...
widget:SetChildCSImageIcon(_playerCmp.moneyIcon,self.args.icon,false)
local value=type(rankData.data[1])=="number"and rankData.data[1]or mathHelper.int64_to_number(rankData.data[1])
widget:SetChildText(_playerCmp.moneyTx,mathHelper.formatNumber(value))
end
end

function UIRankListTop3Win_2:refeshOwner(rankType)
...
self.ownerWidget:SetChildCSImageIcon(_ownerCmp.moneyIcon,self.args.icon,false)

self.ownerWidget:SetChildActive(_ownerCmp.moneyRoot,self.onwerData.data~=nil)
self.ownerWidget:SetChildActive(_ownerCmp.none,self.onwerData.data==nil)
if self.onwerData.data then
local value=self.onwerData.data
value=type(value)=="number"and value or mathHelper.int64_to_number(value)
self.ownerWidget:SetChildText(_ownerCmp.moneyTx,mathHelper.formatNumber(value))
end
end

function UIRankListTop3Win_2.onRankListRefresh(rankType)
