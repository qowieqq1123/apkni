...
def_class("UIShiLianTaGuaJiLoseWin",UIWindowBase)
...
function UIShiLianTaGuaJiLoseWin:bindComponents()

self.bgRoot=UIObject.get(self,0)
self.fgroot=UIObject.get(self,1)
...
self.fightCountBtn=UIButton.get(self,6)
self.noReward=UIText.get(self,7)
self.layer=UIText.get(self,8)
self.nextLayer=UIText.get(self,9)
self.titleRoot=UIObject.get(self,10)
self.shoutongrewards=UIObject.get(self,11)
self.finalHpProgress=UIProgress.get(self,12)
self.closeTips=UIButton.get(self,13)
self.quitText=UIText.get(self,14)
self.continueText=UIText.get(self,15)
self.quitButton=UIButton.get(self,16)
self.continueButton=UIButton.get(self,17)
...
function UIShiLianTaGuaJiLoseWin:unbindComponents()
...
function UIShiLianTaGuaJiLoseWin:onLoaded(...)
self:bindComponents()
end


function UIShiLianTaGuaJiLoseWin:__delete()
self:stopCloseTimer()
...
function UIShiLianTaGuaJiLoseWin:onShow(argtable,afterOnloaded)
...
shiLianTaModel:setGuaJILoseArgs()
...
local curLayer=shiLianTaModel:getGuaJILoseLayer()
local guajiLayer=shiLianTaModel:getGuaJILayer()or curLayer
self:showGuajiReward(guajiLayer,curLayer)


end

function UIShiLianTaGuaJiLoseWin:showGuajiReward(guajiLayer,curLayer)
if not guajiLayer then
guajiLayer=shiLianTaModel:getCurLayer()
end
if not curLayer then
curLayer=shiLianTaModel:getCurLayer()
end
self.layer:setText(FMT.fmt("本次闯荡锁妖塔层数：{0}",guajiLayer))
...
function UIShiLianTaGuaJiLoseWin:onHide()

end

function UIShiLianTaGuaJiLoseWin:onCloseTips()
...
function UIShiLianTaGuaJiLoseWin:onQuitButton()
local cb=self.quitCallBack
self:closeSelf()
if cb then cb()end
end

function UIShiLianTaGuaJiLoseWin:onContinueButton()
local cb=self.continuCallBack
self:closeSelf()
if cb then cb()end
end

function UIShiLianTaGuaJiLoseWin:onFightCountBtn()
if self.isWait then
return
end
...
function UIShiLianTaGuaJiLoseWin:stopCloseTimer()
