...
def_class("UIPushGiftAdvertWin",UIWindowBase)
...
function UIPushGiftAdvertWin:bindComponents()
...
function UIPushGiftAdvertWin:unbindComponents()
...
function UIPushGiftAdvertWin:onLoaded(...)
self:bindComponents()
self.arrowTweener={}
...
function UIPushGiftAdvertWin:__delete()
self:unbindComponents()
pushGiftTwoController.onCloseAdvert()
end

function UIPushGiftAdvertWin:onShow(argtable,afterOnloaded)


local ids=pushGiftTwoController:getAdvertParams()
...
function UIPushGiftAdvertWin:onHide()

end

function UIPushGiftAdvertWin:onSelectDefaultGiftByIdx(selectIdx)
local id=self.ids[selectIdx]
if self.giftId==id then return end
self.giftId=id
self.idx=nil
self.selectIdx=selectIdx
local idx=pushGiftTwoModel:getNextHasLeftBuyTimesGift(id)
self:onSelectPage(idx)
end

function UIPushGiftAdvertWin:onSelectGift(id,idx)
...
function UIPushGiftAdvertWin:onSelectPage(idx)
if self.idx==idx then return end
self.idx=idx
pushGiftTwoModel:flushPushAdvert(self.giftId)
self:freshInfo()
end

function UIPushGiftAdvertWin:freshInfo()
self:freshTabBtns()
self:setArrowStatus()
self:freshGiftInfo()
end

function UIPushGiftAdvertWin:freshTabBtns()
...
function UIPushGiftAdvertWin:freshTabInfo(index)
local id=self.giftId
local idx=self.idx
...
function UIPushGiftAdvertWin:freshGiftInfo()
local id=self.giftId
local idx=self.idx
local cfg=pushGiftTwoConfig.getConfig(id)
self.cfg=cfg
...
function UIPushGiftAdvertWin:freshItemList()
local id=self.giftId
local idx=self.idx
local cfg=pushGiftTwoConfig.getConfig(id)
self.cfg=cfg
...
function UIPushGiftAdvertWin:setArrowStatus()
...
function UIPushGiftAdvertWin:onBuySuccess(id,idx)
if id==self.giftId and idx==self.idx then
local left=pushGiftTwoModel:getIdxLeftBuyTimes(id,idx)
if left<=0 then
local len=#pushGiftTwoConfig.getTotalBuyTimes(id)
while left<=0 and idx<len do
idx=idx+1
left=pushGiftTwoModel:getIdxLeftBuyTimes(id,idx)
end
if left<=0 then
local len=#self.ids
if len>1 then
...
function UIPushGiftAdvertWin:removeId(id)
for i,v in ipairs(self.ids)do
if v==id then
...
function UIPushGiftAdvertWin:moveNext()
local selectIdx=self.selectIdx
local len=#self.ids
self.giftId=nil
...
function UIPushGiftAdvertWin:onBtnBuy()
local idx=self.idx
local id=self.giftId
local leftTimes=pushGiftTwoModel:getIdxLeftBuyTimes(id,idx)
if leftTimes<=0 then
...
function UIPushGiftAdvertWin:onBtnClose()
...
function UIPushGiftAdvertWin:onBtnTab_1()
self:onSelectPage(1)
end



function UIPushGiftAdvertWin:onBtnTab_2()
self:onSelectPage(2)
end



function UIPushGiftAdvertWin:onBtnTab_3()
self:onSelectPage(3)
end

function UIPushGiftAdvertWin:onBtnTab_4()
self:onSelectPage(4)
end



function UIPushGiftAdvertWin:onBtnRightArrow()
local selectIdx=self.selectIdx
local len=#self.ids
...
function UIPushGiftAdvertWin:onBtnLeftArrow()
local selectIdx=self.selectIdx
local len=#self.ids
...
function UIPushGiftAdvertWin.onItemClick(itemid,index,itemguid,attach)
...
function UIPushGiftAdvertWin:onSelectOption(selectLookup,args)
...
function UIPushGiftAdvertWin:showOptionSelect(giftId,lvIdx,shoidIdx)
