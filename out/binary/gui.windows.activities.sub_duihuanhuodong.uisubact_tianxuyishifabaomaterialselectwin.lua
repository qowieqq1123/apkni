...
def_class("UISubAct_tianxuyishiFaBaoMaterialSelectWin",UIWindowBase)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:bindComponents()
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:unbindComponents()
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:__delete()
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onShow(argtable,afterOnloaded)
self.needItemColor=argtable.needItemStage
...
self.colorCount=argtable.stageCount
...
local firstItem=self.bagList[1]
...
local itemData=self.bagList[idx]
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:on_item_list_changed(argstable)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:refreshTargetCostItem()
...
conf={itemid=itemid,itemcount='',showCountBG=false,iconColor=self.needItemColor,showname=false}
local prop=itemsComponentHelper.getCommonFillDataSmall(conf)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onSliderChange(newSelectCnt)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:refreshSelectBagList(diffNum)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:refreshBagList()

local filter={}
filter[ITEM_FILTER_TYPE.eItemType1]={ITEM_FILTER_COMPARE.eEquals,1}
filter[ITEM_FILTER_TYPE.eStage]={ITEM_FILTER_COMPARE.eEquals,{self.needItemColor}}
self.bagList=bagControl.getBagItemsByFilter(BAG_TYPE.eMaterialsBag,filter,nil,false)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:getBagItemIdx(bagListIdx)
local idx=bagListIdx%_colomn
return math.ceil(bagListIdx/_colomn),idx==0 and _colomn-1 or idx-1
end

function UISubAct_tianxuyishiFaBaoMaterialSelectWin:refreshBagListItem(bagListIdx)
...
local itemInfo=self.bagList[bagListIdx]
if itemInfo and itemInfo.itemguid then
local itemguid=tostring(itemInfo.itemguid)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onStartAction()
end

function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onFreshAction(index,itemWidget)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onItemClick(itemid,index,itemguid,attach)
...
local argstable={
formType=TIPS_FORM_TYPE.eFaBaoMaterial,
itemid=itemid,
itemguid=itemguid,
attach={formType=TIPS_FORM_TYPE.eFaBaoMaterial,index=index,selectNumCmpArgs=selectNumCmpArgs,},
}
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:setSelectNum(itemguid,selectedNum,index,itemid)
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:getSelectNum(itemguid)
if self.selectList==nil then self.selectList={}end
if self.selectList[tostring(itemguid)]then
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onCloseBtn()
self:closeSelf()
end

function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onExchangeBtn()
...
table.insert(itemList,{itemguid,cnt})
table.insert(itemList2,{itemid,cnt})
else
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onSubBtn()
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onAddBtn()
if self.max<=1 then
return
end
...
function UISubAct_tianxuyishiFaBaoMaterialSelectWin:onHelpBtn()
local d={}
d.mode=3
d.title="规则介绍"
