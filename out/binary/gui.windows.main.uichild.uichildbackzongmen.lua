...
UIChildBackZongMen=simple_class(UIChildObject)

local defaultIcon='button_zjmzongmenqh'
local _this

function UIChildBackZongMen:onLoaded()

end

function UIChildBackZongMen:onShow(isInit)
if isInit then
_this=self
end
...
function UIChildBackZongMen:OnButtonClick()
if not mainControl:isSceneType(eSceneType.eZongmen)or not zongmenControl:isMountid(mapIdType.zhufeng)then

cameraMoveController:Begin({eSceneType.eZongmen,mapIdType.zhufeng})
end
end

function UIChildBackZongMen:OnYuanZhuButtonClick()
xianjieController:reqHelpAll()
end

function UIChildBackZongMen:release()
...
function UIChildBackZongMen.refreshReddot(class,sub_typo,last_flag,flag)
...
function UIChildBackZongMen:refreshReddotView(isreddot)
...
function UIChildBackZongMen:doPunchRotation(isreddot)
