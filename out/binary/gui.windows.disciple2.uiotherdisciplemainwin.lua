...
def_class("UIOtherDiscipleMainWin",UIWindowBase)
...
function UIOtherDiscipleMainWin:bindComponents()

self.discipleListPanel=UIScrollView.get(self,0)
self.root=UIObject.get(self,1)
self.copyBtn=UIButton.get(self,2)
self.back=UIObject.get(self,3)
self.menuAnimGrid=UIObject.get(self,4)
self.menulist=UIObject.get(self,5)
self.scrollView=UIScrollView.get(self,6)
self.menu_anim_1=UIObject.get(self,7)
self.menu_anim_2=UIObject.get(self,8)
self.menu_anim_3=UIObject.get(self,9)
self.menu_anim_4=UIObject.get(self,10)
...
function UIOtherDiscipleMainWin:unbindComponents()
...
wins={'UIOtherDiscipleInfoWin',},
},
[FULL_TAB_TYPE.eOtherDiscipleAttr]={

wins={'UIOtherDiscipleAttrWin'},
},
}
...
function UIOtherDiscipleMainWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UIOtherDiscipleMainWin:__delete()
_this=nil
...
function UIOtherDiscipleMainWin.onTestModelChange(flag)
if _this==nil then return end

_this:showCopyBtn()
end

function UIOtherDiscipleMainWin:showCopyBtn()
local show=false
...
function UIOtherDiscipleMainWin:onShow(argtable,afterOnloaded)
...
if mathHelper.compareInt64(dzData.base.discipleguid,self.disciple_guid)then
self.curDisIndex=i
break
end
end
...
local showlist=#self.dislist>1
self.discipleListPanel:setActive(showlist)
end

function UIOtherDiscipleMainWin:onLoadFinish()
self.animLock1=nil
...
function UIOtherDiscipleMainWin:clearMenuTweener()
...
function UIOtherDiscipleMainWin:onClickClose()
if self.animLock1==true then return end
self:closeSelf()
end

function UIOtherDiscipleMainWin:freshMenuList()
...
function UIOtherDiscipleMainWin:fillMenu(index,tabType,config)
...
function UIOtherDiscipleMainWin:freshMenuSelect(index,is_select)
if index==nil then
return
end
local selectMenuIdx=self.selectMenuIdx
...
function UIOtherDiscipleMainWin:on_click_callback(id,index,guid,attach)
if self.animLock1==true then return end
...
function UIOtherDiscipleMainWin:getTabType()
return self.tabType
end

function UIOtherDiscipleMainWin:onShowArgRecv(argtable)
self.tabType=argtable.tabType
self:refreshWin()
end

function UIOtherDiscipleMainWin:refreshWin()
local show_wins={}
local close_wins={}
...
if cur_wins and#cur_wins>0 then
for i,v in ipairs(cur_wins)do
local args={guid=self.disciple_guid}
...
function UIOtherDiscipleMainWin:closeAllWin()
if self.activeWin~=nil then
for k,v in pairs(self.activeWin)do
...
function UIOtherDiscipleMainWin:refreshDiscipleList()
...
function UIOtherDiscipleMainWin:changItemSelect(item,isSelect)
item:SetChildActive(3,isSelect)
end

function UIOtherDiscipleMainWin:on_select_dis(id,index,guid,attach)
...
local baseData=dzData.base
self.disciple_guid=baseData.discipleguid
if self.activeWin~=nil then
for k,v in pairs(self.activeWin)do
if v==true then
local win=UIManager:findActiveWindow(k)
if win and win.onChangeDisciple then
win:onChangeDisciple(baseData.discipleguid)
end
end
end
end
...
function UIOtherDiscipleMainWin:onCopyBtn()
local dzData=self.dislist[self.curDisIndex]
WriteInCopyBuffer(tostring(dzData.base.discipleguid))
end

function UIOtherDiscipleMainWin:getDiscipleList()
return self.dislist
end
