pushGiftTwoManager=gameState.addListener({})
...
{
overfunc=function(params)
return not pushGiftTwoManager:checkServerLimitOpen(params)
end
},
}
...
local cnt=pushGiftTwoModel:getAlreadyBuyTimesById(id)
if times and times>0 then
return cnt>=times
end
return pushGiftTwoModel:isBuyOver(id)
end,
...
if not pushGiftTwoModel:isAlreadyDo(giftid)then return false end
local cost=pushGiftTwoModel:getStartedTime(giftid)
...
function pushGiftTwoManager:onAppStart()

notifySystem:listenNotify(notifyConfig.onDiscipleJJChange,function()
pushGiftTwoManager:onChanged(_checkType.eDZJingjie)
end)

notifySystem:listenNotify(notifyConfig.onDiscipleLTChange,function()
pushGiftTwoManager:onChanged(_checkType.eDZLianTi)
end)

notifySystem:listenNotify(notifyConfig.onGongFaActive,function()
pushGiftTwoManager:onChanged(_checkType.eGongFaActive)
pushGiftTwoManager:onChanged(_checkType.eUnFinishActiveGongFa)
end)

notifySystem:listenNotify(notifyConfig.building_event,function(eventType,...)
if eventType==buildingEvent.zongmenLevelUp then
pushGiftTwoManager:onChanged(_checkType.eZMlv)
end
if eventType==buildingEvent.levelUpComplete then
pushGiftTwoManager:onChanged(_checkType.eCheckbuildLevel)
end
end)
notifySystem:listenNotify(notifyConfig.on_system_open,function(sysid,isNew)
if sysid==SYSTEM_DEFINE.eLimitedTimeGift2 and initProControl.isDone()then
self:initalize()
end
pushGiftTwoManager:onChanged(_checkType.eSystemOpen)
end)
...
pushGiftTwoManager:onChanged(_checkType.eTaskFinish)
end
end)
notifySystem:listenNotify(notifyConfig.onNewDay,function()
pushGiftTwoManager:onChanged(_checkType.eServerOpenDay)
pushGiftTwoManager:onChanged(_checkType.eServerOpenOneDay)
pushGiftTwoManager:onChanged(_checkType.eWeekOpen)
end)

notifySystem:listenNotify(notifyConfig.on_item_list_first_get,function(itemlist)
pushGiftTwoManager:onChanged(_checkType.eFirstGetItem,itemlist)
end)

notifySystem:listenNotify(notifyConfig.onDiscipleTianMingLvChange,function(dzguid,oldlv,lv)
pushGiftTwoManager:onChanged(_checkType.eDZTianMing,dzguid)
end)
notifySystem:listenNotify(notifyConfig.onDiscipleDaoYanLvChange,function(dis_guid,olddylv,dylv)
pushGiftTwoManager:onChanged(_checkType.eDZDaoYan_lv,dis_guid)
end)

socketManager:addNotify(14,1,function()
pushGiftTwoManager:onChanged(_checkType.eLeiJiRecharge)
end)

notifySystem:listenNotify(notifyConfig.onDiscipleStateChange,function(dzguid,state)
pushGiftTwoManager:onChanged(_checkType.eChuiWeiGift,state)
end)

notifySystem:listenNotify(notifyConfig.onDiscipleLingGenUpLevel,function(dzguid)
pushGiftTwoManager:onChanged(_checkType.eDZLingGenLv,dzguid)
end)

socketManager:addNotify(15,33,function(id,idx,buytimes)
local hasTimes=pushGiftTwoModel:hasLeftBuyTimes(id)
if hasTimes then return end
pushGiftTwoManager:onChanged(_checkType.eBuyGift,id)
end)

socketManager:addNotify(2,53,function()
pushGiftTwoManager:onChanged(_checkType.eFabaoOverLv)
end)

notifySystem:listenNotify(notifyConfig.onDiscipleJJBroke,function(guid,res,jjlv)
if res~=0 then
pushGiftTwoManager:onChanged(_checkType.eTuPoJingjieFail,jjlv)
end
end)

socketManager:addNotify(2,96,function()
pushGiftTwoManager:onChanged(_checkType.eDaoBingJllv)
end)
...
pushGiftTwoManager:onChanged(_checkType.eKuafuActivity)
elseif serverType==activitiesServerType.eRole then
pushGiftTwoManager:onChanged(_checkType.eRoleActivity)
else
pushGiftTwoManager:onChanged(_checkType.eSelfActivity)
end
end
end)
...
pushGiftTwoManager:onChanged(_checkType.eYLZSeriousInjuryCount)
end
end)
notifySystem:listenNotify(notifyConfig.onGuBaoShengXing,function(gbid)
pushGiftTwoManager:onChanged(_checkType.eGuBaoStarUp,gbid)
end)
...
pushGiftTwoManager:onChanged(_checkType.eLingShouJingJie)
end)

notifySystem:listenNotify(notifyConfig.onLingShouXMChange,function()
pushGiftTwoManager:onChanged(_checkType.eLingShouXueMai)
end)
end

function pushGiftTwoManager:onEnterState(isReconnect)
pushGiftTwoManager:stopTimer()
_lookupCfg={}
_eventCache={}
self.hasEnter=false
_stamp={}
...
function pushGiftTwoManager:onLeaveState(isReconnect)
pushGiftTwoManager:stopTimer()
_lookupCfg={}
_eventCache={}
self.hasEnter=false
...
function pushGiftTwoManager:onProtocolReq()

end

function pushGiftTwoManager:initalize()
if not systemModel.isOpen(SYSTEM_DEFINE.eLimitedTimeGift2)or
not initProControl.isDoneKF()or
not initProControl.isDone()then return end
if self.isInit==true then return end
self.isInit=true
pushGiftTwoManager:initCfg()
pushGiftTwoManager:startTimer()
pushGiftTwoModel:initAllhideGift()
pushGiftTwoController.openAdvertWin()
end

function pushGiftTwoManager:isInitCfg()
return self.isInit
end


function pushGiftTwoManager:initCfg()
_tickCache={}
...
if pushGiftTwoManager:checkOtherOut(cfg)then return end
if not pushGiftTwoModel:isHide(id)and
pushGiftTwoModel:isCanActive(id)then

if pushGiftTwoManager:canOpen(openconf,nil,true)then
pushGiftTwoController.openGift(id)
else
...
function pushGiftTwoManager:isOutTick(id)
if _tickCache[id]==nil then return true end
...
function pushGiftTwoManager:startTimer()
...
local id=cfg.id
if not pushGiftTwoModel:isHide(id)and
pushGiftTwoModel:isCanActive(id)and
pushGiftTwoManager:canOpen(cfg.openconf,args)then
pushGiftTwoController.openGift(id)
_tickCache[id]=true
_openedTable[id]=0
end
...
local cfg=pushGiftTwoConfig.getConfig(id)
if pushGiftTwoModel:isDoing(id)or
pushGiftTwoModel:isBuyOver(id)or
pushGiftTwoModel:isExpired(id)or
pushGiftTwoModel:isHide(id)or
pushGiftTwoManager:isOutTick(id)or
(_openedTable[id]and _openedTable[id]>=_openMaxCnt)then
_tickCache[id]=nil
elseif pushGiftTwoModel:isCanActive(id)and
not pushGiftTwoModel:isHide(id)and
pushGiftTwoManager:canOpen(cfg.openconf)then
pushGiftTwoController.openGift(id)
...
local expiredDatas=pushGiftTwoModel:getAllExpiredAgainData()
for id,_ in pairs(expiredDatas)do
if(_openedTable[id]==nil or _openedTable[id]and _openedTable[id]<_openMaxCnt)and pushGiftTwoModel:isExpiredAgain(id)then
...
local ids=pushGiftTwoModel:getGiftIds()
if ids and#ids>0 then
local len=#ids
for i=len,1,-1 do
local id=ids[i]
if not pushGiftTwoConfig.isForver(id)and
pushGiftTwoModel:getLeftBuyTime(id)<=0 then
...
_circleCheckCurTime=0
pushGiftTwoManager:onTickChange()
end
...
function pushGiftTwoManager:resetStamp(id)
_stamp[id]=nil
end

function pushGiftTwoManager:stopTimer()
...
function pushGiftTwoManager:canOpen(openconf,args,init)
...
function pushGiftTwoManager:onChanged(checkType,...)
...
function pushGiftTwoManager:freshEnter()
local ids=pushGiftTwoModel:getLimitGiftIds()
local has=#ids>0
...
local enterInfo=
{
enterType=ENTER_TYPE.ePushGiftTwo,
...
enterManager:freshFunc('onfreshGifts',ENTER_TYPE.ePushGiftTwo)
end
end

function pushGiftTwoManager:removeEnter()
...
function pushGiftTwoManager:onFabaoChange(isInit)
if isInit then return end
pushGiftTwoManager:onChanged(_checkType.eFabaoOverLv)
end

function pushGiftTwoManager:onDaoBingChange(isInit)
if isInit then return end
pushGiftTwoManager:onChanged(_checkType.eDaoBingJllv)
end


function pushGiftTwoManager:onTickChange()
pushGiftTwoManager:onChanged(_checkType.eDelayOpen)
end

function pushGiftTwoManager:checkOtherOut(cfg)
...
function pushGiftTwoManager:checkServerLimitOpen(serverlimit)
