...
def_class("UIMDiscipleSelect_manager",UIMDiscipleSelect)
...
function UIMDiscipleSelect_manager:onLoaded(...)
self:bindComponents()
_this=self
...
function UIMDiscipleSelect_manager:__delete()
...
function UIMDiscipleSelect_manager:getDiscipleDatas()
local list={}
...
function UIMDiscipleSelect_manager:onShow(argtable,afterOnloaded)
self.args=argtable
...
function UIMDiscipleSelect_manager:initView()

end

function UIMDiscipleSelect_manager:refreshView()
self:refreshScrollView()
self:refreshButtons()
end

function UIMDiscipleSelect_manager:reSelectDisciple(default_idx)
...
function UIMDiscipleSelect_manager:getNetDataList()
...
function UIMDiscipleSelect_manager:initDiscipleList()
self.dzIdStr=nil
...
function UIMDiscipleSelect_manager:getSpecialScore(cfg,dzId,build_effects,bdType)
local list={}
for i,v in ipairs(build_effects)do
...
function UIMDiscipleSelect_manager:refreshScrollView()
...
function UIMDiscipleSelect_manager:checkRecommend(guid_str)
...
function UIMDiscipleSelect_manager:refreshItem(id,item)
local index=id+1
...
local fight_str=FMT.fmt('<color=#7d3b17>聪慧</color> {0}',data.disciple.attrList[3])
...
if self.bdType==SLG_SYSTEM_TYPE.eYuShouFang then
level_effect_str=''
end
...
elseif self.isShowCharm==3 then
state_title_str=''
state_color_fmt=''
state_state_str=''
end
...
if self.isShowCharm and self.isShowCharm==3 then desc_str_2=''end
...
function UIMDiscipleSelect_manager.onDescSlotClick(disIdx,speIdx)
...
function UIMDiscipleSelect_manager:refreshSelect(index,flag)
local item=self.grids[index-1]
local item_cmp_index_=discipleSelectController.item_cmp_index
item:SetChildActive(item_cmp_index_.img_select,flag)
end

function UIMDiscipleSelect_manager:onClickItem(index)
...
function UIMDiscipleSelect_manager:refreshButtons()
local c=#self.disciplelist
if c>0 then
...
function UIMDiscipleSelect_manager:OnEnable()

end


function UIMDiscipleSelect_manager:OnDisable()

end



function UIMDiscipleSelect_manager:onClickClose()
if not self.wait_replace_building_mgr then
self.parentWin:closeSelf()
end
end

function UIMDiscipleSelect_manager:onBtnWork()
if self.select_index then
local data=self.disciplelist[self.select_index]
local disdata=data.disciple
local guid=disdata.discipleguid
...
if not self.isPrison then
if self:CheckYSFConditions(guid)then
return
end
...
guildOrderController:setAutoSC(true)
self.callback(guid)
end
end
else
...
function UIMDiscipleSelect_manager:onBtnFire()
if not discipleSelectController.isDiziEmptyOrNil(self.dzIdStr)then
...
local func=function()
if _this==nil or not _this.isVisible then return end
_this.be_fire_building_mgr_id=guid
zongmenControl:reqChangeBuildingManager(sfId_,_this.bdData.un_build_id,int64.new(0))
end

local isCanFire=zongmenControl:checkBuildingManagerCanFire(sfId_,self.bdData.un_build_id,func)
if isCanFire then
return func()
end
else
...
function UIMDiscipleSelect_manager.on_building_event(etype,sfId,ubdId,dzId,olddzId)
...
function UIMDiscipleSelect_manager:onSearchBtn()
...
function UIMDiscipleSelect_manager:onSearchCancelBtn()
...
function UIMDiscipleSelect_manager:onSearchChange(str)
self:refreshInputBtns(str)
end

function UIMDiscipleSelect_manager:refreshInputBtns(change_str)
...
function UIMDiscipleSelect_manager:clearSearchInput()
self.inputstr=nil
local str=''
...
function UIMDiscipleSelect_manager:hasAnyAnPaiDZ()
...
function UIMDiscipleSelect_manager:CheckYSFConditions(guid)
if self.bdType and self.bdType==SLG_SYSTEM_TYPE.eYuShouFang then
