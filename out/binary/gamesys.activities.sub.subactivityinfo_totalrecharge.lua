...
local subActivityInfo_totalRecharge={name='totalRecharge'}

function subActivityInfo_totalRecharge:onInit()

end

function subActivityInfo_totalRecharge:onStart()
self.onRechargeFunc=function(...)self:onRecharge(...)end
self:listenNotify(notifyConfig.onRecharge,self.onRechargeFunc)
end

function subActivityInfo_totalRecharge:onDelete()
...
function subActivityInfo_totalRecharge:checkReddot()
local reddot=false
...
local yuenanTarget=target[GameVersion]
targetRechargeNum=yuenanTarget[index]
end
isFinish=totalRechargeNum/10>=targetRechargeNum
end
...
function subActivityInfo_totalRecharge:onRecharge(firstFlag,lastTotalRecharge,totalRecharge,lastDailyRecharge,dailyRecharge)
if not self.data then
...
function subActivityInfo_totalRecharge:getSelectData(taskIndex,rewardIndex)
...
function subActivityInfo_totalRecharge:getSelectDataByTaskIndex(taskIndex)
if self.data.selectData then
return self.data.selectData[taskIndex]
end
end

function subActivityInfo_totalRecharge:setSelectData(taskIndex,rewardIndex,selectIndex)
...
function subActivityInfo_totalRecharge:checkTaskIsGot(taskIndex)
if not self.data then
...
function subActivityInfo_totalRecharge:reqTotalRecharge_checkFreeRewardsIsGot()
...
function subActivityInfo_totalRecharge:reqTotalRecharge_getFreeRewards()
...
function subActivityInfo_totalRecharge:reqTotalRecharge_getTaskReward(taskIndexlist)
...
function subActivityInfo_totalRecharge:reqTotalRecharge_selectTaskReward(taskIndex,selectList)
local json_str=jsonHelper.encode({2,taskIndex,selectList})
activitiesController:sendProtocol(actSendType.eComonReqHandle,self.act_id,self.sub_act_type,self.sub_act_id,json_str)
end

return subActivityInfo_totalRecharge