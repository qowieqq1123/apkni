...
def_class("UIXingJiaoShangRenMainWin",UIWindowBase)
...
function UIXingJiaoShangRenMainWin:bindComponents()
...
self.freeBtn=UIButton.get(self,4)
...
self.timeBg=UIImage.get(self,10)
self.timeTx=UIText.get(self,11)
self.titleImage=UIImage.get(self,12)
...
function UIXingJiaoShangRenMainWin:unbindComponents()
...
function UIXingJiaoShangRenMainWin:onLoaded(...)
self:bindComponents()
_this=self
end


function UIXingJiaoShangRenMainWin:__delete()
...
function UIXingJiaoShangRenMainWin:onShow(argtable,afterOnloaded)
if argtable then
...
function UIXingJiaoShangRenMainWin:onHide()
...
function UIXingJiaoShangRenMainWin:onJumpBtn()
local info=self.info
if info:checkDoing()then
UIFullCommonControl:closeUI(nil,true)
UIManager:showWindow("UIXingJiaoShangRenExchangeWin",{info=info})
else
UIManager.info("商人已离开")
end
end

function UIXingJiaoShangRenMainWin:onFreeBtn()
if self.info:getFree()then
call_activitiesHandle_func("activitiesHandle_xingjiaoshangren","reqRewardFree",self.activityId,self.subId)
else
loggerUtil.logErrFMT("已领取该礼包")
end
end

function UIXingJiaoShangRenMainWin:refreshView()
...
if titleImage~=""then
self.titleImage:setSprite("ui/windows/activities/sub_xingjiaoshangren/xingjiaoshangren_atlas_pak.ab",titleImage)
else
...
function UIXingJiaoShangRenMainWin:startCDTick()
...
function UIXingJiaoShangRenMainWin:stopCDTick()
if self.cdTick then
...
function UIXingJiaoShangRenMainWin:updateCDTick()
...
function UIXingJiaoShangRenMainWin:refreshFree()
local free=self.info:getFree()
self.freeBtn:setActive(free)
end