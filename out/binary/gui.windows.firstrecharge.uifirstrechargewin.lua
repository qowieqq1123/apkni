...
def_class("UIFirstRechargeWin",UIWindowBase)
...
function UIFirstRechargeWin:bindComponents()
...
function UIFirstRechargeWin:unbindComponents()
...
function UIFirstRechargeWin:onLoaded(...)
_this=self
self:bindComponents()
end


function UIFirstRechargeWin:__delete()
...
function UIFirstRechargeWin:onShow(argtable,afterOnloaded)

self.allCfg=firstRechargeModel:getSortCfgList()
self.selectIndex=1
...
self.tabList=firstRechargeModel:getShowTabList()
...
function UIFirstRechargeWin:onHide()
if self.isImageFloat then
self:doLocalMoveY(false)
end
end
...
function UIFirstRechargeWin:showBgModel()
...
function UIFirstRechargeWin:finishLoadBgModel(isFront)
...
function UIFirstRechargeWin:selectTab(selectIndex,dayIndex)
...
self.frCfg=cfgHelper.get1(cfg_firstchargeconfig_get,self.rechargeId)
self.dayCount=self.tabList[selectIndex].dayCount
if not firstRechargeModel:getFirstRechargePageOpenFlagByRechargeId(self.rechargeId)then
firstRechargeModel:setFirstRechargePageOpenFlagByRechargeId(self.rechargeId)
...
self.firstRechargeData=firstRechargeModel:getFirstRechargeDataByRechargeId(self.rechargeId)
if not dayIndex then
local _,canGetDayIndex=firstRechargeModel:checkReddotById(self.rechargeId)
...
function UIFirstRechargeWin:selectDizi(selectIndex)
...
function UIFirstRechargeWin:refresh(isChangePage)
...
function UIFirstRechargeWin:refreshTab()
...
function UIFirstRechargeWin:refreshTabPanel()
...
local reddot=firstRechargeModel:checkReddotById(tabData.rechargeId)
...
function UIFirstRechargeWin:refreshLeftPanel(isChangePage)
...
function UIFirstRechargeWin:showItemModel(isChangePage)
...
function UIFirstRechargeWin:showBuildingModel()
...
function UIFirstRechargeWin:showSpecialImageModel()
...
function UIFirstRechargeWin:showSpecialSpineModel()
...
function UIFirstRechargeWin:refreshDiziPanel()
local showModelItemId=self.showModelPram.itemid
...
function UIFirstRechargeWin:refreshGubaoPanel()
local showModelItemId=self.showModelPram.itemid
...
function UIFirstRechargeWin:refreshSelectDiziPanel()
...
function UIFirstRechargeWin:refreshRightPanel()

local widget

local rewardsAllCfg=self.frCfg.rewards
local rewardsCfg=firstRechargeModel:getVersionAndPfCfg(rewardsAllCfg)
...
function UIFirstRechargeWin:refreshBottomPanel()
...
self.nextRefreshTime=firstRechargeModel:getNextRefreshTime()
if self.nextRefreshTime then
isNeedTimer=true
end


local isBought=firstRechargeModel:checkIsBought(self.rechargeId)
...
local isGot=firstRechargeModel:checkIsGotFirstRechargeRewardByIdAndDay(self.rechargeId,self.dayIndex)
...
self.canGetRewardTime=firstRechargeModel:getCanGetRewardTimeByIdAndDay(self.rechargeId,self.dayIndex)
...
function UIFirstRechargeWin:onMask()

end



function UIFirstRechargeWin:onCloseBtn()
self:closeSelf()
end

function UIFirstRechargeWin:onInfoBtn()
local showModelItemId=self.showModelPram.itemid
UIRecruitControl:showItemDiscipleInfoByItemId2(showModelItemId,self.selectDiziIndex)
end

function UIFirstRechargeWin:onConfirmBtn(isBought)
if isBought then

payControl.reqPay(self.rechargeId)
else

firstRechargeController:reqGetFirstRechargeReward(self.rechargeId,self.dayIndex)
...
function UIFirstRechargeWin:onDayBtn_1()
self:onDayBtnClick(1)
end

function UIFirstRechargeWin:onDayBtn_2()
self:onDayBtnClick(2)
end

function UIFirstRechargeWin:onDayBtn_3()
self:onDayBtnClick(3)
end

function UIFirstRechargeWin:onDayBtnClick(day,isForceRefresh)
...
function UIFirstRechargeWin:onRewardItemClick(itemId,itemIndex)
if itemId==-1 then
return
end
...
function UIFirstRechargeWin:onClickTab(index)
...
function UIFirstRechargeWin:onClickSelectDiziItem(index)
...
function UIFirstRechargeWin:onClickItemMask()
tipsManager.showTips({itemid=self.clickItemId,move=TIPS_MOVE_POS.eRight})
end


function UIFirstRechargeWin:doLocalMoveY(isFloat)
if isFloat then
...
function UIFirstRechargeWin:setRemainingTimeTimer()
self:clearTimer()
...
function UIFirstRechargeWin:clearTimer()
if self.timer then
...
function UIFirstRechargeWin:refreshDayBtnReddot()
local nowTime=gameUtilityModel.getServerLongTime()
for i,reddot in ipairs(self.dayBtnReddot)do
local reddotFlag=false
if i<=self.dayCount then

if not firstRechargeModel:checkIsGotFirstRechargeRewardByIdAndDay(self.rechargeId,i)then

local targetTime=firstRechargeModel:getCanGetRewardTimeByIdAndDay(self.rechargeId,i)
...
function UIFirstRechargeWin:doModelFadeIn()
self:clearModelTween()
...
function UIFirstRechargeWin:clearModelTween()
