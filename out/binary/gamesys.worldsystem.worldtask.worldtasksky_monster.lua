...
worldTaskSky_Monster=simple_class(worldTaskSky)

function worldTaskSky_Monster:__init(data)
worldTaskSky.__init(self,data)
end

function worldTaskSky_Monster:start(serverTime)
...
function worldTaskSky_Monster:startMove(fast)
...
function worldTaskSky_Monster:createMoveData(index,fast)
...
self:handleForwardWays(ways,index,disObj,fast,forward,partTime5,partTimes)
else

disObj:ChangeModelColor(Color.New(1,1,1,0),0)
table.insert(ways,self:waitWay_EndPoint(index,true,partTime5))
end
...
if index==1 and self.LT_Time(pass,partTime4)and self.GE_Time(fast,partTime3)then
worldHUDModel:UpdateHUDByKey(worldTaskModel:convertMissionUnitKey(self.key,self:getFirstShowDisciple()))
end
local unitId=tonumber(targetInfo[2])
...
function worldTaskSky_Monster:onMoveComplete(index,fast)
...
function worldTaskSky_Monster:isPassForward()
return self.GE_Time(self.fast,self.onceTime)
end

function worldTaskSky_Monster:isPassWait()
return self.GE_Time(self.fast,self.onceTime+self.waitFight)
end

function worldTaskSky_Monster:isFightTime()
return self.GE_Time(self.fast,self.moveTime+self.jumpTime+self.fadeDuration)
and self.LT_Time(self.fast,self.onceTime+self.waitFight)
end

function worldTaskSky_Monster:isWaitingBack()
return self.corners and self:isPassForward()and not self:isPassWait()or false
end

function worldTaskSky_Monster:getWaitedTime()
return self.fast-self.onceTime
end

function worldTaskSky_Monster:isLeaderReach()
return self:isFightTime()
end

function worldTaskSky_Monster:goBack()
