...
def_class("UI_activity_main_Win",UIWindowBase)
...
function UI_activity_main_Win:bindComponents()
...
function UI_activity_main_Win:unbindComponents()
...
function UI_activity_main_Win:onLoaded(...)
_this=self
self:bindComponents()
...
function UI_activity_main_Win:__delete()
_this=nil
...
function UI_activity_main_Win:onHide()

end

function UI_activity_main_Win.onActivityStateChange(actID,state)
...
function UI_activity_main_Win.onSubActivityOpen(actID,subType,subid,flag,args)
if _this==nil then return end
...
function UI_activity_main_Win.onSubActivityStateChange(actID,subType,subid,state)
if _this==nil then return end
...
function UI_activity_main_Win.onSubActivityReddotChange(actID,subType,subID)
if _this==nil then return end
...
function UI_activity_main_Win.onSubActUnlock(actID,subType,subID)
if _this==nil then return end
...
function UI_activity_main_Win:onShow(argtable,afterOnloaded)
self:clearSubWin()
...
function UI_activity_main_Win:onShowArgRecv(argtable)
self:onShow(argtable)
end

function UI_activity_main_Win:changeBG(bgname)
...
function UI_activity_main_Win:changeMoney(moneytypes)
...
function UI_activity_main_Win:initSubList()
...
function UI_activity_main_Win:refreshMenu()
...
function UI_activity_main_Win:getMenuItemIndex(actID,subType,subID)
for i,v in ipairs(self.sublist)do
...
function UI_activity_main_Win:refreshMenuItem(item,idx)
if item==nil then
...
item:SetChildNewBieComponentId(4,FMT.fmt('UI_activity_main_Win.actMenuItem_{0}',idx))
end

function UI_activity_main_Win:refreshMenuItemSelect(item,idx,flag)
if item==nil then
item=self.menuGrid:getChildLayoutGroupGridItem(idx-1)
end
...
function UI_activity_main_Win:refreshMenuItemState(item,idx)
if item==nil then
item=self.menuGrid:getChildLayoutGroupGridItem(idx-1)
end
local data=self.sublist[idx]
...
function UI_activity_main_Win:onItemClick(idx)
...
function UI_activity_main_Win:refreshSubWin()
self:hideSubWin()
...
function UI_activity_main_Win:changeParam(args)
...
args.parentWin='UI_activity_main_Win'
args.extraParams=self.extraParams
end

function UI_activity_main_Win:hideSubWin()
...
function UI_activity_main_Win:clearSubWin()
...
function UI_activity_main_Win:onClickClose()
...
function UI_activity_main_Win:onClickBlock()
...
function UI_activity_main_Win:activeRoot(flag)
self.root:setActive(flag)
end

function UI_activity_main_Win:fadeRoot(alpha,duration)
self.root:setChildCanvasGroupDOFade(alpha,duration,nil)
end

function UI_activity_main_Win:activeBlack(flag)
self.blackImg:setActive(flag)
end

function UI_activity_main_Win:setLockClick(flag)
self.lockClick=flag
end

function UI_activity_main_Win.listenChangTag(actid,subtype,subid)
...
function UI_activity_main_Win:refreshMenuItemFlag(item,idx,act_id_,sub_act_type_,sub_act_id_)
if item==nil then
