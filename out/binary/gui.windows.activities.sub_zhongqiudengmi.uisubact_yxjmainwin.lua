...
def_class("UISubAct_YXJmainWin",UIWindowBase)
...
function UISubAct_YXJmainWin:bindComponents()
...
function UISubAct_YXJmainWin:unbindComponents()
...
function UISubAct_YXJmainWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UISubAct_YXJmainWin:__delete()
self:clearTimer()
...
function UISubAct_YXJmainWin:onShow(argtable,afterOnloaded)
...
self.bgModel:setChildUIModelShowTarget(6094,1,{},eAnimationID.stand,false,false,0,function()
...
function UISubAct_YXJmainWin:onHide()

end


function UISubAct_YXJmainWin:refreshOtherDay()
...
function UISubAct_YXJmainWin:onRulebtn()
local d={}
d.title='规则说明'
d.mode=3
d.name='UISubAct_YXJmainWin_rule_%d'
d.showBlack=true
UIManager:showWindow('UIRuleWin',d)
end

function UISubAct_YXJmainWin:onGobtn()
...
self:showWindow("UISubAct_YXJMtitleWin",temp)
end
end
end

function UISubAct_YXJmainWin:refreshActivityTime()
self:clearTimer()
local func
func=function()
...
function UISubAct_YXJmainWin:clearTimer()
if self.timer then
...
function UISubAct_YXJmainWin:doTalkAnim_player()
...
function UISubAct_YXJmainWin:talkEnd()
...
function UISubAct_YXJmainWin:ClickSpeaking_player()

end


function UISubAct_YXJmainWin:freshinfo()
local idx=self.sub_actInfo:getOpenDayIndex()
...
function UISubAct_YXJmainWin:onClickItem(itemId,index,guid,attach)
tipsManager.showTips({itemid=itemId,itemguid=nil,showModel=true})
end

function UISubAct_YXJmainWin:getMaxTitleNum()
local idx=self.sub_actInfo:getOpenDayIndex()
local cfg_questions=self.subcfg.questions
...
function UISubAct_YXJmainWin:getNowNum()
local finishIdex=self.sub_actInfo:getAnserdata()
return finishIdex
end

function UISubAct_YXJmainWin:freshbtntxt()
local reward_bits=self.sub_actInfo:getRewardBits()
local idx=self.sub_actInfo:getOpenDayIndex()
local finishIdex=self:getNowNum()
local maxNum=self:getMaxTitleNum()
...
function UISubAct_YXJmainWin:freshdatijindu()
local finishIdex=self:getNowNum()
local maxNum=self:getMaxTitleNum()
local reward_bits=self.sub_actInfo:getRewardBits()
...
function UISubAct_YXJmainWin:openRewardwin()
local temp=
{
act_id=_this.actID,
...
function UISubAct_YXJmainWin:freshRolePanel()
...
function UISubAct_YXJmainWin:doSpeaking_playera()
local cfg=self.subcfg.shopspeaks
local speakList=cfg
local rand=math.random(1,#speakList)
local speakStr=speakList[rand]
...
function UISubAct_YXJmainWin:doTalkAnim_playera()
if self.talkTween2~=nil then
self.talkTween2:Kill()
self.talkTween2=nil
end
...
function UISubAct_YXJmainWin:talkEnda()
if self.speakShowTimer then
self:stopTimerByID(self.speakShowTimer)
self.speakShowTimer=nil
end
