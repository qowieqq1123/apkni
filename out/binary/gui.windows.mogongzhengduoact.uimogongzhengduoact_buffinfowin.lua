...
def_class("UIMoGongZhengDuoAct_buffInfoWin",UIWindowBase)
...
function UIMoGongZhengDuoAct_buffInfoWin:bindComponents()
...
self.effectDesc=UIText.get(self,9)
self.effectTip=UIObject.get(self,10)
self.jijieBtn=UIButton.get(self,11)
self.jijieBtnText=UIText.get(self,12)
self.menuItemGroup=UIObject.get(self,13)
self.noLogTips=UIObject.get(self,14)
self.noRankTips=UIObject.get(self,15)
self.noTeamTips=UIObject.get(self,16)
self.rankPanel=UIObject.get(self,17)
self.rankScrollView=UIObject.get(self,18)
...
self.simpleRecordPanel=UIButton.get(self,23)
self.simpleRecordScrollView=UIObject.get(self,24)
self.sortConditionButton=UIButton.get(self,25)
self.teamContent=UIObject.get(self,26)
self.teamCountText=UIText.get(self,27)
self.teamListBtn=UIButton.get(self,28)
self.teamPanel=UIObject.get(self,29)
self.teamScrollView=UIObject.get(self,30)
self.xianyuName=UIText.get(self,31)
...
function UIMoGongZhengDuoAct_buffInfoWin:unbindComponents()
...
function UIMoGongZhengDuoAct_buffInfoWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UIMoGongZhengDuoAct_buffInfoWin:__delete()
_this=nil
...
function UIMoGongZhengDuoAct_buffInfoWin:onShow(argtable,afterOnloaded)
self.buildID=argtable and argtable.arenaId
...
self.selectMenuIndex=moGongZhengDuoActModel:getBuffBuildInfoWinSelectMenuIndex()or 1
end
self.isFromOvArenaWin=argtable and argtable.isFromOvArenaWin


moGongZhengDuoActController:reqGetMoGongActData_arenaInfo(self.buildID)

self.cfg=cfgHelper.get(cfg_fairylandclientbuildconfig_get,self.buildID)
...
function UIMoGongZhengDuoAct_buffInfoWin:onHide()

end


function UIMoGongZhengDuoAct_buffInfoWin:refresh(isInit)
...
local _offsetList={
[xjClientBuildType.flcbMGZDZhanHunGe1]={-40,-50},
[xjClientBuildType.flcbMGZDZhanHunGe2]={-40,-50},
[xjClientBuildType.flcbMGZDHuLingTa1]={-40,0},
[xjClientBuildType.flcbMGZDHuLingTa2]={-40,0},
}

function UIMoGongZhengDuoAct_buffInfoWin:refreshMiddlePanel()
...
local pos=_offsetList[self.buildID]
self.arenaModel:setChildAnchoredPos(pos[1],pos[2])
...
local arenaData=xianjieModel:getMGZDBuildServerData(self.buildID)or{}
...
local effectDesc=cfgHelper.get(cfg_mogongyibanjianzhugeconfig_get,self.buildID,'effect_desc')
self.effectDesc:setText(effectDesc)
end

function UIMoGongZhengDuoAct_buffInfoWin:refreshLeftPanel(isInit)
...
function UIMoGongZhengDuoAct_buffInfoWin:refreshRightPanel(isInit)
...
local isHasSelfZhuJun=moGongZhengDuoActModel:checkArenaHasSelfJJOrZJTeam(self.buildID)
local jjBtnStr="发起进攻"
local arenaData=xianjieModel:getMGZDBuildServerData(self.buildID)or{}
...
function UIMoGongZhengDuoAct_buffInfoWin:selectMenu(index)
if not index or self.selectMenuIndex==index then
return
end

self.selectMenuIndex=index
moGongZhengDuoActModel:setBuffBuildInfoWinSelectMenuIndex(index)
self:refreshRightPanel()
end

function UIMoGongZhengDuoAct_buffInfoWin:refreshRankPanel(isInit)
...
local arenaData=moGongZhengDuoActModel:getArenaBuildData(self.buildID)
...
function UIMoGongZhengDuoAct_buffInfoWin:refreshTeamPanel()
local zjList=moGongZhengDuoActModel:getArenaZhuJunList(self.buildID)or{}
local count=#zjList
local maxTeamCount=cfgHelper.get(cfg_mogongyibanjianzhugeconfig_get,self.buildID,"guardMax")
...
function UIMoGongZhengDuoAct_buffInfoWin:getArenaOccupySortListData(isReset)
if not isReset and self.arenaOccupySortList and not next(self.arenaOccupySortList)then
return self.arenaOccupySortList
end

local sortList={}
local arenaData=moGongZhengDuoActModel:getArenaBuildData(self.buildID)or{}
...
function UIMoGongZhengDuoAct_buffInfoWin:refreshSimpleRecordPanel(isInit)
...
descStr=FMT.fmt("<color=#549327>攻占{0}成功</color>",self.cfg.name)
elseif logType==2 then

descStr=FMT.fmt("<color=#c82c2c>攻占{0}失败</color>",self.cfg.name)
elseif logType==3 then
descStr="<color=#549327>首次占领</color>"
elseif logType==4 then
descStr="<color=#549327>援军到达</color>"
else
...
function UIMoGongZhengDuoAct_buffInfoWin:refreshDetailPanel(isInit)
...
function UIMoGongZhengDuoAct_buffInfoWin:getXJZmData(actorId)
local zmData
...
function UIMoGongZhengDuoAct_buffInfoWin:getLogCfgIdByLogData(logData,logType)
local logCfgId
...
function UIMoGongZhengDuoAct_buffInfoWin:getDetailLogDesc(logData,logCfgId)
...
local arenaCfg=cfgHelper.get(cfg_fairylandclientbuildconfig_get,self.buildID)
...
function UIMoGongZhengDuoAct_buffInfoWin:changeLogPanelShow(isShowDetail)
...
function UIMoGongZhengDuoAct_buffInfoWin:getArenaSimpleLogSortList(isReset)
...
local sortList={}
local logDataList=moGongZhengDuoActModel:getArenaLogList(self.buildID)or{}
local useTypeList={
[11]=true,
[12]=true,
...
if logData.defenseGuildName==""then
logType=3
else
logType=4
end
...
function UIMoGongZhengDuoAct_buffInfoWin:getArenaDetailLogSortList(isReset)
...
function UIMoGongZhengDuoAct_buffInfoWin:getArenaDetailLogFilterList(isInit)
...
function UIMoGongZhengDuoAct_buffInfoWin:getZhenYingTypeByActorId(actorId)
...
function UIMoGongZhengDuoAct_buffInfoWin:setUpdateTimer()
self:clearUpdateTimer()
...
function UIMoGongZhengDuoAct_buffInfoWin:clearUpdateTimer()
...
function UIMoGongZhengDuoAct_buffInfoWin:getFilterData(isReset)
...
function UIMoGongZhengDuoAct_buffInfoWin.selectConditionBack(data)
if _this==nil then
return
end
...
function UIMoGongZhengDuoAct_buffInfoWin:clearSearchInput()
self.inputstr=nil
local str=''
...
function UIMoGongZhengDuoAct_buffInfoWin:refreshInputBtns()
local showCancel=self.inputstr~=nil
self.searchCancelBtn:setActive(showCancel)
self.searchBtn:setActive(not showCancel)
end

function UIMoGongZhengDuoAct_buffInfoWin:onSearchChange(str)
self:refreshInputBtns(str)
end
...
function UIMoGongZhengDuoAct_buffInfoWin:onClickMask()
self:onCloseClick()
end



function UIMoGongZhengDuoAct_buffInfoWin:onCloseBtn()
self:onCloseClick()
end



function UIMoGongZhengDuoAct_buffInfoWin:onJijieBtn()
...
local arenaId=self.buildID
local arenaData=xianjieModel:getMGZDBuildDataByBuildID(arenaId)
...
local clientBuildCfg=cfgHelper.get1(cfg_fairylandclientbuildconfig_get,arenaId)


local buffBuildDataType=clientBuildCfg.clientParam.buffBuildDataType
local orderType=buffBuildDataType==xjDataType.eMGZD_ZhanHunGe and xjOrderType.eMoGongZhanHunGe or xjOrderType.eMoGongHuLingTa
...
local gridX=clientBuildCfg.x
local gridZ=clientBuildCfg.y
local sceneidx=clientBuildCfg.sceneidx
...
local arenaIdStr=tostring(_this.buildID)
local func=function(selectDzList,selectMoneyList,boatId)
local infoguid_str=arenaIdStr
local guid=int64.new(infoguid_str)
local ordertype=orderType
local params=''
xianjieController:reqOrder(guid,ordertype,selectDzList,selectMoneyList,params,boatId,nil,nil)
end

UIFullFightPrepareControl:showXJYunZhouBuZhenWindowEx({callback=func,extraCost=extraCost,wayTime=wayTime,orderType=orderType,targetFight=0})

end



function UIMoGongZhengDuoAct_buffInfoWin:onSimpleRecordPanel()
...
function UIMoGongZhengDuoAct_buffInfoWin:onSortConditionButton()
local filterName,filterFlag=self:getFilterData()
local args={}
...
function UIMoGongZhengDuoAct_buffInfoWin:onSearchBtn()
...
function UIMoGongZhengDuoAct_buffInfoWin:onSearchCancelBtn()
...
function UIMoGongZhengDuoAct_buffInfoWin:onCloseClick(atOnce)
...
xianjieController:closeWin('UIMoGongZhengDuoAct_buffInfoWin',atOnce)
end

function UIMoGongZhengDuoAct_buffInfoWin:onArrowBtn()
self:changeLogPanelShow(false)
end

function UIMoGongZhengDuoAct_buffInfoWin:onSoldierInfoBtnClick(widget,index,zsList)
...
function UIMoGongZhengDuoAct_buffInfoWin:onTeamListBtn()

local win=UIManager:findActiveWindow("UIXianJieExtra_MJZDWin")
if win and win.isVisible then
win:onZhanKuangBtn(1,{buildID=self.buildID})
else
UIManager.error("活动已结束")
end
end

function UIMoGongZhengDuoAct_buffInfoWin:onDetailPanelMask()
return self:onArrowBtn()
end

function UIMoGongZhengDuoAct_buffInfoWin:onXjbjbtn()
