...
def_class("xzsChildTYCYChallenge",UICloneObject)





xzsChildTYCYChallenge.abName="ui/windows/xiaozhushou/child/xzschildtycychallenge.ab"

xzsChildTYCYChallenge.assetName="xzsChildTYCYChallenge"


function xzsChildTYCYChallenge:bindComponents()
...
function xzsChildTYCYChallenge:unbindComponents()
...
function xzsChildTYCYChallenge:onLoaded(...)
self:bindComponents()
end


function xzsChildTYCYChallenge:__delete()
...
function xzsChildTYCYChallenge:onShow(argtable,afterOnloaded)
...
function xzsChildTYCYChallenge:startFightTimer()
...
function xzsChildTYCYChallenge:stopFightTimer()
...
function xzsChildTYCYChallenge:onFinishErr()
...
function xzsChildTYCYChallenge:autoChallenge(delay)
...
local oldTotalDamage=worldLeaderModel:getTotalDamage()or 0
local oldChallengeTimes=worldLeaderModel:getChallengeTimes()or 0
local lastDamage=self.hurt
local totalDamage=lastDamage+oldTotalDamage
worldLeaderController.recv_248_33(lastDamage,totalDamage,oldChallengeTimes+1)
self:stopFightTimer()
self:updateDetailProgress(1,1,3.6)
self.isFighting=false
fightModel:setSendExtraArgs(eBattleType.worldLeader,nil)
end
local waitTime=worldLeaderController:tycySkipFight(completeCall)
if waitTime<=0 then
self.isFighting=true
end
end

function xzsChildTYCYChallenge:updateDetailProgress(target,max,duration)
if target>=max then
self.progress:setFinishAction(function()
self.progress:setActive(false)
self:refreshRewards()
self:refreshCostPanel()
worldLeaderController:tycyAutoFight(self.orderID)
end)
end
self.progress:animateThreeParams(target,max,duration)
end

function xzsChildTYCYChallenge:refreshRewards()
...
local str=string.format("挑战1次妖物，累计伤害<color=#CA631D>%s</color>，排第<color=#CA631D>%d</color>名，获得以下奖励：",hurtStr,self.rank)
if self.buyNum then
...
function xzsChildTYCYChallenge:refreshCostPanel()
