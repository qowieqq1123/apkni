...
def_class("UICommonShowPrizeWin",UIWindowBase)
...
function UICommonShowPrizeWin:bindComponents()
...
function UICommonShowPrizeWin:unbindComponents()
...
function UICommonShowPrizeWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UICommonShowPrizeWin.onItemUse(itemid,num)
...
function UICommonShowPrizeWin:onHide()

end

function UICommonShowPrizeWin:__delete()
_this=nil
...
function UICommonShowPrizeWin:onShow(argtable,afterOnloaded)
self.curStep=1
self:stopAllTimer()
...
logErr('没有奖励却打开了UICommonShowPrizeWin?')
end
...
local autoCloseDelayTime=argtable.autoCloseDelayTime
if autoCloseDelayTime then
self:delayDo(autoCloseDelayTime,function()
if _this==nil then return end
self:closeSelf()
end)
end
...
function UICommonShowPrizeWin:setAttachCB(callback)
self.attachCallback=callback
end

function UICommonShowPrizeWin:initRewardView()
...
function UICommonShowPrizeWin:refreshAllPos()
...
function UICommonShowPrizeWin:refreswhScrollView()
...
function UICommonShowPrizeWin:refreshItemPos(item,idx)
...
function UICommonShowPrizeWin:refreshItem(item,widget)
...
stage=stage and FMT.fmt('仙·{0}',stage)or''
porp[PropIndex(DataPropKey.eWidgetText,6)]=stage
elseif isxmEquip==EQUIP_XianMo_TYPES.eMo then
stage=stage and FMT.fmt('魔·{0}',stage)or''
porp[PropIndex(DataPropKey.eWidgetText,6)]=stage
end
end
...
function UICommonShowPrizeWin:playShow()
local delay=0
local d=0.1
local num=self.rewardnum
local grids=self.goodGridPanel:getChildLayoutGroupGridList()
...
function UICommonShowPrizeWin:playEffect()
...
function UICommonShowPrizeWin:replay(itemid_,num)
local index=self.curStep
...
function UICommonShowPrizeWin:playEffectEnd()
...
function UICommonShowPrizeWin:onFinish()
...
function UICommonShowPrizeWin:addNote(str)
table.insert(self.noteslist,1,str)
self:refreshNotes()
end

function UICommonShowPrizeWin:refreshNotes()
...
function UICommonShowPrizeWin:onClickItem(itemId,index,itemguid,attach)
itemsComponentHelper.onItemClickEx(itemId,index,itemguid,attach)
end

function UICommonShowPrizeWin:onClickClose()
