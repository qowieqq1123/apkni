...
def_class("UISubAct_dropActTwoWin",UIWindowBase)
...
function UISubAct_dropActTwoWin:bindComponents()
...
self.baoXiangBtn=UIButton.get(self,6)
self.baoXiangReddot=UIObject.get(self,7)
self.timeText=UIText.get(self,8)
self.ruleBg=UIImage.get(self,9)
self.ruleTitle=UIText.get(self,10)
self.ruleText=UIText.get(self,11)
self.dropItemList=UIObject.get(self,12)
self.jumpText=UIText.get(self,13)
self.bannerImg=UIImage.get(self,14)
...
function UISubAct_dropActTwoWin:unbindComponents()
...
function UISubAct_dropActTwoWin:onLoaded(...)
self:bindComponents()
end


function UISubAct_dropActTwoWin:__delete()
self:clearTimer()
...
function UISubAct_dropActTwoWin:onShow(argtable,afterOnloaded)
if argtable then
...
function UISubAct_dropActTwoWin:onHide()
self:clearTimer()
if self.moneyWinName then
self:closeWindow(self.moneyWinName)
end
...
function UISubAct_dropActTwoWin:refresh()
...
local showItemList=self.config.showItemList
self.dropItemList:setChildLayoutGroupCreateItems(#showItemList,function(i)
local itemWidget=self.dropItemList:getChildLayoutGroupGridItem(i-1)
local itemid=showItemList[i]
if itemid then
local countStr=''
...
self.baoXiangBtn:setActive(not isOpenBaoxiang)
...
function UISubAct_dropActTwoWin:onJumpBtn()
...
function UISubAct_dropActTwoWin:onBaoXiangBtn()

activitiesController:sendProtocol(actSendType.eComonReqHandle,self.activityId,self.subType,self.subId,"")
end

function UISubAct_dropActTwoWin:onClickRewardItem(itemId,index,guid,attach)
if itemId==-1 then
return
end
...
function UISubAct_dropActTwoWin:setRemainingTimeTimer()
self:clearTimer()
local func=function()
...
function UISubAct_dropActTwoWin:clearTimer()
if self.timer then
