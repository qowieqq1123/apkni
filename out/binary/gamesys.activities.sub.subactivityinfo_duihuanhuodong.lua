...
local subActivityInfo_duihuanhuodong={name='duihuanhuodong'}

function subActivityInfo_duihuanhuodong:onInit()
local itemLookup={}
...
function subActivityInfo_duihuanhuodong:onStart()
...
function subActivityInfo_duihuanhuodong:on_money_changed(moneyType,lastVal,val)
...
function subActivityInfo_duihuanhuodong:on_item_changed(changeType,itemguid,itemid,oldcount,newcount)
if self.itemLookup then
...
function subActivityInfo_duihuanhuodong:onUpdate()

end

function subActivityInfo_duihuanhuodong:onDelete()
self.itemLookup=nil
end

function subActivityInfo_duihuanhuodong:checkReddot()
...
function subActivityInfo_duihuanhuodong:checkNewDay()
...
function subActivityInfo_duihuanhuodong:getGoodBuyNum(idx)
local data=self.data
if data then
...
function subActivityInfo_duihuanhuodong:judeisnewItem(recorddata,idx)
...
function subActivityInfo_duihuanhuodong:GetShowList()
local _key=FMT.fmt('actid{0}_subtype{1}_subid{2}',self.act_id,self.sub_act_type,self.sub_act_id)
local recorddata=userActorArraySetting.get(ACTOR_SETTING_TYPE.eDuiHuanHuoDong,_key,{})
local exchange=self:getSubActConfig('exchange')
local shopGoodList={}
...
function subActivityInfo_duihuanhuodong:handelData(data)
...
return subActivityInfo_duihuanhuodong