...
def_class("UIRankListTop3Win_3",UIWindowBase)
...
function UIRankListTop3Win_3:bindComponents()
...
function UIRankListTop3Win_3:unbindComponents()
...
function UIRankListTop3Win_3:onLoaded(...)
self:bindComponents()
_this=self
...
function UIRankListTop3Win_3:__delete()
...
function UIRankListTop3Win_3:onShow(argtable,afterOnloaded)
...
function UIRankListTop3Win_3:onHide()

end



function UIRankListTop3Win_3:onClickHelpBtn()
...
function UIRankListTop3Win_3:onClickDetailBtn()
local args={
...
function UIRankListTop3Win_3:onClickTab(index)
...
function UIRankListTop3Win_3:onClickPlayer(index)
...
function UIRankListTop3Win_3:refreshTabList()
...
function UIRankListTop3Win_3:initPanel()
for index,widget in ipairs(self.playerWidget)do
widget:SetChildCSImageSprite(_playerCmp.full,_ab,self.args.fullSp)
end
self.ownerWidget:SetChildCSImageSprite(_ownerCmp.full,_ab,self.args.fullSp)
end

function UIRankListTop3Win_3:refreshPanel()
...
function UIRankListTop3Win_3:refreshPlayer(index)
...
local showProgress=self.args.fullSp==nil or rankData.percent<100
widget:SetChildActive(_playerCmp.progress,showProgress)
widget:SetChildActive(_playerCmp.full,not showProgress)
if showProgress then
widget:SetChildProgressValue(_playerCmp.progress,rankData.percent,100)
widget:SetChildProgressText(_playerCmp.progress,FMT.fmt("{0}%",math.floor(rankData.percent*10)/10))
end
end
end

function UIRankListTop3Win_3:refeshOwner(rankType)
...
local percent=self.onwerData.percent or 0
local showProgress=self.args.fullSp==nil or percent<100
self.ownerWidget:SetChildActive(_ownerCmp.progress,showProgress)
self.ownerWidget:SetChildActive(_ownerCmp.full,not showProgress)
if showProgress then
self.ownerWidget:SetChildProgressValue(_ownerCmp.progress,percent,100)
self.ownerWidget:SetChildProgressText(_ownerCmp.progress,FMT.fmt("{0}%",math.floor(percent*10)/10))
end
end

function UIRankListTop3Win_3.onRankListRefresh(rankType)
