...
platformSDK_PC_Flash=simple_class(platformSDK_None)

local cjson=require'cjson'

local _WindowsHelper=CS.FlashPCHelper
...
[CHAT_CHANNNEL.eJianwen]=sdkchannel.other,
...
function platformSDK_PC_Flash:__init(...)
...
function platformSDK_PC_Flash:reqLogin()
_WindowsHelper.ExecCmd("reqLogin","")
end


function platformSDK_PC_Flash:reqLogout()
_WindowsHelper.ExecCmd("reqLogout","")
end



function platformSDK_PC_Flash:reqPay(id,count,params,subscribe)
...
function platformSDK_PC_Flash:reqReport(typo,info)
...
function platformSDK_PC_Flash:reqFangChenMi()

end


function platformSDK_PC_Flash:onPay(json,jsonStr)
...
function platformSDK_PC_Flash:LoginCallBack(json,jsonStr)
...
function platformSDK_PC_Flash:LogoutCallBack(json,jsonStr)
...
function platformSDK_PC_Flash:PurchasePaymentResult()

end


function platformSDK_PC_Flash:OnApplicationQuit()
local showdata=
{
type='UIDialouge',
title='温馨提示',
...
function platformSDK_PC_Flash:reqApplicationQuit_PC()
_WindowsHelper.ExecCmd("setQuitState","quit")
platformHelper:exitGame()
end


function platformSDK_PC_Flash:reqMsgSecCheck(scene,content,callback,to_PrivatePlayerData)
...
info.guild_id="0"
if chatModel.channelId and chatModel.channelId==CHAT_CHANNNEL.eXianmeng then
...
function platformSDK_PC_Flash:onreMsgSecCheck(json,jsonStr)
...
function platformSDK_PC_Flash:Chitchat(content)
local playerinfo=platformHelper:getPlayerInfo()
local info={}
info=platformHelper.concat(info,playerinfo)
