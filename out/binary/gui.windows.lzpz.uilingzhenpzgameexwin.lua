...
def_class("UILingZhenPZGameExWin",UIWindowBase)
...
function UILingZhenPZGameExWin:bindComponents()
...
function UILingZhenPZGameExWin:unbindComponents()
...
function UILingZhenPZGameExWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UILingZhenPZGameExWin:__delete()
...
lingZhenPengZhuangModel:setLingZhenPengZhuangGameData(createList,itemList,combineList,selectQueue,recordList,skillCount,score,mapExpireTime,selectMapId)
end
else
lingZhenPengZhuangModel:clearLingZhenPengZhuangGameData()
end
...
function UILingZhenPZGameExWin:onShow(argtable,afterOnloaded)
self.actID=LIMIT_ACT_TYPE.eLingZhenPengZhuang

self.selectMapId,self.mapExpireTime=lingZhenPengZhuangModel:getTodayMapData()
...
local npcSpeakParam=cfgHelper.get(cfg_lingzhenpengzhuangconfig_get,1,"catSpeakParam")
...
self.mapCfg=cfgHelper.get(cfg_lingzhenpengzhuangmapconfig_get,self.selectMapId)
self.mapIconNameList=cfgHelper.get(cfg_lingzhenpengzhuangconfig_get,1,"mapIconName")
...
local nowTopScore=lingZhenPengZhuangModel:getTopScore()
local score=self.score:getValue()or 0
...
function UILingZhenPZGameExWin:onHide()
self:clearComboModeTimer()
self:clearTimer()
self:clearSpeakTimer()
end

function UILingZhenPZGameExWin:initData()
local list={}
...
function UILingZhenPZGameExWin:countPos(index,layer,needCountMinPos)
...
function UILingZhenPZGameExWin:setCreateData()
...
function UILingZhenPZGameExWin:countTopLeft()
local x,y=self:countTopLeftByData(self.layer)
self.rectMinX=x
self.rectMinY=y
end

function UILingZhenPZGameExWin:countTopLeftByData(layer)
local layerData=self.mapData[layer]
local col=layerData.col
...
function UILingZhenPZGameExWin:getNextIndex()
...
function UILingZhenPZGameExWin:createItemList()
self.isPlaying=true
...
local saveGameData=lingZhenPengZhuangModel:getLingZhenPengZhuangGameData()
...
function UILingZhenPZGameExWin:clearTimer()
...
function UILingZhenPZGameExWin:addToItemList(data,checkStack)
...
function UILingZhenPZGameExWin:removeFormItemList(data,checkStack)
...
function UILingZhenPZGameExWin:setAllItemStackUp()
...
function UILingZhenPZGameExWin:setChangeItemStackUp(data,checkAdd)
...
function UILingZhenPZGameExWin:checkStackUp(dataA,dataB)
...
function UILingZhenPZGameExWin:checkStackUp_allUpLayer(data)
...
function UILingZhenPZGameExWin:checkHasUnderItem(data)
local itemLayer=data.layer
local itemX=data.x
local itemY=data.y
...
function UILingZhenPZGameExWin:handleItemClick(id)
...
function UILingZhenPZGameExWin:addToCombineList(data)
self.isPlaying=true
...
function UILingZhenPZGameExWin:playMoveAnim(index,callback)
...
function UILingZhenPZGameExWin:checkAndCombine()
local rval=-1
local index=-1
local count=0
for i,v in ipairs(self.combineList)do
...
function UILingZhenPZGameExWin:addScore(score)
self:setScore(self.score:getValue()+score)
end

function UILingZhenPZGameExWin:enterComboMode()
...
function UILingZhenPZGameExWin:clearComboModeTimer()
...
function UILingZhenPZGameExWin:clearComboMode()
self:clearComboModeTimer()
self.isComboMode=false
end

function UILingZhenPZGameExWin:setScore(score)
...
function UILingZhenPZGameExWin:addToQueue(data)
table.insert(self.selectQueue,1,data)
end

function UILingZhenPZGameExWin:removeFormQueue(id)
...
function UILingZhenPZGameExWin:pushToPool(id,widget)
widget:SetChildActive(-1,false)
self.itemPool[id]=widget
end

function UILingZhenPZGameExWin:getItemFromPool()
...
function UILingZhenPZGameExWin:handleSuccess()

self:clearComboMode()
local score=self.score:getValue()
local originalTopScore=lingZhenPengZhuangModel:getTopScore()or 0
self.ignoreSave=true
lingZhenPengZhuangController:reqSetScore(score)
local callback=function()
UILZPZControl:showMainWin()
if not _this then return end
self:closeSelf()
end
UIManager:showWindow('UILZPZResultWin',{success=true,score=score,originalTopScore=originalTopScore,callback=callback})
end

function UILingZhenPZGameExWin:handleFailure()
self:clearComboMode()
local count=self.skillCount[4]
local skillId=4
local cfg=cfgHelper.get1(cfg_lzpzskillinfoconfig_get,skillId)
...
function UILingZhenPZGameExWin:showFailureWin()
...
UIManager:showWindow('UILZPZResultWin',{success=false,score=score,originalTopScore=originalTopScore,callback=callback})
end

function UILingZhenPZGameExWin:showSkillWin(id)

self:showWindow('UILZPZSkillWin',{id=id,count=self.skillCount[id]})

end

function UILingZhenPZGameExWin:handleSkill(id)
...
function UILingZhenPZGameExWin:checkNeedHandleSkillList()
...
function UILingZhenPZGameExWin:refreshSkillBtn()
...
function UILingZhenPZGameExWin:removeItem(removeCount,isLast)
local list={}
local len=#self.combineList
...
function UILingZhenPZGameExWin:removeSingleItemById(targetId)
local list={}
local len=#self.combineList
for i=1,len do
...
function UILingZhenPZGameExWin:recaptionItem(data)
...
function UILingZhenPZGameExWin:revocationItem()
...
function UILingZhenPZGameExWin:refreshRecordScrollView()
...
function UILingZhenPZGameExWin:randomItemList()
local list={}
...
function UILingZhenPZGameExWin:redealItemList()
self:clearTimer()
...
function UILingZhenPZGameExWin:clearCombineList()
...
function UILingZhenPZGameExWin:initAddScoreShow()
local maxScoreItemCount=10
self.scoreArea:setChildLayoutGroupCreateItems(maxScoreItemCount)
end


function UILingZhenPZGameExWin:addScoreShow(score)
...
function UILingZhenPZGameExWin:getScoreItemIndex()
...
function UILingZhenPZGameExWin:setExpireTimer()
self:clearExpireTimer()
local func=function()
...
endContent="灵阵绘画已结算"
clickCallback=function()
return UILZPZControl:closeUI(true)
end
end
...
function UILingZhenPZGameExWin:clearExpireTimer()
...
function UILingZhenPZGameExWin:doSpeaking()
self:clearSpeakTimer()
...
local speakLib=cfgHelper.get(cfg_lingzhenpengzhuangconfig_get,1,"catSpeakLib")
local specialSpeakLib=cfgHelper.get(cfg_lingzhenpengzhuangconfig_get,1,"catSpecialSpeakLib")
...
function UILingZhenPZGameExWin:doTalkAnim()
...
function UILingZhenPZGameExWin:talkEnd()
self:clearSpeakTimer()
...
function UILingZhenPZGameExWin:clearSpeakTimer()
...
function UILingZhenPZGameExWin:onHelpBtn()
...
function UILingZhenPZGameExWin:onSkillBtnC()
if self.isCreatingMap or self.isInAnim then
...
function UILingZhenPZGameExWin:onSkillBtnB()
if self.isCreatingMap or self.isInAnim then
...
function UILingZhenPZGameExWin:onSkillBtnA()
if self.isCreatingMap or self.isInAnim then
...
function UILingZhenPZGameExWin:onCloseClick()
...
function UILingZhenPZGameExWin:onCloseBtn()
self:onCloseClick()
end

function UILingZhenPZGameExWin:onBottomClick()
...
function UILingZhenPZGameExWin:test_printSelectMapId()
UIManager.info(FMT.fmt("当前地图id：{0}",self.selectMapId))
end


function UILingZhenPZGameExWin:test_finishGame(score)
self.score:setValue(score)
self:handleSuccess()
end
