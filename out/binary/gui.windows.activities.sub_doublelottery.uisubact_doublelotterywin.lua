...
def_class("UISubAct_doubleLotteryWin",UIWindowBase)
...
function UISubAct_doubleLotteryWin:bindComponents()
...
function UISubAct_doubleLotteryWin:unbindComponents()
...
function UISubAct_doubleLotteryWin:onLoaded(...)
_this=self
self:bindComponents()
self.isToggle=userActorSetting.get('skipDoubleLotteryAnim',false)
...
function UISubAct_doubleLotteryWin:__delete()
self:clearTimer()
...
function UISubAct_doubleLotteryWin:onShow(argtable,afterOnloaded)
if argtable then
...
function UISubAct_doubleLotteryWin:onHide()
self:clearTimer()
self:clearAllFMTweener()
self.rewardView:setActive(false)
end

function UISubAct_doubleLotteryWin:refresh(isInit,isFromShowFun)
...
function UISubAct_doubleLotteryWin:refreshBoxPanel(boxIndex,isInit)
...
function UISubAct_doubleLotteryWin:getBoxPanelWidgetBase(boxIndex)
local boxWidget
if boxIndex==1 then
...
function UISubAct_doubleLotteryWin:selectBox(boxIndex,isInit)
...
function UISubAct_doubleLotteryWin:refreshDrawBtn()
...
function UISubAct_doubleLotteryWin:refreshAllMoney()
for i=1,2 do
self:refreshMoneyByBoxIndex(i)
end
end

function UISubAct_doubleLotteryWin:refreshMoneyByBoxIndex(boxIndex)
local widget
if boxIndex==1 then
...
function UISubAct_doubleLotteryWin:refreshMoneyValueByBoxIndex(boxIndex,lastVal)
local widget
if boxIndex==1 then
widget=self.moneyRootType1:getWidgetBase()
elseif boxIndex==2 then
...
function UISubAct_doubleLotteryWin:refreshShowReward()
...
function UISubAct_doubleLotteryWin:getDefaultBoxIndex()
...
function UISubAct_doubleLotteryWin:clearFMTweenerByIndex(index)
...
function UISubAct_doubleLotteryWin:clearAllFMTweener()
...
function UISubAct_doubleLotteryWin:playLotteryAnim(maxItemColor,boxIndex,callback)
...
function UISubAct_doubleLotteryWin:setRemainingTimeTimer()
self:clearTimer()
local func=function()
...
function UISubAct_doubleLotteryWin:clearTimer()
if self.timer then
...
function UISubAct_doubleLotteryWin:onItemListChanged(argsTable)
...
function UISubAct_doubleLotteryWin:onItemChanged(changeType,itemguid,itemid,lastcount,itemcount)
...
function UISubAct_doubleLotteryWin:onMoneyChanged(moneyType,lastVal,val)
...
function UISubAct_doubleLotteryWin:freshToggle(isToggle)
self.jumpAnimToggle:setToggle(isToggle)
end

function UISubAct_doubleLotteryWin:onToggleChanged(name,isToggle,data)
if self.isToggle==isToggle then return end
self.isToggle=isToggle
userActorSetting.flushVal('skipDoubleLotteryAnim',isToggle)
self:freshToggle(isToggle)
end


function UISubAct_doubleLotteryWin:onDrawBtnOne(boxIndex)
...
function UISubAct_doubleLotteryWin:onDrawBtnMany(boxIndex)
if not boxIndex then
boxIndex=self.selectBoxIndex
end
...
function UISubAct_doubleLotteryWin:onMoneyBtnType1()
local boxIndex=1
local costCfg=self.config.box_cost_item[boxIndex]
local moneyType=costCfg[1]
...
function UISubAct_doubleLotteryWin:onMoneyBtnType2()
local boxIndex=2
local costCfg=self.config.box_cost_item[boxIndex]
local moneyType=costCfg[1]
self:onAddClick(moneyType)
...
function UISubAct_doubleLotteryWin:onTargetRewardBtn()
self:showWindow('UISubAct_doubleLottery_TargetWin',{act_id=self.activityId,sub_act_type=self.subType,sub_act_id=self.subId})
end



function UISubAct_doubleLotteryWin:onProbabilityBtn()
local percentCfgList=self.config.probabilityShow
self:showWindow('UISubAct_doubleLottery_PercentWin',{percentList=percentCfgList})
end

function UISubAct_doubleLotteryWin:onClickBoxPanel(boxIndex)
self:selectBox(boxIndex)
end

function UISubAct_doubleLotteryWin:onAddClick(moneyType)
gainControl:showCommonGainWin_item(moneyType)
end


function UISubAct_doubleLotteryWin:onClickRewardItem(itemId,index,guid,attach)
if itemId==-1 then
return
end
...
function UISubAct_doubleLotteryWin.test_showLotteryAnim(color)
_this:playLotteryAnim(color or 3)
end

function UISubAct_doubleLotteryWin:onClickProbability(boxIndex)
...
function UISubAct_doubleLotteryWin:onProbabilityRoot()
