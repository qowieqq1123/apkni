...
def_class("UIXJ_RPCtOnlyWin",UIWindowBase)
...
function UIXJ_RPCtOnlyWin:bindComponents()
...
self.rankItem=UIObject.get(self,6)
self.spinebg=UIObject.get(self,7)
self.closebtn=UIButton.get(self,8)
self.spinebg2=UIObject.get(self,9)
...
function UIXJ_RPCtOnlyWin:unbindComponents()
...
function UIXJ_RPCtOnlyWin:onLoaded(...)
self:bindComponents()
...
function UIXJ_RPCtOnlyWin:__delete()
...
function UIXJ_RPCtOnlyWin:onShow(argtable,afterOnloaded)
...
function UIXJ_RPCtOnlyWin:onHide()

end
function UIXJ_RPCtOnlyWin:onClickClose()
self:closeSelf()
end

function UIXJ_RPCtOnlyWin:onClickItem(itemId,index,guid,attach)
tipsManager.showTips({itemid=itemId,itemguid=nil})
end

function UIXJ_RPCtOnlyWin:onSjbtn()
...
function UIXJ_RPCtOnlyWin:onchoosebtn(index)
...
function UIXJ_RPCtOnlyWin.on_item_changed(changeType,itemguid,itemid,lastcount,itemcount)
if _this==nil then
return
end
_this:freshinfo()
...
function UIXJ_RPCtOnlyWin.onMoneyChange(moneyType,lastVal,val)
if _this==nil then
return
end
_this:freshinfo()
...
function UIXJ_RPCtOnlyWin:showTopMoney(datas)
if not datas then
return
end
local mlist={}
...
function UIXJ_RPCtOnlyWin:freshinfo()

self.desc:setText(self.cfg.npcdesc2[2]or"")
...
self.selectid=1
local item=self.rankItem:getWidgetBase()
...
self.isenorgh[1]=isengough

self:showTopMoney(choicedata)
end
