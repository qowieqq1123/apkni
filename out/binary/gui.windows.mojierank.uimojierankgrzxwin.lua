...
def_class("UIMoJieRankGRZXWin",UIWindowBase)
...
function UIMoJieRankGRZXWin:bindComponents()
...
function UIMoJieRankGRZXWin:unbindComponents()
...
function UIMoJieRankGRZXWin:onLoaded(...)
self:bindComponents()
_this=self
end


function UIMoJieRankGRZXWin:__delete()
self:unbindComponents()
_this=nil
end


function UIMoJieRankGRZXWin:onClickRewardItem(itemId,index,guid,attach)
if itemId==-1 then return end
tipsManager.showTips({itemid=itemId,itemguid=guid,move=TIPS_MOVE_POS.eRight})
end


function UIMoJieRankGRZXWin:severmkrankfresh(rankType)
if _this then
...
function UIMoJieRankGRZXWin:onShow(argtable,afterOnloaded)
...
function UIMoJieRankGRZXWin:onHide()
self.center:setActive(false)
self.norank:setActive(false)
end


function UIMoJieRankGRZXWin:getTypeReward(flag)
local cfgrank=self.mojiecfg.rank
return cfgrank[flag]
end

function UIMoJieRankGRZXWin:getRewardByRank(cfg,rank)
if cfg then
...
function UIMoJieRankGRZXWin:refreshRankList()
...
function UIMoJieRankGRZXWin:onFreshAction(i,grid)
...
function UIMoJieRankGRZXWin:onStartAction()

end

function UIMoJieRankGRZXWin:frsehSelfRank()
local widget=self.selfitem:getWidgetBase()
local myRank=xianjieController:getmojieMyRank(_eRankListType.xf)
local rankDatas=self.rankDatas
local cfgrewards=self.cfgrewards
local Faxfval=xianjieController:getXianFaxfval()or 0
...
local val=mathHelper.formatNumber4(tonumber(tostring(Faxfval)),2)
...
local num=tonumber(tostring(Faxfval))
