...
def_class("UISubAct_ZhenXinHuaQiXiWin",UIWindowBase)
...
function UISubAct_ZhenXinHuaQiXiWin:bindComponents()
...
self.startBtn=UIButton.get(self,10)
self.startReddot=UIObject.get(self,11)
self.timeTxt=UIText.get(self,12)
self.topDesc=UIText.get(self,13)
...
function UISubAct_ZhenXinHuaQiXiWin:unbindComponents()
...
function UISubAct_ZhenXinHuaQiXiWin:onLoaded(...)
self:bindComponents()
_this=self
end


function UISubAct_ZhenXinHuaQiXiWin:__delete()
...
function UISubAct_ZhenXinHuaQiXiWin:onShow(argtable,afterOnloaded)
...
self.mbg:setChildUIModelShowTarget(6379,1,nil,eAnimationID.stand,false,false,0,function()
...
function UISubAct_ZhenXinHuaQiXiWin:onHide()

end

function UISubAct_ZhenXinHuaQiXiWin:refreshAll()
self:refreshInfo()
self:refreshReward()
...
function UISubAct_ZhenXinHuaQiXiWin:refreshRecordReddot()
...
function UISubAct_ZhenXinHuaQiXiWin:refreshInfo()
local curDay=self.sub_actInfo:getCurDay()
self.recordBtn:setActive(curDay>1)
end

function UISubAct_ZhenXinHuaQiXiWin:refreshReward()
local curDay=self.sub_actInfo:getCurDay()
...
function UISubAct_ZhenXinHuaQiXiWin:refreshProgress(isRecv)
local curDay=self.sub_actInfo:getCurDay()
local ques=self.sub_actcfg.ques_list[curDay]
if not ques then
...
function UISubAct_ZhenXinHuaQiXiWin:refreshActivityTime()
self:clearTimer()
local func
func=function()
...
function UISubAct_ZhenXinHuaQiXiWin:clearTimer()
if self.timer then
...
function UISubAct_ZhenXinHuaQiXiWin:doPunchRotation(widget,componentIndex,reddotIndex,isReddot)
...
function UISubAct_ZhenXinHuaQiXiWin:endAllReddotPunchRotation()
if self.reddotTweenerList==nil or not next(self.reddotTweenerList)then
return
end
...
function UISubAct_ZhenXinHuaQiXiWin:onRankBtn()
local args={}
...
function UISubAct_ZhenXinHuaQiXiWin:onRecordBtn()
local curDay=self.sub_actInfo:getCurDay()
local args={}
args.act_id=self.actID
args.sub_act_type=self.subType
args.sub_act_id=self.subid
args.day=curDay-1
self:showWindow("UISubAct_ZhenXinHuaQiXiDaTiTipsWin",args)
...
function UISubAct_ZhenXinHuaQiXiWin:onStartBtn()
local curDay=self.sub_actInfo:getCurDay()
local args={}
args.act_id=self.actID
args.sub_act_type=self.subType
...
function UISubAct_ZhenXinHuaQiXiWin:onRuleBtn()
local d={}
d.title='规则'
d.mode=3
d.name='ZhenXinHuaQiXi_rule_%d'
d.showBlack=true
