...
def_class("UILiLianTouZiWin",UIWindowBase)
...
function UILiLianTouZiWin:bindComponents()
...
function UILiLianTouZiWin:unbindComponents()
...
function UILiLianTouZiWin:onLoaded(...)
self:bindComponents()
...
function UILiLianTouZiWin:__delete()
...
function UILiLianTouZiWin:onShow(argtable,afterOnloaded)
self.investData=UILiLianControl:getInvestData()
self.receiveList={}
...
function UILiLianTouZiWin:refresh()
self:setRewardList()
self:setSPRewards()
self:setTZValue()
end
...
function UILiLianTouZiWin:onHide()

end

function UILiLianTouZiWin:showNPC()
...
function UILiLianTouZiWin:setTZValue()
local lcfg=cfgHelper.get1(cfg_guanqiainvestconfig_get,self.investData.rechargeId)
...
function UILiLianTouZiWin:setSPRewards()
local lcfg=cfgHelper.get1(cfg_guanqiainvestconfig_get,self.investData.rechargeId)
...
function UILiLianTouZiWin:setRewardList()
local levelRewards=UILiLianControl:getTZRewardList(self.investData.rechargeId)
...
item:SetChildText(_item_index.level,UILiLianControl:getLevelName('关卡：',data.level))
self:setLevelReward(item,data.rwId)
if unlock then
item:SetChildActive(_item_index.tips,false)
local complete=UILiLianControl:isLevelComplete(data.level)
if complete then
...
function UILiLianTouZiWin:setLevelReward(item,rwId)
local rwcfg=cfgHelper.get1(cfg_awardconfig_get,rwId)
local rewards=rwcfg.showItems
...
function UILiLianTouZiWin:receiveAll()
for k,v in pairs(self.receiveList)do
UILiLianControl:setTZRewardCheck(k)
UILiLianControl:reqReceiveInvest(self.investData.rechargeId,k)
end
self.receiveList={}
end

function UILiLianTouZiWin:onPayBtn()
...
function UILiLianTouZiWin:onCloseClick()
self:closeSelf()
end