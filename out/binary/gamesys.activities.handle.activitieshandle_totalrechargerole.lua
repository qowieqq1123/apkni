...
activitiesHandle_totalRechargeRole=new_activitiesHandle('activitiesHandle_totalRechargeRole',activitiesHandle)

function activitiesHandle_totalRechargeRole:onInit()

end

function activitiesHandle_totalRechargeRole.recv_249_213(args)
local actId,subId,totalRechargeNum,rewardFlag,len,zxlist=unpack(args)

local subType=SUB_ACTIVITY_TYPE.eRechargeAct1Role
...
function activitiesHandle_totalRechargeRole.recv_249_214(actId,subId,id,len,idxList)
if idxList then
local subType=SUB_ACTIVITY_TYPE.eRechargeAct1Role
local data=activitiesModel:getSubActInfoData(actId,subType,subId)
