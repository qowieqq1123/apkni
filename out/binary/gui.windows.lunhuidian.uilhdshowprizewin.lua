...
def_class("UILHDShowPrizeWin",UIWindowBase)
...
function UILHDShowPrizeWin:bindComponents()
...
function UILHDShowPrizeWin:unbindComponents()
...
function UILHDShowPrizeWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UILHDShowPrizeWin:__delete()
_this=nil
...
function UILHDShowPrizeWin:onShow(argtable,afterOnloaded)
self.curStep=1
self:stopAllTimer()
...
logErr('没有奖励却打开了UILHDShowPrizeWin?')
end
...
function UILHDShowPrizeWin.onItemUse(itemid,num)
if _this==nil then return end
...
function UILHDShowPrizeWin:onHide()

end

function UILHDShowPrizeWin:setAttachCB(callback)
self.attachCallback=callback
end

function UILHDShowPrizeWin:initRewardView()
...
function UILHDShowPrizeWin:refreshAllPos()
...
function UILHDShowPrizeWin:refreswhScrollView()
...
if rowNum<=1 then
flag=false
self.goodScrollView:setChildAnchoredPos(self.scrollViewPos.x,100)
self.goodScrollView:setChildSizeDelta(self.scrollViewWidth,200)
else
self.goodScrollView:setChildAnchoredPos(self.scrollViewPos.x,160)
self.goodScrollView:setChildSizeDelta(self.scrollViewWidth,256)
end
self.goodScrollView:setChildScrollRectEnable(flag)
end

function UILHDShowPrizeWin:refreshItemPos(item,idx)
...
function UILHDShowPrizeWin:refreshItem(item,widget)
...
function UILHDShowPrizeWin:playShow()
local delay=0
local d=0.1
local num=self.rewardnum
local grids=self.goodGridPanel:getChildLayoutGroupGridList()
...
function UILHDShowPrizeWin:playEffect()
...
function UILHDShowPrizeWin:replay(itemid_,num)
local index=self.curStep
...
function UILHDShowPrizeWin:playEffectEnd()
...
function UILHDShowPrizeWin:onFinish()
self.playingAnim=false
end



function UILHDShowPrizeWin:addNote(str)
table.insert(self.noteslist,1,str)
self:refreshNotes()
end

function UILHDShowPrizeWin:refreshNotes()
...
function UILHDShowPrizeWin:onClickItem(itemId,index,itemguid,attach)
itemsComponentHelper.onItemClickEx(itemId,index,itemguid,attach)
end

function UILHDShowPrizeWin:onClickClose()
self:closeSelf()
end