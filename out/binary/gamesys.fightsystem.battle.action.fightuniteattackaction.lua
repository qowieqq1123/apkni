def_class('fightUniteAttackAction',fightBaseAction)

function fightUniteAttackAction:__init()
self.typo=fightActionType.UNITE_ATTACK
end

function fightUniteAttackAction:getTypo()
return self.typo
end



function fightUniteAttackAction:init(_round,_rawData,_srcID,_dstID)
self.round=_round
...
function fightUniteAttackAction:exe(immediately)
...
function fightUniteAttackAction:srcFlowText(deltaTime)
self.flowDelay=self.flowDelay-deltaTime
if self.isFlow and self.flowDelay<0 then
local srcEnt=self.battle:getEntity(self.srcIDs[1])
if srcEnt then
...
function fightUniteAttackAction:fetchData(curIndex,parentRawData)
self.uaRawDatas={}
...
function fightUniteAttackAction:logExe(logContent)
...
logStr=FMT.fmt('{0} [联合攻击][{1}]',logStr,dstName)
logContent(logStr)
self.isComplete=true
end

function fightUniteAttackAction:update(deltaTime)
...
function fightUniteAttackAction:onDespwan()
