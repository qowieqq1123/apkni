...
def_class("UIWanBaoXunBaoDui_ShowPrizeWin",UIWindowBase)
...
function UIWanBaoXunBaoDui_ShowPrizeWin:bindComponents()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:unbindComponents()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:onHide()

end

function UIWanBaoXunBaoDui_ShowPrizeWin:__delete()
_this=nil
...
function UIWanBaoXunBaoDui_ShowPrizeWin:onShow(argtable,afterOnloaded)
self.curStep=1
...
function UIWanBaoXunBaoDui_ShowPrizeWin:setAttachCB(callback)
self.attachCallback=callback
end

function UIWanBaoXunBaoDui_ShowPrizeWin:initRewardView()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:refreshAllPos()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:refreswhScrollView()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:refreshItemPos(item,idx)
...
function UIWanBaoXunBaoDui_ShowPrizeWin:refreshItem(item,widget)
...
limit.itemcount=num<=1 and''or mathHelper.formatNumber(num,false)
...
function UIWanBaoXunBaoDui_ShowPrizeWin:playShow()
local delay=0
local d=0.1
local num=self.rewardnum
local grids=self.goodGridPanel:getChildLayoutGroupGridList()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:playEffect()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:replay(itemid_,num)
local index=self.curStep
...
function UIWanBaoXunBaoDui_ShowPrizeWin:playEffectEnd()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:onFinish()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:addNote(str)
table.insert(self.noteslist,1,str)
self:refreshNotes()
end

function UIWanBaoXunBaoDui_ShowPrizeWin:refreshNotes()
...
function UIWanBaoXunBaoDui_ShowPrizeWin:onClickItem(itemId,index,guid,attach)
tipsManager.showTips({itemid=itemId,itemguid=nil,move=TIPS_MOVE_POS.eRight})
end

function UIWanBaoXunBaoDui_ShowPrizeWin:onClickClose()
