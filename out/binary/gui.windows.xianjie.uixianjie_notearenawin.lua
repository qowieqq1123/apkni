...
def_class("UIXianJie_noteArenaWin",UIWindowBase)
...
function UIXianJie_noteArenaWin:bindComponents()
...
function UIXianJie_noteArenaWin:unbindComponents()
...
function UIXianJie_noteArenaWin:onLoaded(...)
self:bindComponents()
...
function UIXianJie_noteArenaWin:__delete()
...
function UIXianJie_noteArenaWin:onShow(argtable,afterOnloaded)
...
function UIXianJie_noteArenaWin:onHide()
if xianjieModel:jude_xianjielog_reddot()~=xianjieModel:Get_reddotchangeflag()then
notifySystem:postNotify(notifyConfig.onXianjieLogReddotChange)
xianjieModel:Set_reddotchangeflag(not xianjieModel:Get_reddotchangeflag())
end
...
function UIXianJie_noteArenaWin:onStartAction(index,widget)

end

function UIXianJie_noteArenaWin:onFreshAction(index,widget)
self:refreshItem(widget,index)
end

function UIXianJie_noteArenaWin:splitStr(str)
return cjson.decode(str)
end


function UIXianJie_noteArenaWin:RefreshWin()
xianjieModel:saveRecord_Arena()
local itemIdList={}
local arenatb=xianjieModel:Get_arenatb()
...
self.onekeyReddot:setActive(xianjieModel:get_ArenaReddot())
self.onekeyBtn:setActive(xianjieModel:get_ArenaReddot())
...
function UIXianJie_noteArenaWin:refreshItem(item,idx)
if item==nil then
return
end
local datatb=xianjieModel:Get_singleArenatb(idx)
...
function UIXianJie_noteArenaWin:Set_BigType(item,cfgid)
...
function UIXianJie_noteArenaWin:Set_Reward(item,len,list,recv,islose)
...
function UIXianJie_noteArenaWin:SetFightBtn(item,datatb,idx)
...
function UIXianJie_noteArenaWin:Set_Righttop(item,datatb)
...
function UIXianJie_noteArenaWin:showjijie(guid)
...
function UIXianJie_noteArenaWin:setRiZhiInfo(item,cfgid,datatb,str_cfg,handletype)
if cfgid>=3 then
...
function UIXianJie_noteArenaWin:SetStr(str_cfg,item,json_str,cfgid,handletype)
local str_2="暂无信息"
...
if handletype==5 then
if cfgid==35 or cfgid==36 then
atkOwnerSceneidx=tbstr[1]
atkActorLeaderName=tbstr[2]
arenaId=tbstr[3]
defActorLeaderName=tbstr[4]
defOwnerSceneidx=tbstr[6]or 0
elseif cfgid==37 or cfgid==38 then
atkOwnerSceneidx=tbstr[1]
atkActorLeaderName=tbstr[2]
arenaId=tbstr[3]
defOwnerSceneidx=tbstr[4]
defActorLeaderName=tbstr[5]
elseif cfgid==39 or cfgid==41 then
atkOwnerSceneidx=tbstr[1]
atkActorLeaderName=tbstr[2]
arenaId=tbstr[3]
if cfgid==39 then
defOwnerSceneidx=tbstr[5]or 0
end
elseif cfgid==40 then
arenaId=tbstr[1]
defOwnerSceneidx=tbstr[2]
atkOwnerSceneidx=tbstr[3]
atkActorLeaderName=tbstr[4]
end
elseif handletype==7 then
arenaId=tbstr[1]
...
function UIXianJie_noteArenaWin:onClearBtn()
local tb=xianjieModel:Find_Arenaguid()
if next(tb)then
...
function UIXianJie_noteArenaWin:onOnekeyBtn()
local tb=xianjieModel:Find_ArenaReward()
if next(tb)then
...
function UIXianJie_noteArenaWin:onTips()
local d={}
d.title='规则说明'
d.mode=3
d.showBlack=true
