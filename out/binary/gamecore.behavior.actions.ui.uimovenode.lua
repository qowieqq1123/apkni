UIMoveNode=simple_class(baseNode)

function UIMoveNode:reset()
...
function UIMoveNode:update(interval)
...
if duration>0 then
self.isplaying=true
local tweener=widget:SetChildDOMove(wIndex,val,duration,function()
self.isplaying=false
self.bComplete=true
self:quicklyTick()
end)
...
widget:SetChildPosition(wIndex,val)
return nodeState.success
end
