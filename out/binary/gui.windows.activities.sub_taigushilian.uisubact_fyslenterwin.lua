...
def_class("UISubAct_fyslEnterWin",UIWindowBase)
...
function UISubAct_fyslEnterWin:bindComponents()
...
function UISubAct_fyslEnterWin:unbindComponents()
...
function UISubAct_fyslEnterWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UISubAct_fyslEnterWin:__delete()
...
function UISubAct_fyslEnterWin:onShow(argtable,afterOnloaded)
if argtable then
...
local len=activitiesHandle_fuyaoBoss:getBossTongGuangIdx(self.actId,self.subType,self.subId,self.bossid)
if len>0 then
local damagevalue=activitiesHandle_fuyaoBoss:getBossJieDuanDamageValue(self.actId,self.subType,self.subId,self.bossid,len)
...
self.monster=self.bossData[1][self.guankaIdx][1]
...
self:initShouLingData()
local red=activitiesHandle_fuyaoBoss:checkIsNewBoss(self.actId,self.subType,self.subId,self.bossid)
self.fbreddot:setActive(red)
end
end


function UISubAct_fyslEnterWin:onHide()

end


function UISubAct_fyslEnterWin:initShouLingData()
...
function UISubAct_fyslEnterWin:refreshShouLingData()

_this.monster=_this.bossData[1][_this.guankaIdx][1]
...
if mosterId==0 then
for k,v in ipairs(_this.monsterCfg.monList)do
if v~=0 then
mosterId=v
break
end
...
local last_pass=activitiesHandle_fuyaoBoss:checkBossNanDuPass(_this.actId,_this.subType,_this.subId,_this.bossid,k-1)
...
_this.shuomingtxt:setText("祖师可以反复挑战无限难度首领，提升最高伤害，获取伤害阶段奖励")
end
...
local damage=activitiesHandle_fuyaoBoss:checkBossNanDuPass(_this.actId,_this.subType,_this.subId,_this.bossid,_this.guankaIdx)
if _this.guankaIdx<#_this.cfg_nandu_list then
...
function UISubAct_fyslEnterWin:nanduOnClick(idx)
if _this.guankaIdx==idx then
return
end
if idx>1 then
local last_pass=activitiesHandle_fuyaoBoss:checkBossNanDuPass(_this.actId,_this.subType,_this.subId,_this.bossid,idx-1)
if last_pass and last_pass<0 then
else
...
function UISubAct_fyslEnterWin:onClickSkill(index)
local skillList={}
skillList=_this.config.texing[_this.monster]or{}
...
loggerUtil.logWarnFMT("伏妖试炼怪物无效怪物特性:{0},{1},{2}",_this.monster)
end
...
UISubAct_fyslEnterWin:showWindow('UISimpleTeXingTipsWin',args)
end


function UISubAct_fyslEnterWin:onBackground()
...
function UISubAct_fyslEnterWin:onFightBtn()
...
fightLaunchController:sendFight(eBattleLaunch.fuyaoshilian,guidList,mcfg.mapId or 0,zfId,
...
fightController.showPrepareWin(fightPreSelectModel.fightType.fuyaoshilian,winArgs,function(...)
UIManager:showWindow('UISubAct_fyslFightExtraWin',{_actid,_subType,_subId,_bossid,tmlist,tmidx})
end)
end


function UISubAct_fyslEnterWin:Getspecialfaze(diziList_id,tmlist)
...
function UISubAct_fyslEnterWin:GetspecialLGfaze()
local disciplesList=discipleLookup:getSortDiscipleList(eDiscipleSortType.eFightSort)
local templist={}
...
function UISubAct_fyslEnterWin:GetspecialCTfaze()
local disciplesList=discipleLookup:getSortDiscipleList(eDiscipleSortType.eFightSort)
local templist={}
...
function UISubAct_fyslEnterWin:onShareBtn()
end
function UISubAct_fyslEnterWin:onRankBtn()
end


function UISubAct_fyslEnterWin:onCloseBtn()
self:onBackground()
end


function UISubAct_fyslEnterWin:onSettingBtn()

end
function UISubAct_fyslEnterWin:startLeaveTick()
...
function UISubAct_fyslEnterWin:updateLeaveTick()
...
function UISubAct_fyslEnterWin:stopLeaveTick()
if self.leaveTick then
self:stopTimerByID(self.leaveTick)
self.leaveTick=nil
end
end
function UISubAct_fyslEnterWin:initMoney()
...
function UISubAct_fyslEnterWin:refreshMoney(val)
local cur=val or itemsModel.getCount(self.costItem)
self.moneyNum:setText(cur)
end
function UISubAct_fyslEnterWin:onMoneyAdd()
self.info:showMoneyBuyPanel(self.costItem)
end
function UISubAct_fyslEnterWin:onMoneyBg()
gainControl:showGainWin(self.costItem)
end
function UISubAct_fyslEnterWin:onActivityEnd(actId,subType,subId)
