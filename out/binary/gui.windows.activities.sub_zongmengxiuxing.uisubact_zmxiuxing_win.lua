...
def_class("UISubAct_ZMXiuXing_Win",UIWindowBase)
...
function UISubAct_ZMXiuXing_Win:bindComponents()
...
function UISubAct_ZMXiuXing_Win:unbindComponents()
...
local hight=9
local abNmae="ui/windows/activities/sub_zongmengxiuxing/zongmengxiuxing_atlas_pak.ab"
local _this

function UISubAct_ZMXiuXing_Win:onLoaded(...)
_this=self
self:bindComponents()
...
function UISubAct_ZMXiuXing_Win:__delete()
...
function UISubAct_ZMXiuXing_Win:onShow(argtable,afterOnloaded)
...
function UISubAct_ZMXiuXing_Win:onHide()
...
function UISubAct_ZMXiuXing_Win:refreshShowModel()
...
function UISubAct_ZMXiuXing_Win:clearShowItemModel()
...
function UISubAct_ZMXiuXing_Win:showItemModel_DaoBing(oItemId)
...
function UISubAct_ZMXiuXing_Win:showItemModel_NormalItemModel(itemId,modelParams)
...
function UISubAct_ZMXiuXing_Win:showItemModel_ZuShi(itemId)
local itemCfg=itemsConfig.getConfig(itemId)
...
function UISubAct_ZMXiuXing_Win:showItemModel_DiZi(itemId)
...
function UISubAct_ZMXiuXing_Win:showItemModel_HeadKuangOrQiPaoKuangOrEmot(itemId)
local itemCfg=itemsConfig.getConfig(itemId)
...
function UISubAct_ZMXiuXing_Win:showItemModel_GuBao(itemId,relevantPram)
...
function UISubAct_ZMXiuXing_Win:showItemModel_NormalItemImage(itemId,count)
...
function UISubAct_ZMXiuXing_Win:createHorContent(targetIndex)
...
function UISubAct_ZMXiuXing_Win:refreshHorContent()
local horContentLen=self.taskCfgLen
...
function UISubAct_ZMXiuXing_Win:refreshVerContent()
...
function UISubAct_ZMXiuXing_Win:createProgressContent()
...
function UISubAct_ZMXiuXing_Win:refreshProgressContent(isJump)
...
function UISubAct_ZMXiuXing_Win:handleCfg()
local taskCfgLookUp={}
local taskDayLookUp={}
...
function UISubAct_ZMXiuXing_Win:sortTaskDayCfg(day)
if day then
...
function UISubAct_ZMXiuXing_Win:getReddotFlag_taskDay(taskDay)
return TempState.eRecv==self:getTaskDayState(taskDay)
end

function UISubAct_ZMXiuXing_Win:getLockFlag_taskDay(taskDay)
return TempState.eNotRecv==self:getTaskDayState(taskDay)
end

function UISubAct_ZMXiuXing_Win:getRecvedFlag_taskDay(taskDay)
return TempState.eRecved==self:getTaskDayState(taskDay)
end

function UISubAct_ZMXiuXing_Win:getTaskDayState(taskDay)
...
function UISubAct_ZMXiuXing_Win:getTaskState(taskDay,index)
local dayTaskCfg=self.taskCfgLookUp[taskDay]
...
function UISubAct_ZMXiuXing_Win:getAllTaskPro()
local allcount=self.sub_actInfo:getAllTaskPro()
return allcount
end

function UISubAct_ZMXiuXing_Win:getTargetState(index)
local state=self.sub_actInfo:getTargetState(index)
return state
end


function UISubAct_ZMXiuXing_Win:refreshActTimer()
...
function UISubAct_ZMXiuXing_Win:onRewardInfoBtn()
self:onClickItem(self.showModelItemId)
end


function UISubAct_ZMXiuXing_Win:onClickItem(itemId,index,guid,attach)
...
function UISubAct_ZMXiuXing_Win:onHorItemClick(index)
...
function UISubAct_ZMXiuXing_Win:onVeritemRecvBtn(index)
...
function UISubAct_ZMXiuXing_Win:onVeritemGotoBtn(index)
local curTaskDay=self.curTaskDay
local curDayTaskCfg=self.taskCfgLookUp[curTaskDay]
local verItemCfg=curDayTaskCfg[index]
...
function UISubAct_ZMXiuXing_Win:onProgressItemClick(index)

end

function UISubAct_ZMXiuXing_Win:onProgressItemAddClick(index)
...
function UISubAct_ZMXiuXing_Win:getSelectList()
return self.data.selectList
end

function UISubAct_ZMXiuXing_Win:setSelectList(index,value)
...
function UISubAct_ZMXiuXing_Win:onProgressItemChangeClick(index)
local itemList={}
local temp={}
local cfg=self.target_rewards[index]
local rewards=cfg[2]
...
function UISubAct_ZMXiuXing_Win:onClickRewardItem(index,...)
...
function UISubAct_ZMXiuXing_Win:onItemClick()
self:onClickItem(self.showModelItemId)
end

function UISubAct_ZMXiuXing_Win:onLeftBtn()
...
function UISubAct_ZMXiuXing_Win:onRightBtn()
local biglen=#self.config.bigRewards
self.bigRewardIndex=self.bigRewardIndex+1
...
function UISubAct_ZMXiuXing_Win.onShowPrize(prizeType,temp,effectData)
...
function UISubAct_ZMXiuXing_Win:checkUseItme(temp)
local useFlag=false
...
function UISubAct_ZMXiuXing_Win.onItemUse(itemid,num)
if _this==nil then return end
