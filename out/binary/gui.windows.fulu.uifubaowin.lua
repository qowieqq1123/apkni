...
def_class("UIFuBaoWin",UIWindowBase)
...
function UIFuBaoWin:bindComponents()

self.scrollview=UIObject.get(self,0)
self.handbook=UIButton.get(self,1)
...
function UIFuBaoWin:unbindComponents()
...
function UIFuBaoWin:__delete()
self:unbindComponents()

_this=nil
end

function UIFuBaoWin.on_combobox_change(index)
local color=_this.indexToColor[index+1]
_this.currColor=color
_this:refreshFuBaoList(color)
end

function UIFuBaoWin:refresh()
if self.currColor then
self:refreshFuBaoList(_this.currColor)
end
end
...
function UIFuBaoWin:onShow(argtable,afterOnloaded)
...
function UIFuBaoWin:onHide()

end

function UIFuBaoWin:initSort()
self.option={
'所有',
'5阶符宝',
'4阶符宝',
...
function UIFuBaoWin:getFuBaoDatas(color)
...
function UIFuBaoWin:refreshFuBaoList(color)
...
function UIFuBaoWin:checkMake(id)
local cfg=cfgHelper.get1(cfg_fubaofangconfig_get,id)
...
function UIFuBaoWin.onCreateClick(id)
if _this:checkMake(id)then
UIManager:showWindow('UIFuBaoDrawWin',{id,_this.sfId,_this.bdData.un_build_id})
end
end

function UIFuBaoWin:setCostItem(item,index,data)
if data then
...
function UIFuBaoWin:onHandbook()
