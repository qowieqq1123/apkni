...
local subActivityInfo_roledoubleLottery={name='roledoubleLottery'}

function subActivityInfo_roledoubleLottery:onInit()
...
function subActivityInfo_roledoubleLottery:onStart()

end

function subActivityInfo_roledoubleLottery:onDelete()

end

function subActivityInfo_roledoubleLottery:checkReddot()
...
function subActivityInfo_roledoubleLottery:checkBoxReddotByIndex(boxIndex)
...
function subActivityInfo_roledoubleLottery:checkTargetReddot()
...
function subActivityInfo_roledoubleLottery:onShowPrize(prizeType,rewards,effectData)
if prizeType~=ePrizeType.eDoubleLotteryBatch2 or self.act_id~=effectData.actid or self.sub_act_id~=effectData.act2id then
return
end
...
UIManager:showWindow('UISubAct_roledoubleLottery_PrizeWin',args)

UIManager:invokeUIMethod("UISubAct_roledoubleLotteryWin","refresh")
UIManager:invokeUIMethod("UISubAct_roledoubleLottery_TargetWin","refresh",true)


reddotControl.on_change_catch_type(CATCH_TYPE.eActivityChange,self.sub_act_type)
end

local isSkipAnim=userActorSetting.get('skipRoleDoubleLotteryAnim',false)
if not isSkipAnim and UIManager:isActive('UISubAct_roledoubleLotteryWin')then
...
UIManager:invokeUIMethod("UISubAct_roledoubleLotteryWin","playLotteryAnim",maxItemColor,boxId,callback)
else
callback()
end
end

function subActivityInfo_roledoubleLottery:getRemainingFreeDrawCount(boxIndex)
...
function subActivityInfo_roledoubleLottery:reqDoubleLotteryDraw(boxIndex,drawType)
local remainingDrawCount=self:getRemainingFreeDrawCount(boxIndex)
...
function subActivityInfo_roledoubleLottery:reqDoubleLotteryGetTargetReward()
local json_str=jsonHelper.encode({2})
activitiesController:sendProtocol(actSendType.eComonReqHandle,self.act_id,self.sub_act_type,self.sub_act_id,json_str)
end

return subActivityInfo_roledoubleLottery