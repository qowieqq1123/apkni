...
def_class("UIMDiscipleSelect_tiantishilian",UIMDiscipleSelect)
...
function UIMDiscipleSelect_tiantishilian:onLoaded(...)
self:bindComponents()
...
function UIMDiscipleSelect_tiantishilian:__delete()
self:unbindComponents()
_this=nil
end

function UIMDiscipleSelect_tiantishilian:getDiscipleDatas()
local list={}
...
function UIMDiscipleSelect_tiantishilian:onShow(argtable,afterOnloaded)
self.args=argtable
...
function UIMDiscipleSelect_tiantishilian:onHide()

end

function UIMDiscipleSelect_tiantishilian:initView()

end

function UIMDiscipleSelect_tiantishilian:refreshView()
self:refreshScrollView()
self:refreshButtons()
end

function UIMDiscipleSelect_tiantishilian:reSelectDisciple(default_idx)
...
function UIMDiscipleSelect_tiantishilian:getNetDataList()
...
function UIMDiscipleSelect_tiantishilian:initDiscipleList()
...
local checkCurrent=self.dzIdStr and data.discipleguidStr==self.dzIdStr
...
local jiyuan=UIDiscipleModel:getDiscipleBaseAttr(guid,DISCIPLE_BASE_ATTR_TYPE.eJiYuan)
locData.jiyuan=jiyuan
...
function UIMDiscipleSelect_tiantishilian:refreshScrollView()
...
function UIMDiscipleSelect_tiantishilian:checkRecommend(guid_str)
...
return self.tuijian_dizi_str~=nil and mathHelper.compareInt64(guid_str,self.tuijian_dizi_str)
end

function UIMDiscipleSelect_tiantishilian:refreshItem(id,item)
local index=id+1
...
local meili_desc
local meili_title_str=UIDiscipleModel:getDiscipleBaseAttrName(DISCIPLE_BASE_ATTR_TYPE.eJiYuan)
local meili_str=tostring(data.jiyuan)
...
function UIMDiscipleSelect_tiantishilian.onDescSlotClick(disIdx,speIdx)
...
function UIMDiscipleSelect_tiantishilian:refreshSelect(index,flag)
local item=self.grids[index-1]
local item_cmp_index_=discipleSelectController.item_cmp_index
item:SetChildActive(item_cmp_index_.img_select,flag)
end

function UIMDiscipleSelect_tiantishilian:onClickItem(index)
...
function UIMDiscipleSelect_tiantishilian:refreshButtons()
local c=#self.disciplelist
if c>0 then
...
function UIMDiscipleSelect_tiantishilian:OnEnable()

end


function UIMDiscipleSelect_tiantishilian:OnDisable()

end



function UIMDiscipleSelect_tiantishilian:onClickClose()
if not self.wait_replace_building_mgr then
self.parentWin:closeSelf()
end
end

function UIMDiscipleSelect_tiantishilian:onBtnWork()
if self.select_index then
local data=self.disciplelist[self.select_index]
local disdata=data.disciple
local guid=disdata.discipleguid
local checkCurrent=data.checkCurrent
local checkChuiwei=data.checkChuiwei
if not checkChuiwei then
if checkCurrent then
...
function UIMDiscipleSelect_tiantishilian:onBtnFire()

end

function UIMDiscipleSelect_tiantishilian.onChangeInteractNPCDZ()


_this:onClickClose()
end

function UIMDiscipleSelect_tiantishilian:onSearchBtn()
...
function UIMDiscipleSelect_tiantishilian:onSearchCancelBtn()
...
function UIMDiscipleSelect_tiantishilian:onSearchChange(str)
self:refreshInputBtns(str)
end

function UIMDiscipleSelect_tiantishilian:refreshInputBtns(change_str)
...
function UIMDiscipleSelect_tiantishilian:clearSearchInput()
self.inputstr=nil
local str=''
