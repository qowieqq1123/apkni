







def_class("UIEnterTotalTouZiActivity",UICloneObject)





UIEnterTotalTouZiActivity.abName="ui/windows/main/uientrychild/uienternomalitem.ab"

UIEnterTotalTouZiActivity.assetName="UIEnterNomalItem"


function UIEnterTotalTouZiActivity:bindComponents()

self.icon=UIButton.get(self,0)
self.reddot=UIObject.get(self,1)
self.name=UIText.get(self,2)
self.time=UIText.get(self,3)
self.timeBg=UIObject.get(self,4)
self.qipao=UIObject.get(self,5)
self.qipaoText=UIText.get(self,6)
self.model=UIObject.get(self,7)
self.clickBg=UIButton.get(self,8)
self.extendbg=UIObject.get(self,9)
self.lldhQiPao=UIObject.get(self,10)

self.icon:setButtonClick(function()self:onIcon()end)

self.clickBg:setButtonClick(function()self:onClickBg()end)

end


function UIEnterTotalTouZiActivity:unbindComponents()
local _UIObject_release=UIObject.release
_UIObject_release(self.icon);self.icon=nil;
_UIObject_release(self.reddot);self.reddot=nil;
_UIObject_release(self.name);self.name=nil;
_UIObject_release(self.time);self.time=nil;
_UIObject_release(self.timeBg);self.timeBg=nil;
_UIObject_release(self.qipao);self.qipao=nil;
_UIObject_release(self.qipaoText);self.qipaoText=nil;
_UIObject_release(self.model);self.model=nil;
_UIObject_release(self.clickBg);self.clickBg=nil;
_UIObject_release(self.extendbg);self.extendbg=nil;
_UIObject_release(self.lldhQiPao);self.lldhQiPao=nil;
end






local iconname='button_hdrk_0002'


function UIEnterTotalTouZiActivity:onLoaded(...)
self:bindComponents()
self:addReddotNotify(REDDIT_TYPE.eTotalTouZiActivity,function()
self:freshReddot()
end)
self:addNotify(notifyConfig.onTouZiAutoTotalReddotChange,function()
self:freshReddot()
end)
end


function UIEnterTotalTouZiActivity:__delete()
self:stopSelfTimer()
self:unbindComponents()
end




function UIEnterTotalTouZiActivity:onShow(argtable,afterOnloaded)
enterConfig.preInitEnter(self)
local info=argtable.info
local enterIconType=info.enterIconType
local enterType=info.enterType
local cfg=enterConfig.getConfig(enterIconType,enterType)
local abname=enterConfig.getSpriteAB()
self.widget:SetChildCSImageSprite(0,abname,iconname)
self.widget:SetChildActive(0,true)
self.widget:SetChildUIModelRemoveTarget(7)

self.widget:SetChildActive(1,false)

self.widget:SetChildText(2,'')

self.widget:SetChildText(3,'')

self.widget:SetChildActive(4,false)

self.widget:SetChildButtonClick(0,function()
UIFullTotalTouZiActivityontrol:showMenuWindow()
end,true)
self.widget:SetChildButtonClick(self.clickBg:getID(),function()
UIFullTotalTouZiActivityontrol:showMenuWindow()
end,true)

self:startTimer()

self.extendbg:setActive(argtable.isEx or false)
self.qipao:setActive(false)

self.widget:SetChildActive(self.reddot:getID(),UIFullTotalTouZiActivityontrol:getTotalReddot())
end


function UIEnterTotalTouZiActivity:onHide()

end



function UIEnterTotalTouZiActivity:onIcon()

end

function UIEnterTotalTouZiActivity:onClickBg()

end

function UIEnterTotalTouZiActivity:startTimer()


















self:stopSelfTimer()
local func=function()
if self==nil or self.isClose or self.widget==nil then return end
local leftTime=UIFullTotalTouZiActivityontrol:getMinLeftTime()
local isFinish=leftTime==nil or leftTime<=0
if self.leftTime~=leftTime then
self.widget:SetChildActive(4,not isFinish)
self.leftTime=leftTime
end
if isFinish then
return
end
self.widget:SetChildText(3,timeHelper.format_time_stamp3(self.leftTime))
end
self.timer=self:setTimer(1,0,func)
func()
end

function UIEnterTotalTouZiActivity:stopSelfTimer()
if self.timer then
self:stopTimerByID(self.timer)
end
self.timer=nil
end

function UIEnterTotalTouZiActivity:freshReddot()
self.widget:SetChildActive(self.reddot:getID(),UIFullTotalTouZiActivityontrol:getTotalReddot())
end
