...
worldTaskSky_Tour=simple_class(worldTaskSky)

function worldTaskSky_Tour:__init(data)
...
function worldTaskSky_Tour:start(serverTime)
...
function worldTaskSky_Tour:showUnitObject()
...
function worldTaskSky_Tour:startNormalMove(fast)
self:showUnitObject()
...
function worldTaskSky_Tour:createNormalMove(fast)
...
local partTime1=self.jumpTime*math.floor(1/2)
...
local ways={}
self:handleForwardWays(ways,1,disObj,fast,true,0,partTimes)
...
function worldTaskSky_Tour:onNormalMoveStep(fast)
...
function worldTaskSky_Tour:startWanderMove()
if not self.objects[1]then
self:showUnitObject()
end
...
function worldTaskSky_Tour:getWanderPath(startIdx,endIdx)
...
function worldTaskSky_Tour:doAiAction(kind,aiInfo,infoIdx,sPos,ePos)
...
function worldTaskSky_Tour:extractAIIndex()
...
function worldTaskSky_Tour:extractPointIndex()
...
function worldTaskSky_Tour:createWanderWalkMove(startIdx,endIdx,aiInfo)
...
function worldTaskSky_Tour:createWanderAnimationPoint(id,minTime,maxTime,param)
local animation=id
...
function worldTaskSky_Tour:createWanderEffectPoint(id,minTime,maxTime,param)
local effect=id
local duration=maxTime and math.random(minTime,maxTime)or minTime
...
function worldTaskSky_Tour:createWanderEmotPoint(info,minTime,maxTime,param)
local text=info[1]
local role=info[2]
local duration=maxTime and math.random(minTime,maxTime)or minTime
...
function worldTaskSky_Tour:createWanderObjectShow(info,minTime,maxTime,param)
...
function worldTaskSky_Tour:createWanderColorChange(info,minTime,maxTime,param)
...
function worldTaskSky_Tour:quit()
worldTaskBase.quit(self)
self.wanderObj=nil
end

function worldTaskSky_Tour:finish()
...
function worldTaskSky_Tour:isWaitingBack()
