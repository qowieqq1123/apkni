...
local subActivityInfo_xinashichouka={name='xinashichouka'}

function subActivityInfo_xinashichouka:onInit()

end

function subActivityInfo_xinashichouka:onStart()

end

function subActivityInfo_xinashichouka:onUpdate()

end

function subActivityInfo_xinashichouka:onDelete()

end

function subActivityInfo_xinashichouka:checkReddot()
local data=self.data
if data then
local sub_actcfg=self:getSubActConfig()

if activitiesHandle_xianshichouka.checkHasFree(self.sub_act_type,self.sub_act_id,data.free)then
return true
end
...
function subActivityInfo_xinashichouka:handleNote(note)
...
function subActivityInfo_xinashichouka:initNotes(loglist)
local notesList={}
if loglist then
...
function subActivityInfo_xinashichouka:setNotesNew(loglist)
...
function subActivityInfo_xinashichouka:getOneNoteStr()
local data=self.data
...
function subActivityInfo_xinashichouka:clearNoteSelect()
local data=self.data
if data then
...
function subActivityInfo_xinashichouka:reqXianShiChouKa_checkDailyRewardsIsGot()
...
function subActivityInfo_xinashichouka:reqXianShiChouKa_getDailyRewards()
...
local win2=UIManager:findActiveWindow('UISubAct_xianshichoukaWin')
if win2 then
...
return subActivityInfo_xinashichouka