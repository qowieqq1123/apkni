...
def_class("UIWorldMapWin",UIWindowBase)
...
function UIWorldMapWin:bindComponents()

self.Head=UIObject.get(self,0)
self.ActRoot=UIObject.get(self,1)
self.AreaRoot=UIObject.get(self,2)
self.Clouds=UIObject.get(self,3)
self.HeadBg=UIObject.get(self,4)
...
function UIWorldMapWin:unbindComponents()
...
local increaseTime=0.5
local increaseValue=1.25
local _tween=nil
...
function UIWorldMapWin:onLoaded(...)
self:bindComponents()
_this=self
self.waitLAct={}
...
function UIWorldMapWin:__delete()
...
function UIWorldMapWin:onShow(argtable,afterOnloaded)
playerController:setHeadIcon(self.winlua,self.Head:getID(),{scale=0.6})

local world=worldMapController:getEnterScene()
local allWorldCfg=cfg_worldconfig()
local enterWorldCfg=allWorldCfg[world]
...
function UIWorldMapWin:onHide()

end



function UIWorldMapWin.onClickArea(index)
...
_this:doAnimationScale(callback)
worldMapController:setCurrentScene(index)
...
function UIWorldMapWin.refreshMapItem(index)
...
function UIWorldMapWin:doAnimationScale(callback)
self:clearTweener()
...
_tween=Lua.SequenceProxy.New()
_tween:Join(self.Root:setChildDOScale(increaseValue,increaseTime))
_tween:Join(self.Root:setChildCanvasGroupDOFade(0,increaseTime))
_tween:OnComplete(onFinish)
end

function UIWorldMapWin:clearTweener()
if _tween then
_tween:Kill()
_tween=nil
end
...
function UIWorldMapWin:startWaitLActTick(actId,index)
...
function UIWorldMapWin:stopWaitLActTick(actId)
if actId then
...
function UIWorldMapWin:updateWaitLActTick()
local stopList={}
...
if leastTime>0 then
item:SetChildText(_actCmp.conditionTx,FMT.fmt("{0}后{1}现世",timeHelper.formatSimpleTime(leastTime),actCfg.name))
else
...
function UIWorldMapWin:refreshActTickInfo()
self:stopWaitLActTick()
...
if jumpManager:jump({id=JUMP_TYPE.eLimitActivity,args={actID=actId,extraParams=extraParams}})then
local position=cfgHelper.get2(cfg_worldconfig_get,world,"cameraPos")
...
function UIWorldMapWin.on_building_event(event,level,exp,lastLv)
if event==buildingEvent.zongmenLevelUp and level~=lastLv then
_this:refreshActTickInfo()
end
end

function UIWorldMapWin.onNewDay()
_this:refreshActTickInfo()
end