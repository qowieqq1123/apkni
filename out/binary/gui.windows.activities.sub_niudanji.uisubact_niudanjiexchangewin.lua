...
def_class("UISubAct_NiuDanJiExchangeWin",UIWindowBase)
...
function UISubAct_NiuDanJiExchangeWin:bindComponents()
...
self.currentTx=UIText.get(self,6)
self.moneyBtn=UIButton.get(self,7)
self.leftBtn=UIButton.get(self,8)
...
function UISubAct_NiuDanJiExchangeWin:unbindComponents()
...
function UISubAct_NiuDanJiExchangeWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UISubAct_NiuDanJiExchangeWin:__delete()
...
function UISubAct_NiuDanJiExchangeWin:onShow(argtable,afterOnloaded)
...
function UISubAct_NiuDanJiExchangeWin:onHide()

end

function UISubAct_NiuDanJiExchangeWin.on_money_changed(mType,oldValue,newValue)
...
function UISubAct_NiuDanJiExchangeWin:onCloseBtn()
if self.loaded then
self:closeSelf()
end
end

function UISubAct_NiuDanJiExchangeWin:onBackground()
self:onCloseBtn()
end

function UISubAct_NiuDanJiExchangeWin:refreshStar()
...
function UISubAct_NiuDanJiExchangeWin:refreshView(actId,subType,subId)
...
function UISubAct_NiuDanJiExchangeWin:refreshList()
self.dataList={}
...
function UISubAct_NiuDanJiExchangeWin:onLeftBtn()
...
function UISubAct_NiuDanJiExchangeWin:onRightBtn()
...
function UISubAct_NiuDanJiExchangeWin:onMoneyBtn()
local moneyType=self.config.money[1]
gainControl:showGainWin(moneyType)
end

function UISubAct_NiuDanJiExchangeWin:onLoadBackFinish()
...
call_activitiesHandle_func("activitiesHandle_niudanji","reqExchange",_this.activityId,_this.subId,itemData.index,num)
end
}
...
allowclickBG='false',
okcallback=function(...)
call_activitiesHandle_func("activitiesHandle_niudanji","reqExchange",_this.activityId,_this.subId,itemData.index,least)
end,
showclosebtn=true,
}
