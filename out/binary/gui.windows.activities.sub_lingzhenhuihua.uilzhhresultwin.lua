...
def_class("UILZHHResultWin",UIWindowBase)
...
function UILZHHResultWin:bindComponents()
...
function UILZHHResultWin:unbindComponents()
...
function UILZHHResultWin:onLoaded(...)
self:bindComponents()
_this=self
...
function UILZHHResultWin:__delete()
_this=nil
...
function UILZHHResultWin:onShow(argtable,afterOnloaded)
...
self.roletopScore=argtable.roletopScore or 0
...
function UILZHHResultWin:onHide()

end

function UILZHHResultWin:refreshResultPanel()
...
if self.roletopScore then
self.nowTopScore=self.roletopScore
end
...
local allRewardCfg=self.config.stage
local originalLevel=1
local nowLevel=1
local maxLevelId=#allRewardCfg
local rewardList_lookup={}
for i=1,#allRewardCfg do
local cfg=allRewardCfg[i]
local score=cfg[1]
local lastLevel=i>1 and i-1 or nil
local lastLevelScore=lastLevel and allRewardCfg[lastLevel][1]or 0
...
local lastLevelScore=lastLevel and allRewardCfg[lastLevel][1]or 0
local levelMaxScore=allRewardCfg[self.curLevel][1]
local maxScore=allRewardCfg[maxLevelId][1]
...
function UILZHHResultWin:onProgressAniFinish()
if not _this then
return
end
local allRewardCfg=self.config.stage
...
function UILZHHResultWin:onCloseClick()
...
function UILZHHResultWin:onBg()
self:onCloseClick()
end


function UILZHHResultWin:onClickRewardItem(itemId,index,guid,attach)
if itemId==-1 then
return
end
