...
activitiesHandle_roledoubleLottery=new_activitiesHandle('activitiesHandle_roledoubleLottery',activitiesHandle)

function activitiesHandle_roledoubleLottery:onInit()

end

function activitiesHandle_roledoubleLottery.recv_247_4(args)
local subType=SUB_ACTIVITY_TYPE.eTianMoBaoXia2
local actId=args[1]
local subId=args[2]
...
function activitiesHandle_roledoubleLottery.recv_247_5(actId,subId,maxGotRewardIndex)
local subType=SUB_ACTIVITY_TYPE.eTianMoBaoXia2
local info=activitiesModel:getSubActInfo(actId,subType,subId)
if not info then return end
if info:hasData()then
