...
def_class("UILZDKPrizeWin",UIWindowBase)
...
function UILZDKPrizeWin:bindComponents()
...
function UILZDKPrizeWin:unbindComponents()
...
function UILZDKPrizeWin:onLoaded(...)
_this=self
self:bindComponents()
...
function UILZDKPrizeWin.onItemUse(itemid,num)
...
function UILZDKPrizeWin:onHide()

end

function UILZDKPrizeWin:__delete()
_this=nil
...
function UILZDKPrizeWin:onShow(argtable,afterOnloaded)
self.curStep=1
self:stopAllTimer()
...
local color=itemConfig.color or 1
...
logErr('没有奖励却打开了UILZDKPrizeWin?')
end
end

function UILZDKPrizeWin:setAttachCB(callback)
self.attachCallback=callback
end

function UILZDKPrizeWin:initRewardView()
...
function UILZDKPrizeWin:refreshAllPos()
...
function UILZDKPrizeWin:refreswhScrollView()
...
function UILZDKPrizeWin:refreshItemPos(item,idx)
...
function UILZDKPrizeWin:refreshItem(item,widget)
...
local limit=self:getItemConf(item)
local porp=itemsComponentHelper.getCommonFillData(nil,limit)
...
function UILZDKPrizeWin:playShow()
local delay=0
local d=0.1
local num=self.rewardnum
local grids=self.goodGridPanel:getChildLayoutGroupGridList()
...
function UILZDKPrizeWin:playEffect()
...
function UILZDKPrizeWin:replay(itemid_,num)
local index=self.curStep
...
function UILZDKPrizeWin:playEffectEnd()
...
function UILZDKPrizeWin:onFinish()
...
function UILZDKPrizeWin:addNote(str)
table.insert(self.noteslist,1,str)
self:refreshNotes()
end

function UILZDKPrizeWin:refreshNotes()
...
function UILZDKPrizeWin:onClickItem(itemId,index,itemguid,attach)
itemsComponentHelper.onItemClick(itemId,index,itemguid,attach)
end

function UILZDKPrizeWin:onClickClose()
...
function UILZDKPrizeWin:getItemConf(item)
local itemguid=item.itemguid
local itemid=item.itemid
local itemConfig=itemsConfig.getConfig(itemid)
local num=item.num or 1
...
conf.showbg=true
conf.select=false
conf.color=eQualityColor.eRed
conf.iconName=iconHelper.getIconName(itemid)
conf.itemcount=num<=1 and''or num
conf.showCountBG=num>1
conf.graynum=0
conf.name=itemConfig.name
conf.itemid=itemid
conf.guid=item.itemguid or-1
conf.attach=''
conf.colorEffect=nil
conf.stageStr=""
conf.showStageBg=false
conf.range=false
return conf
end